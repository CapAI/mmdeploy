...
def test_array_resize():
    ...
    try:
        ...
        assert str(e).startswith("cannot resize this array: it does not own its data")  # noqa: PT017
...
def test_pass_array_pyobject_ptr_return_sum_str_values_ndarray():
    ...
    assert (
        m.pass_array_pyobject_ptr_return_sum_str_values(np.array(WrapWithPyValueHolder(-3, "four", 5.0), dtype=object))
        == "-3four5.0"
    )
