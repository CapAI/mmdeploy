...
class NCNNWrapper(BaseWrapper):
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if batch_size > 1:
            logger.warning(f"ncnn only support batch_size = 1, but given {batch_size}")
        for input_tensor in input_list[1:]:
            assert input_tensor.size(0) == batch_size, "All tensors should have same batch size"
            assert input_tensor.device.type == "cpu", "ncnn only supports cpu device"
        # set output names
        ...
        for batch_id in range(batch_size):
            ...
            for name in output_names:
                ...
                if mat.empty():
                    logger.warning(f'The "{name}" output of ncnn model is empty.')
                    continue
    ...
    def __ncnn_execute(self, extractor: ncnn.Extractor, output_names: Sequence[str]) -> Dict[str, ncnn.Mat]:
