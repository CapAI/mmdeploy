...
def create_runtime(onnx_file: str, engines: List[pplnn.Engine]) -> pplnn.Runtime:
    ...
    assert runtime_builder is not None, "Failed to create " "onnx.RuntimeBuilder."
    ...
    if device_id == -1:
        ...
        if disable_avx512:
            ...
            if status != pplcommon.RC_SUCCESS:
                logger.error("x86 engine Configure() failed: " + pplcommon.GetRetCodeStr(status))
                sys.exit(1)
        ...
        if quick_select:
            ...
            if status != pplcommon.RC_SUCCESS:
                logger.error("cuda engine Configure() failed: " + pplcommon.GetRetCodeStr(status))
                sys.exit(1)

        if input_shapes is not None:
            status = cuda_engine.Configure(pplnn.cuda.ENGINE_CONF_SET_INPUT_DIMS, input_shapes)
            if status != pplcommon.RC_SUCCESS:
                logger.error(
                    "cuda engine Configure(ENGINE_CONF_SET_INPUT_DIMS) " "failed: " + pplcommon.GetRetCodeStr(status)
                )
                sys.exit(1)

        if export_algo_file is not None:
            status = cuda_engine.Configure(pplnn.cuda.ENGINE_CONF_EXPORT_ALGORITHMS, export_algo_file)
            if status != pplcommon.RC_SUCCESS:
                logger.error(
                    "cuda engine Configure(ENGINE_CONF_EXPORT_ALGORITHMS) " "failed: " + pplcommon.GetRetCodeStr(status)
                )
                sys.exit(1)

        if import_algo_file is not None:
            status = cuda_engine.Configure(pplnn.cuda.ENGINE_CONF_IMPORT_ALGORITHMS, import_algo_file)
            if status != pplcommon.RC_SUCCESS:
                logger.error(
                    "cuda engine Configure(ENGINE_CONF_IMPORT_ALGORITHMS) " "failed: " + pplcommon.GetRetCodeStr(status)
                )
                sys.exit(1)
