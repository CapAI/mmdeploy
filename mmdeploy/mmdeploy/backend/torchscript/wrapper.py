...
class TorchscriptWrapper(BaseWrapper):
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning("Can not import torchvision. " "Models require ops in torchvision might not available.")
        super().__init__(output_names)
        self.ts_model = model
        if isinstance(self.ts_model, str):
            self.ts_model = torch.jit.load(self.ts_model)

        assert isinstance(self.ts_model, torch.jit.RecursiveScriptModule), "failed to load torchscript model."

        self._input_names = input_names
        self._output_names = output_names

    def forward(
        self, inputs: Union[torch.Tensor, Sequence[torch.Tensor], Dict[str, torch.Tensor]]
        ...
        if is_dict_inputs:
            # inputs to dict
            assert self._input_names is not None, "input names have not been given."
    ...
    def __torchscript_execute(self, inputs: Sequence[torch.Tensor]) -> Sequence[torch.Tensor]:
