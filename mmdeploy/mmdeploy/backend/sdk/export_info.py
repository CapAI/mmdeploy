...
def get_task(deploy_cfg: mmengine.Config) -> Dict:
    ...
    return dict(task=task_name, codebase=codebase_name, version=codebase_version)


def get_model_name_customs(
    deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, work_dir: str, device: str
) -> Tuple:
    ...
    task_processor = build_task_processor(model_cfg=model_cfg, deploy_cfg=deploy_cfg, device=device)
...
def get_models(
    deploy_cfg: Union[str, mmengine.Config], model_cfg: Union[str, mmengine.Config], work_dir: str, device: str
) -> List:
    ...
    if get_partition_config(deploy_cfg) is not None:
        ir_name = get_partition_config(deploy_cfg)["partition_cfg"][0]["save_file"]
    weights = ""
...
def get_inference_info(deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, work_dir: str, device: str) -> Dict:
    ...
    if backend in (Backend.TORCHSCRIPT, Backend.RKNN):
        ...
        if get_partition_config(deploy_cfg) is not None:
            output_names = get_partition_config(deploy_cfg)["partition_cfg"][0]["output_names"]
...
def get_preprocess(deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, device: str):
    task_processor = build_task_processor(model_cfg=model_cfg, deploy_cfg=deploy_cfg, device=device)
    ...
    if transforms[0]["type"] != "Lift":
        assert (
            transforms[0]["type"] == "LoadImageFromFile"
        ), "The first item type of pipeline should be LoadImageFromFile"
    return dict(
        type="Task", module="Transform", name="Preprocess", input=["img"], output=["prep_output"], transforms=transforms
    )


def get_postprocess(
    deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, work_dir: str, device: str, **kwargs
) -> Dict:
...
def get_deploy(deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, work_dir: str, device: str) -> Dict:
    """Get the inference information for pipeline.json.
    ...
    _, customs = get_model_name_customs(deploy_cfg, model_cfg, work_dir=work_dir, device=device)
...
def get_pipeline(deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, work_dir: str, device: str) -> Dict:
    """Get the inference information for pipeline.json.
    ...
    infer_info = get_inference_info(deploy_cfg, model_cfg, work_dir=work_dir, device=device)
    postprocess = get_postprocess(deploy_cfg, model_cfg, work_dir, device=device)
    task = get_task_type(deploy_cfg)
    ...
    return dict(pipeline=dict(input=input_names, output=output_names, tasks=[preprocess, infer_info, postprocess]))


def get_detail(deploy_cfg: mmengine.Config, model_cfg: mmengine.Config, pth: str) -> Dict:
...
def export2SDK(
    ...
    **kwargs,
    ...
    mmengine.dump(deploy_info, "{}/deploy.json".format(work_dir), sort_keys=False, indent=4)
    mmengine.dump(pipeline_info, "{}/pipeline.json".format(work_dir), sort_keys=False, indent=4)
    mmengine.dump(detail_info, "{}/detail.json".format(work_dir), sort_keys=False, indent=4)
