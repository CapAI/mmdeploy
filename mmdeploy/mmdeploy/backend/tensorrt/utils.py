    ...
    **kwargs,
    ...
    if cuda_version is not None:
        ...
        if version_major < 11:
            ...
            tactic_source = config.get_tactic_sources() - (1 << int(trt.TacticSource.CUBLAS_LT))
    ...
    if int8_mode:
        ...
        config.int8_calibrator = HDF5Calibrator(
            ...
            device_id=device_id,
            algorithm=int8_param.get("algorithm", trt.CalibrationAlgoType.ENTROPY_CALIBRATION_2),
        )
        if version.parse(trt.__version__) < version.parse("8"):
