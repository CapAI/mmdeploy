...
class ONNXNMSop(torch.autograd.Function):
    ...
    @staticmethod
    def symbolic(
        g, boxes: Tensor, scores: Tensor, max_output_boxes_per_class: int, iou_threshold: float, score_threshold: float
    ):
        ...
        if not sym_help._is_value(max_output_boxes_per_class):
            max_output_boxes_per_class = g.op(
                "Constant", value_t=torch.tensor(max_output_boxes_per_class, dtype=torch.long)
            )
...
class TRTBatchedNMSop(torch.autograd.Function):
    ...
    def forward(
        ...
        if return_index:
            ret = ret + (torch.randint(0, out_boxes, (batch_size, out_boxes)).to(scores.device),)
        return ret
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdeploy.mmcv.ops.nms._multiclass_nms", backend="tensorrt")
def multiclass_nms_static(
    ...
    keep_top_k = max_output_boxes_per_class if keep_top_k < 0 else min(max_output_boxes_per_class, keep_top_k)
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdeploy.mmcv.ops.nms._multiclass_nms", backend=Backend.COREML.value)
def multiclass_nms__coreml(
    ...
    boxes, scores, box_index, _ = coreml_nms(
        boxes, scores, iou_threshold, score_threshold, min(keep_top_k, max_output_boxes_per_class)
    )
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdeploy.mmcv.ops.nms._multiclass_nms", ir=IR.TORCHSCRIPT)
def multiclass_nms__torchscript(
    ...
    scores = scores.permute(0, 2, 1)
    dets, labels = _select_nms_index(scores, boxes, keeps, batch_size, keep_top_k=keep_top_k)
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdeploy.mmcv.ops.nms._multiclass_nms", backend="ascend")
