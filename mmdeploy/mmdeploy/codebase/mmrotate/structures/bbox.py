...
@FUNCTION_REWRITER.register_rewriter("mmrotate.structures.bbox.box_converters.qbox2rbox")
def qbox2rbox__default(boxes: Tensor) -> Tensor:
    ...
    _ws = torch.unsqueeze(_dist_torch(points[..., 0, :], points[..., 1, :]), axis=-1)
    _hs = torch.unsqueeze(_dist_torch(points[..., 1, :], points[..., 2, :]), axis=-1)
    _thetas = torch.unsqueeze(
        torch.atan2(-(points[..., 1, 0] - points[..., 0, 0]), points[..., 1, 1] - points[..., 0, 1]), axis=-1
    )
