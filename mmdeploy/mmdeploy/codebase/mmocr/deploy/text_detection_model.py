...
class End2EndModel(BaseBackendModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if data_preprocessor is not None:  # skip when it is SDKEnd2EndModel
            data_preprocessor.update(model_cfg.model.get("cfg", {}).get("data_preprocessor", {}))
    ...
    def forward(
        ...
        **kwargs,
        ...
        from mmdeploy.codebase.mmdet.deploy.object_detection_model import End2EndModel as DetModel
        ...
        if len(x) == 3:  # instance seg
            ...
            for i in range(batch_dets.size(0)):
                ...
                if self.deploy_cfg is not None:
                    ...
                    export_postprocess_mask = codebase_cfg.get("export_postprocess_mask", True)
                if not export_postprocess_mask:
                    masks = DetModel.postprocessing_masks(bboxes, masks, ori_w, ori_h, batch_masks.device)
                else:
                    masks = masks[:, :img_h, :img_w]
                masks = torch.nn.functional.interpolate(masks.unsqueeze(0).float(), size=(ori_h, ori_w))
                ...
                for mask_idx, mask in enumerate(masks.cpu()):
                    ...
                    scores += [batch_dets[i, :, 4][mask_idx].cpu()] * len(contours)
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        boundaries = self.wrapper.invoke(inputs[0].permute([1, 2, 0]).contiguous().detach().cpu().numpy())
