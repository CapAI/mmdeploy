...
@MODULE_REWRITER.register_rewrite_module("mmocr.models.common.modules.PositionalEncoding", backend="default")
class PositionalEncoding(nn.Module):
    ...
    def _get_sinusoid_encoding_table(self, n_position, d_hid, device):
        ...
        denominator = torch.Tensor(
            [
                1.0 / torch.tensor(10000).to(device).pow(torch.tensor(2 * (hid_j // 2) / d_hid)).to(device)
