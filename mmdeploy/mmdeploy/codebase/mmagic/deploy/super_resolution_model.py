...
class End2EndModel(BaseBackendModel):
    ...
    def convert_to_datasample_list(
        self, predictions: DataSample, data_samples: DataSample, inputs: Optional[torch.Tensor]
    ) -> List[DataSample]:
        ...
        if inputs is not None:
            destructed_input = self.data_preprocessor.destruct(inputs, data_samples, "img")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if lq.device != torch.device(self.device):
            get_root_logger().warning(f"expect input device {self.device}" f" but get {lq.device}.")
        lq = lq.to(self.device)

        batch_outputs = self.wrapper({self.input_name: lq})[self.output_names[0]].to("cpu")
        ...
        predictions = self.convert_to_datasample_list(predictions, data_samples, inputs)
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs = []
        for input in inputs:
            output = self.wrapper.invoke(input.permute(1, 2, 0).contiguous().detach().cpu().numpy())
            outputs.append(torch.from_numpy(output).permute(2, 0, 1).contiguous())
