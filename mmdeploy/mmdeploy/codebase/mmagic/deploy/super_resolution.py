...
def process_model_config(
    ...
    if is_static_cfg:
        resize = {"type": "Resize", "scale": (input_shape[0], input_shape[1]), "keys": ["img"]}
...
def _get_dataset_metainfo(model_cfg: Config):
    ...
    for dataloader_name in ["test_dataloader", "val_dataloader", "train_dataloader"]:
        ...
        if isinstance(dataloader_cfg, list):
            ...
            dataset_list = [module_dict.get(dataset.type, None) for dataset in dataset_cfg]
            if len(dataset_list) == 0:
                continue
            meta_list = []
            for i, dataset in enumerate(dataset_list):
                if hasattr(dataset, "_load_metainfo") and isinstance(dataset._load_metainfo, Callable):
                    meta = dataset._load_metainfo(dataset_cfg[i].get("metainfo", None))
    ...
    def visualize(
        ...
        **kwargs
        ...
        if not (show_result or output_file):
            warnings.warn("show_result==False and output_file is not specified, only " "result image will be returned")
            return result
