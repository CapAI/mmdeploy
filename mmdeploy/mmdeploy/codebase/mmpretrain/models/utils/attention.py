...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmpretrain.models.utils.attention.MultiheadAttention.forward", backend=Backend.NCNN.value
)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmpretrain.models.utils.ShiftWindowMSA.forward",  # noqa: E251
...
def shift_window_msa__forward__default(self, query, hw_shape):
    ...
    query = torch.cat([query, query.new_zeros(B, C, window_size, query.shape[-1])], dim=-2)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmpretrain.models.utils.ShiftWindowMSA.get_attn_mask",  # noqa: E251
    extra_checkers=LibVersionChecker("mmpretrain", min_version="1.0.0rc7"),
)
def shift_window_msa__get_attn_mask__default(self, hw_shape, window_size, shift_size, device=None):
    ...
    if shift_size > 0:
        ...
        w_mask = torch.cat(
            [
                torch.zeros((hw_shape[1] - window_size), dtype=torch.int64, device=device),
        ...
        h_mask = torch.cat(
            [
                torch.zeros((hw_shape[0] - window_size), dtype=torch.int64, device=device),
