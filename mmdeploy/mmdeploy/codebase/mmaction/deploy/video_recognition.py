...
def process_model_config(
    ...
    if input_shape is not None:
        ...
        if has_crop and tuple(input_shape) != (crop_size, crop_size):
            logger.error(f"`input shape` should be equal to `crop_size`: {crop_size}," f" but given: {input_shape}")
        if has_resize and (not has_crop):
            if keep_ratio:
                logger.error("Resize should set `keep_ratio` to False" " when `input shape` is given.")
            if tuple(input_shape) != scale:
                logger.error(f"`input shape` should be equal to `scale`: {scale}," f" but given: {input_shape}")
    return cfg
...
class VideoRecognition(BaseTask):
    ...
    def build_backend_model(self, model_files: Sequence[str] = None, **kwargs) -> torch.nn.Module:
    ...
    def visualize(
        self, image: str, result: list, output_file: str, window_name: str = "", show_result: bool = False, **kwargs
    ):
        ...
        try:
            ...
            for i, frame in enumerate(frames):
                ...
                texts.append(f"top-1 label: {top1_item[0]}, score: {top1_item[1]}")
            ...
            logger.warn("Please install moviepy and decord to " "enable visualize for mmaction")
    ...
    def get_preprocess(self, *args, **kwargs) -> Dict:
        ...
        lift_key = ["Resize", "Normalize", "TenCrop", "ThreeCrop", "CenterCrop"]
        for key in lift_key:
            if key == "Normalize":
