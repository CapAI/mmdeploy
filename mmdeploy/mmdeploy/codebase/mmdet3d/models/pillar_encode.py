...
@FUNCTION_REWRITER.register_rewriter("mmdet3d.models.voxel_encoders.pillar_encoder.PillarFeatureNet.forward")
def pillar_encoder__forward(self, features, num_points, coors, *args, **kwargs):
    ...
    if self._with_voxel_center:
        if not self.legacy:
            f_center = features[..., :3] - (
                ...
                + torch.tensor([self.z_offset, self.y_offset, self.x_offset]).to(device)
            ).unsqueeze(1).flip(2)
        else:
            f_center = features[..., :3] - (
                coors[..., 1:] * torch.tensor([self.vz, self.vy, self.vx]).to(device)
