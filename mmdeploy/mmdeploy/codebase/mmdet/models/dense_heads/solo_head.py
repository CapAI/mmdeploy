...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.dense_heads.solo_head." "SOLOHead.predict_by_feat")
def solohead__predict_by_feat(
    ...
    **kwargs
    ...
    cfg = self.test_cfg
    mlvl_cls_scores = [
        item.permute(0, 2, 3, 1).view(item.size(0), -1, self.cls_out_channels) for item in mlvl_cls_scores
    ]
    ...
    batch_mlvl_cls_scores = batch_mlvl_cls_scores.where(score_mask, batch_mlvl_cls_scores.new_zeros(1)).view(-1)
