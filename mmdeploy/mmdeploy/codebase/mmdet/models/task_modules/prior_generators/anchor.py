...
class GridPriorsTRTOp(torch.autograd.Function):
    ...
    def symbolic(g, base_anchors, feat_h, feat_w, stride_h: int, stride_w: int):
        ...
        return g.op("mmdeploy::GridPriorsTRT", base_anchors, zero_h, zero_w, stride_h_i=stride_h, stride_w_i=stride_w)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.task_modules.prior_generators." "AnchorGenerator.single_level_grid_priors",
    backend="tensorrt",
)
def anchorgenerator__single_level_grid_priors__trt(
    self, featmap_size: Tuple[int], level_idx: int, dtype: torch.dtype = torch.float32, device: str = "cuda"
) -> torch.Tensor:
    ...
    if type(self) != AnchorGenerator:
        ...
        return ctx.origin_func(self, featmap_size=featmap_size, level_idx=level_idx, dtype=dtype, device=device)
