...
def process_model_config(
    model_cfg: Config, imgs: Union[Sequence[str], Sequence[np.ndarray]], input_shape: Optional[Sequence[int]] = None
):
    ...
    pipeline = [transform for transform in pipeline if transform.type != "LoadAnnotations"]
...
class ObjectDetection(BaseTask):
    ...
    def build_backend_model(
        self, model_files: Optional[str] = None, data_preprocessor_updater: Optional[Callable] = None, **kwargs
    ) -> torch.nn.Module:
        ...
        model = build_object_detection_model(
            model_files, self.model_cfg, self.deploy_cfg, device=self.device, data_preprocessor=data_preprocessor
        )
        ...
        if not dynamic_flag:
            ...
            if "transforms" in transform:
                ...
                for i, step in enumerate(transform_list):
                    if step["type"] == "Pad" and "pad_to_square" in step and step["pad_to_square"]:
    ...
    def get_partition_cfg(partition_type: str) -> Dict:
        ...
        assert partition_type in MMDET_PARTITION_CFG, f"Unknown partition_type {partition_type}"
    ...
    def get_preprocess(self, *args, **kwargs) -> Dict:
        ...
        if self.codebase.value == "mmyolo":
            transforms = [item for item in pipeline if item["type"] not in ("ToGray", "YOLOv5KeepRatioResize")]
    ...
    def get_postprocess(self, *args, **kwargs) -> Dict:
        ...
        if get_backend(self.deploy_cfg) == Backend.RKNN:
            if "YOLO" in self.model_cfg.model.type or "RTMDet" in self.model_cfg.model.type:
