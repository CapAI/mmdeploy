...
def clip_bboxes(x1: Tensor, y1: Tensor, x2: Tensor, y2: Tensor, max_shape: Union[Tensor, Sequence[int]]):
...
def clip_bboxes__trt8(x1: Tensor, y1: Tensor, x2: Tensor, y2: Tensor, max_shape: Union[Tensor, Sequence[int]]):
...
def pad_with_value(x: Tensor, pad_dim: int, pad_size: int, pad_value: Optional[Any] = None):
...
def pad_with_value_if_necessary(x: Tensor, pad_dim: int, pad_size: int, pad_value: Optional[Any] = None) -> Tensor:
    ...
    Returns:
        Tensor: Padded tensor.
    """
    return __pad_with_value_if_necessary(x, pad_dim, pad_size=pad_size, pad_value=pad_value)


def __pad_with_value_if_necessary(x: Tensor, pad_dim: int, pad_size: int, pad_value: Optional[Any] = None) -> Tensor:
...
@FUNCTION_REWRITER.register_rewriter(
    "mmdeploy.codebase.mmdet.deploy.utils.__pad_with_value_if_necessary", backend=Backend.TENSORRT.value
)
...
def __gather_topk(
    *inputs: Sequence[torch.Tensor], inds: torch.Tensor, batch_size: int, is_batched: bool = True
) -> Tuple[torch.Tensor]:
...
@FUNCTION_REWRITER.register_rewriter("mmdeploy.codebase.mmdet.deploy.utils.__gather_topk", backend=Backend.ASCEND.value)
@FUNCTION_REWRITER.register_rewriter("mmdeploy.codebase.mmdet.deploy.utils.__gather_topk", backend=Backend.COREML.value)
