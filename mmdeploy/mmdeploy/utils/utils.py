...
def target_wrapper(target: Callable, log_level: int, ret_value: Optional[mp.Value] = None, *args, **kwargs):
...
def deprecate(
    status: str = "future", dst_obj: Optional[Union[object, str]] = None, msg: str = "", *args, **kwargs
) -> None:
    ...
    def _register(src_obj):
        def fun(*args, **kwargs):
            if status == "future":
                logger.warning(f"DeprecationWarning: {src_obj.__name__} will be " f"deprecated in the future. {msg}")
            elif status == "past":
                assert dst_obj is not None, "for deprecated object, there"
                " must be a destination object"
                logger.warning(
                    f"DeprecationWarning: {src_obj.__name__} was deprecated," f" use {dst_obj.__name__} instead. {msg}"
                )
            else:
