...
class TimeCounter:
    ...
    @classmethod
    def count_time(cls, name: str, warmup: int = 1, log_interval: int = 1, with_sync: bool = False):
        ...
        def _register(func):
            assert warmup >= 1
            assert name not in cls.names, "The registered function name cannot be repeated!"
            ...
            def fun(*args, **kwargs):
                ...
                if enable and count > warmup:
                    ...
                    if (count - warmup) % log_interval == 0:
                        ...
                        msg = f"[{name}]-{count} times per count: " f"{times_per_count:.2f} ms, " f"{fps:.2f} FPS"
                        cls.logger.info(msg)
    ...
    def activate(
        ...
        **kwargs,
        ...
        if func_name is not None:
            warnings.warn("func_name must be globally unique if you call " "activate multiple times")
            assert func_name in cls.names, "{} must be registered before " "setting params".format(func_name)
