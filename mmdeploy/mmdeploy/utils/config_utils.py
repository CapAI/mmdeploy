...
def get_codebase(deploy_cfg: Union[str, mmengine.Config], model_cfg=None) -> Codebase:
    ...
    if model_cfg is not None:
        ...
        if model_cfg.get("_scope_", None) == "mmrazor" or model_cfg["type"].startswith("mmrazor."):
...
def get_backend(deploy_cfg: Union[str, mmengine.Config]) -> Backend:
    ...
    assert "type" in backend_config, "The backend config of deploy config" 'requires a "type" field'
...
def get_onnx_config(deploy_cfg: Union[str, mmengine.Config]) -> Dict:
    ...
    assert ir_type is None or ir_type == "onnx", "Expect IR type is ONNX," f"but get {ir_type}"
    return onnx_config


def is_dynamic_batch(deploy_cfg: Union[str, mmengine.Config], input_name: Optional[str] = None) -> bool:
...
def is_dynamic_shape(deploy_cfg: Union[str, mmengine.Config], input_name: Optional[str] = None) -> bool:
