...
@FUNCTION_REWRITER.register_rewriter(func_name="torch.nn.functional.normalize", backend="ncnn")
def normalize__ncnn(input: torch.Tensor, p: int = 2, dim: int = 1, eps: float = 1e-12, *args, **kwargs):
    ...
    if input.ndim == 3:
        output = origin_func(input.transpose(1, dim).unsqueeze(2), p=p, dim=1, eps=eps).squeeze(2).transpose(1, dim)
    else:
        # input.ndim == 4:
        if dim == 1:
            output = origin_func(input, p=p, dim=dim, eps=eps)
        else:
            output = origin_func(input.transpose(1, dim), p=p, dim=1, eps=eps).transpose(1, dim)
    return output
