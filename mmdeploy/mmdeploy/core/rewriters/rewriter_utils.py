...
class LibVersionChecker(Checker):
    ...
    def __init__(self, lib: str, min_version: Optional[str] = None, max_version: Optional[str] = None):
        super().__init__()
        self.lib = lib
...
class RewriterRegistry:
            ...
            for record in rewriter_records:
                ...
                if len(checkers) == 0:
                    ...
                    if default_rewriter is None:
                        ...
                        warnings.warn(
                            "Detect multiple valid rewriters for " f"{origin_function}, use the first rewriter."
                        )
                else:
    ...
    def _register(self, name: str, backend: Backend, ir: IR, extra_checkers: List[Checker], **kwargs):
    ...
    def register_object(
        self, name: str, backend: str, ir: IR, extra_checkers: Optional[Union[Checker, List[Checker]]] = None, **kwargs
    ) -> Callable:
