...
def ensure_base_env(work_dir, dep_dir):
    ...
    * mmcv (not compulsory)
    ...
    if make is None or len(make) < 1:
        ...
        os.system('{} DEBIAN_FRONTEND="noninteractive"  apt install make'.format(sudo))
        make = cmd_result("which make")
        if make is None or len(make) < 1:
    ...
    if gplus is None or len(gplus) < 1:
        ...
        os.system(
            '{} DEBIAN_FRONTEND="noninteractive" apt install software-properties-common -y'.format(sudo)  # noqa: E501
        )  # noqa: E501
        os.system("{} apt update".format(sudo))
        if ubuntu is None or len(ubuntu) < 1 or version_major(ubuntu) <= 18:
            os.system("{} add-apt-repository ppa:ubuntu-toolchain-r/test -y".format(sudo))
    ...
    if ocv is None or len(ocv) < 1:
        ...
        if add_ppa:
            os.system("{} add-apt-repository ppa:ignaciovizzo/opencv3-nonfree -y".format(sudo))

        os.system('{} DEBIAN_FRONTEND="noninteractive"  apt install libopencv-dev -y'.format(sudo))
    ...
    print("python version\t:{}".format(cmd_result("python3 --version | awk '{print $2}'")))

    print("cmake bin\t:{}".format(cmake))
    print("cmake version\t:{}".format(cmd_result("cmake --version | head -n 1 | awk '{print $3}'")))

    print("make bin\t:{}".format(make))
    print("make version\t:{}".format(cmd_result(" make --version  | head -n 1 | awk '{print $3}' ")))
    ...
    print("git version\t:{}".format(cmd_result("git --version | awk '{print $3}' ")))
