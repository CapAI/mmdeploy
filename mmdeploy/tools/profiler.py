...
from mmdeploy.utils.config_utils import Backend, get_backend, get_input_shape, load_config
from mmdeploy.utils.timer import TimeCounter


def parse_args():
    parser = argparse.ArgumentParser(description="MMDeploy Model Latency Test Tool.")
    ...
    parser.add_argument("--device", help="device type for inference", default="cuda:0")
    parser.add_argument("--shape", type=str, help="Input shape to test in `HxW` format, e.g., `800x1344`", default=None)
    parser.add_argument("--warmup", type=int, help="warmup iterations before counting inference latency.", default=10)
    parser.add_argument("--num-iter", type=int, help="Number of iterations to run the inference.", default=100)
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
    ...
    parser.add_argument("--batch-size", type=int, default=1, help="the batch size for test.")
    parser.add_argument(
        "--img-ext",
        type=str,
        nargs="+",
...
def main():
    ...
    if nrof_image < total_nrof_image:
        np.random.seed(1234)
        image_files += [image_files[i] for i in np.random.choice(nrof_image, total_nrof_image - nrof_image)]
    image_files = image_files[:total_nrof_image]
    with TimeCounter.activate(warmup=args.warmup, log_interval=20, with_sync=with_sync, batch_size=args.batch_size):
        for i in range(0, total_nrof_image, args.batch_size):
            batch_files = image_files[i : (i + args.batch_size)]
            data, _ = task_processor.create_input(
                batch_files, input_shape, data_preprocessor=getattr(model, "data_preprocessor", None)
            )
