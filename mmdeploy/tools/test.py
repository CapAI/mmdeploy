...
def parse_args():
    ...
    parser.add_argument(
        "--work-dir", default="./work_dir", help="the directory to save the file containing evaluation metrics"
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
    ...
    parser.add_argument("--speed-test", action="store_true", help="activate speed test")
    parser.add_argument(
        "--warmup",
        type=int,
        help="warmup before counting inference elapse, require setting " "speed-test first",
        default=10,
    )
    parser.add_argument(
        "--log-interval",
        type=int,
        help="the interval between each log, require setting " "speed-test first",
        default=100,
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=1,
        help="the batch size for test, would override `samples_per_gpu`" "in  data config.",
    )
    parser.add_argument(
        "--uri",
        action="store_true",
