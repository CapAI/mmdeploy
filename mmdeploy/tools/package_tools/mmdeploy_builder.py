...
def _call_command(cmd, cwd, stdout=None, stderr=None):
    ...
    try:
        ...
        if ret.returncode != 0:
            logging.error(f'Process cmd: "{cmd}"' f" failed with returncode: {ret.returncode}")
            exit(-1)
    except Exception:
...
def _create_bdist_cmd(cfg, c_ext=False, dist_dir=None):
    ...
    # python tag
    python_tag = f"cp{sys.version_info.major}{sys.version_info.minor}" if c_ext else "py3"
...
def clear_mmdeploy():
    ...
    ncnn_ext_paths = glob(osp.join(MMDEPLOY_DIR, "mmdeploy/backend/ncnn/ncnn_ext.*"))
    for ncnn_ext_path in ncnn_ext_paths:
        os.remove(ncnn_ext_path)

    # remove ts_optmizer
    ts_optimizer_paths = glob(osp.join(MMDEPLOY_DIR, "mmdeploy/backend/torchscript/ts_optimizer.*"))
...
def check_env(cfg: Dict):
    ...
    CUDA_TOOLKIT_ROOT_DIR = cmake_envs.get("CUDA_TOOLKIT_ROOT_DIR", CUDA_TOOLKIT_ROOT_DIR)
    CUDA_TOOLKIT_ROOT_DIR = osp.expandvars(CUDA_TOOLKIT_ROOT_DIR)
    nvcc_cmd = "nvcc" if len(CUDA_TOOLKIT_ROOT_DIR) <= 0 else osp.join(CUDA_TOOLKIT_ROOT_DIR, "bin", "nvcc")
    ...
    if osp.exists(TENSORRT_DIR):
        with open(osp.join(TENSORRT_DIR, "include", "NvInferVersion.h"), mode="r") as f:
            data = f.read()
            ...
            if major is not None and minor is not None and patch is not None and build is not None:
                tensorrt_version = (
                    f"{major.group(1)}." + f"{minor.group(1)}." + f"{patch.group(1)}." + f"{build.group(1)}"
                )
...
def create_mmdeploy_runtime(cfg: Dict, work_dir: str):
    cmake_cfg = cfg["cmake_cfg"]
    if cmake_cfg["MMDEPLOY_BUILD_SDK"] == "OFF" or cmake_cfg["MMDEPLOY_BUILD_SDK_PYTHON_API"] == "OFF":
    ...
    for python_version in ["3.6", "3.7", "3.8", "3.9", "3.10"]:
        ...
        python_executable = shutil.which("python").replace("mmdeploy-3.6", f"mmdeploy-{python_version}")
        cmake_options = [f'-D{k}="{v}"' for k, v in cmake_cfg.items() if v != ""]
        cmake_options.append(f"-DMMDeploy_DIR={MMDEPLOY_DIR}/build/install/lib/cmake/MMDeploy")
        ...
        cmake_cmd = " ".join(["cmake ../csrc/mmdeploy/apis/python"] + cmake_options)
        ...
        if sys.platform == "win32":
            ...
            libs_to_copy = ["*net.so", f"*mmdeploy.so.{mmdeploy_version.major}"]
        ...
        for pattern in libs_to_copy:
            ...
            for file in files:
                _copy(file, osp.join(sdk_python_package_dir, "mmdeploy_runtime"))

        # copy onnxruntime
        if "ort" in cfg["cmake_cfg"]["MMDEPLOY_TARGET_BACKENDS"]:
            copy_onnxruntime(cfg, osp.join(sdk_python_package_dir, "mmdeploy_runtime"))
...
def create_sdk(cfg: Dict, work_dir: str):
    ...
    mmdeploy_version = get_version(VERSION_FILE)
    build_sdk_name = build_sdk_name.format(mmdeploy_v=mmdeploy_version, **env_info)
...
def parse_args():
    ...
    parser.add_argument("--output-dir", default=".", help="Output package directory.")
