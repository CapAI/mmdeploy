...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=["mmpretrain", "mmdet", "mmseg", "mmpose", "mmocr", "mmagic", "mmrotate", "mmdet3d"],
    )
    parser.add_argument("-p", "--performance", default=False, action="store_true", help="test performance if it set")
    parser.add_argument("--backends", nargs="+", help="test specific backend(s)")
    parser.add_argument("--models", nargs="+", default=["all"], help="test specific model(s)")
    parser.add_argument(
        "--work-dir", type=str, help="the dir to save logs and models", default="../mmdeploy_regression_working_dir"
    )
    parser.add_argument(
        "--checkpoint-dir", type=str, help="the dir to save checkpoint for all model", default="../mmdeploy_checkpoints"
    )
    parser.add_argument("--device", type=str, help="Device type, cuda:id or cpu, cuda:0 as default", default="cuda:0")
...
def merge_report(work_dir: str, logger: logging.Logger):
    ...
    res_file = work_dir.joinpath(f"mmdeploy_regression_test_{mmdeploy.version.__version__}.xlsx")
    ...
    for report_file in work_dir.iterdir():
        if report_file.name.startswith(".~"):
            ...
            continue
        if "_report.xlsx" not in report_file.name or report_file.name == res_file.name or not report_file.is_file():
            # skip other file
            continue
...
def get_model_metafile_info(global_info: dict, model_info: dict, logger: logging.Logger):
    ...
    checkpoint_save_dir = Path(checkpoint_dir).joinpath(codebase_name, model_name)
    ...
    for meta_model in metafile_info.get("Models"):
        if str(meta_model.get("Config")) not in model_config_files:
            ...
            logger.warning(f'{meta_model.get("Config")} ' f"not in {model_config_files}, pls check ! " "Skip it...")
            continue
        ...
        if weights_save_path.exists() and not global_info.get("checkpoint_force_download", False):
...
def update_report(
    ...
    # save to tmp file
    tmp_str = (
        f"{model_name},{model_config},{task_name},{checkpoint},"
...
def get_pytorch_result(
    model_name: str,
    meta_info: dict,
    checkpoint_path: Path,
    model_config_path: Path,
    model_config_name: str,
    test_yaml_metric_info: dict,
    report_dict: dict,
    ...
    valid_pytorch_metric = {k: v for k, v in pytorch_metric.items() if k in test_yaml_metric_info}
...
def get_fps_metric(shell_res: int, pytorch_metric: dict, metric_info: dict, work_path: Path):
    """Get fps and metric.
...
def get_backend_fps_metric(
    ...
    fps, backend_metric, test_pass = get_fps_metric(return_code, pytorch_metric, metric_info, work_dir)
...
def replace_top_in_pipeline_json(backend_output_path: Path, logger: logging.Logger):
    ...
    for index, task in enumerate(pipeline_tasks):
        ...
        if "topk" not in task.get("params", {}):
            continue
        sdk_pipeline_json["pipeline"]["tasks"][index]["params"]["topk"] = num_classes

    logger.info(f"sdk_pipeline_json = {sdk_pipeline_json}")

    mmengine.dump(sdk_pipeline_json, sdk_pipeline_json_path, sort_keys=False, indent=4)
...
def get_backend_result(
    ...
    if convert_image_info is not None:
        input_img_path = convert_image_info.get("input_img", "./tests/data/tiger.jpeg")
    ...
    if metric_tolerance is not None:
        for metric, new_tolerance in metric_tolerance.items():
            if metric not in metric_info:
                logger.debug(f"{metric} not in {metric_info}," "skip compare it...")
                continue
    ...
    if backend_name in ["ncnn", "openvino"]:
        ...
        if ort.get_device() != "GPU":
            device_type = "cpu"
            logger.warning("Device type is forced to cpu " "since onnxruntime-gpu is not installed")
    ...
    backend_output_path = Path(work_dir).joinpath(
        Path(checkpoint_path).parent.parent.name,
    ...
    if convert_result and test_type == "precision":
        ...
        if backend_test:
            log_path = gen_log_path(backend_output_path.joinpath("backend"), "test_log.txt")
        ...
        if sdk_config is not None:
            ...
            log_path = gen_log_path(backend_output_path.joinpath("sdk"), "test_log.txt")
...
def save_report(report_info: dict, report_save_path: Path, logger: logging.Logger):
    """Convert model to onnx and then get metric.
    ...
    Args:
        ...
        logger (logging.Logger): Logger.
    """
    logger.info("Saving regression test report to " f"{report_save_path}, pls wait...")
    try:
    ...
    logger.info("Saved regression test report to " f"{report_save_path}.")
...
def main():
    ...
    if backend_list is None:
        backend_list = ["onnxruntime", "tensorrt", "openvino", "ncnn", "pplnn", "torchscript"]
    ...
    assert " " not in args.work_dir, f"No empty space included in {args.work_dir}"
    assert " " not in args.checkpoint_dir, f"No empty space included in {args.checkpoint_dir}"

    work_dir = Path(args.work_dir)
    work_dir.mkdir(parents=True, exist_ok=True)

    deploy_yaml_list = [f"./tests/regression/{codebase}.yml" for codebase in args.codebase]

    for deploy_yaml in deploy_yaml_list:
        if not Path(deploy_yaml).exists():
            raise FileNotFoundError(f"deploy_yaml {deploy_yaml} not found, " "please check !")
        ...
        global_info.update({"codebase_name": Path(deploy_yaml).stem.split("_")[0]})
        ...
        for models in tqdm(models_info):
            ...
            if "model_configs" not in models:
                logger.warning('Can not find field "model_configs", ' f"skipping {model_name_origin}...")
                continue

            if args.models != ["all"] and model_name_new not in args.models:
                logger.info(f"Test specific model mode, skip {model_name_origin}...")
                continue
            try:
                model_metafile_info, checkpoint_save_dir, codebase_dir = get_model_metafile_info(
                    global_info, models, logger
                )
            ...
            for model_config in model_metafile_info:
                ...
                for pipeline in pipelines_info:
                    ...
                    if backend_name not in backend_list:
                        logger.warning(f"backend_name ({backend_name}) not " f"in {backend_list}, skip it...")
                        continue

                    backend_file_name = backend_file_info.get(backend_name, None)
                    if backend_file_name is None:
                        logger.warning("backend_file_name is None, " "skip it...")
                        continue
