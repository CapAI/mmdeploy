...
def get_deploy_cfg(input_name, output_name):
    return Config(
        dict(
            ir_config=dict(
                type=IR.TORCHSCRIPT.value, input_names=[input_name], output_names=[output_name], input_shape=None
            ),
...
def test_torch2torchscript(input_name, output_name):
    ...
    torch2torchscript(
        np.random.randint(0, 255, (8, 8, 3)), "", ts_file, deploy_cfg, model_cfg=get_model_cfg(), device="cpu"
    )
