...
def get_partition_deploy_cfg():
    ...
    deploy_cfg._cfg_dict["partition_config"] = dict(type="two_stage", apply_marks=True)
    return deploy_cfg
    ...
    model_cfg = Config(
        dict(
            data=dict(
                samples_per_gpu=1,
                workers_per_gpu=1,
                val=dict(type=dataset_type, ann_file=ann_file, img_prefix=data_root, pipeline=test_pipeline),
            ),
            # model settings
            model=dict(
                type="FasterRCNN",
...
def run_test_create_calib_end2end():
    ...
    create_calib_input_data(
        calib_file, deploy_cfg, model_cfg, None, dataset_cfg=model_cfg, dataset_type="val", device="cpu"
    )
