...
def generate_onnx_file(model):
    with torch.no_grad():
        dynamic_axes = {"input": {0: "batch", 2: "width", 3: "height"}, "output": {0: "batch"}}
        torch.onnx.export(
            model,
            test_img,
            onnx_file,
...
def test_onnx2ascend():
    ...
    model_inputs = mmengine.Config(dict(dynamic_batch_size=[1, 2, 4], input_shapes=dict(input=[-1, 3, 224, 224])))
