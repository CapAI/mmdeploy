...
def get_trt_config(output_names, shape, dynamic_axes=None):
    deploy_cfg_tensorrt = Config(
        dict(
            onnx_config=dict(input_shape=None, output_names=output_names, dynamic_axes=dynamic_axes),
            backend_config=dict(
                type="tensorrt",
                common_config=dict(fp16_mode=False, max_workspace_size=1 << 20),
                model_inputs=[dict(input_shapes=dict(input=dict(min_shape=shape, opt_shape=shape, max_shape=shape)))],
            ),
            codebase_config=dict(type="mmdet", task="ObjectDetection"),
        )
    )
...
def test_get_attribute():
    ...
    rewrite_outputs, _ = get_rewrite_outputs(
        wrapped_func, model_inputs={"tensor": input}, deploy_cfg=deploy_cfg_ncnn, run_with_backend=True
    )
...
def test_group_norm_ncnn():
    ...
    rewrite_output, _ = get_rewrite_outputs(
        wrapped_func, model_inputs={"input": input}, deploy_cfg=deploy_cfg_ncnn, run_with_backend=True
    )
...
def test_interpolate__rknn():
    ...
    wrapped_func = WrapFunction(interpolate_caller, size=[4, 4])
    rewrite_output, _ = get_rewrite_outputs(
        wrapped_func, model_inputs={"input": input}, deploy_cfg=deploy_cfg, run_with_backend=False
    )
...
def test_size__ascend():
    ...
    deploy_cfg_ascend = Config(
        dict(
            onnx_config=dict(input_shape=None),
            backend_config=dict(type="ascend", model_inputs=[dict(input_shapes=dict(input=input.shape))]),
            codebase_config=dict(type="mmdet", task="ObjectDetection"),
        )
    )
    wrapped_func = WrapFunction(model_func)
    rewrite_outputs, _ = get_rewrite_outputs(
        wrapped_func, model_inputs={"input": input}, deploy_cfg=deploy_cfg_ascend, run_with_backend=True
    )
...
class TestTopk:
    ...
    def test_topk_ncnn(self, dim, k):
        ...
        output, _ = get_rewrite_outputs(
            wrapped_func, model_inputs={"input": TestTopk.input}, deploy_cfg=deploy_cfg_ncnn, run_with_backend=True
        )
    ...
    def test_topk_tensorrt(self, dim, k):
        ...
        deploy_cfg_tensorrt = get_trt_config(["indices", "values"], [1, 5, 5, 5])
        output, is_backend_output = get_rewrite_outputs(
            wrapped_func, model_inputs={"input": TestTopk.input}, deploy_cfg=deploy_cfg_tensorrt
        )
...
def test_triu_trt(shape, diagonal):
    ...
    if is_backend_output:
        ...
        assert np.allclose(model_output, rewrite_outputs, rtol=1e-03, atol=1e-05)
    else:
        assert rewrite_outputs is not None


@backend_checker(Backend.NCNN)
@pytest.mark.parametrize("input", [torch.rand(1, 16, 16), torch.rand(1, 3, 16, 16)])
...
def test_getitem__ascend():
    ...
    # convert model
    with RewriterContext(cfg={}, backend=Backend.ASCEND.value, opset=11), torch.no_grad():
...
def test_masked_fill_onnxruntime(input):
    ...
    rewrite_output, _ = get_rewrite_outputs(
        wrapped_func, model_inputs={"input": input}, deploy_cfg=deploy_cfg_ort, run_with_backend=True
    )
    assert rewrite_output is not None


@backend_checker(Backend.ONNXRUNTIME)
@pytest.mark.skipif(parse(torch.__version__) < parse("1.9.0"), reason="requires torch>1.8.0")
...
def test_adaptive_avg_pool2d(output_size):
    ...
    rewrite_output, _ = get_rewrite_outputs(
        model, model_inputs={"input": input}, deploy_cfg=deploy_cfg_ort, run_with_backend=True
    )
...
def test_scaled_dot_product_attention():
    ...
    deploy_cfg_ort = Config(
        dict(
            onnx_config=dict(
                input_shape=None, input_names=["q", "k", "v", "attn_mask"], output_names=["output", "attn"]
            ),
            backend_config=dict(
                type="tensorrt",
                model_inputs=[
                    dict(
                        input_shapes=dict(
                            q=dict(min_shape=q.shape, opt_shape=q.shape, max_shape=q.shape),
                            k=dict(min_shape=k.shape, opt_shape=k.shape, max_shape=k.shape),
                            v=dict(min_shape=v.shape, opt_shape=v.shape, max_shape=v.shape),
                            attn_mask=dict(
                                min_shape=attn_mask.shape, opt_shape=attn_mask.shape, max_shape=attn_mask.shape
                            ),
                        )
                    )
                ],
    ...
    rewrite_output, _ = get_rewrite_outputs(
        model,
        model_inputs={"q": q, "k": k, "v": v, "attn_mask": attn_mask},
        deploy_cfg=deploy_cfg_ort,
        run_with_backend=True,
    )
    assert torch.allclose(pytorch_output[0], rewrite_output[0].to(pytorch_output[0].device))
...
def test_mod__tensorrt(num):
    ...
    assert torch.allclose(pytorch_output, rewrite_output[0], rtol=1e-3, atol=1e-5)
...
def test_any__default(dim, keepdim):
    ...
    assert torch.allclose(pytorch_output.float(), rewrite_output[0].float(), rtol=1e-3, atol=1e-5)
...
def test_linspace__default():
    ...
    deploy_cfg_ort = Config(dict(onnx_config=dict(input_shape=None), backend_config=dict(type="onnxruntime")))
    ...
    for steps in steps_list:
        ...
        wrapped_func = WrapFunction(linspace_caller, start=start, end=end, steps=steps)

        rewrite_outputs, is_backend_output = get_rewrite_outputs(
            wrapped_func, model_inputs={}, deploy_cfg=deploy_cfg_ort, run_with_backend=True
        )
...
@pytest.mark.parametrize("dynamic_axes", [None, dict(input=dict({0: "dim0", 1: "dim1"}))])
def test_cat__tensorrt(dtype, dynamic_axes):
    input = torch.rand(2, 4)
    model = WrapFunction(lambda input: torch.cat([input.to(dtype), input.to(dtype)], -1))
    pytorch_output = model(input)
    rewrite_output, _ = get_rewrite_outputs(
        model,
        model_inputs={"input": input},
        deploy_cfg=get_trt_config(["output"], shape=[2, 4], dynamic_axes=dynamic_axes),
        run_with_backend=True,
    )
    assert pytorch_output.dtype == rewrite_output[0].dtype
    assert torch.allclose(pytorch_output.cpu().float(), rewrite_output[0].cpu().float(), rtol=1e-3, atol=1e-5)
