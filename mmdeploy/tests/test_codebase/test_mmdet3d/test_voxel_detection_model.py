...
class TestVoxelDetectionModel:
    ...
    def setup_class(cls):
        ...
        cls.end2end_model = VoxelDetectionModel(
            Backend.ONNXRUNTIME, [""], device="cuda", deploy_cfg=deploy_cfg, model_cfg=model_cfg
        )
...
def test_build_voxel_detection_model():
    ...
    deploy_cfg = mmengine.Config(
        dict(
            ...
            onnx_config=dict(output_names=["cls_score", "bbox_pred", "dir_cls_pred"]),
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        voxeldetector = build_voxel_detection_model([""], model_cfg, deploy_cfg, "cpu")
