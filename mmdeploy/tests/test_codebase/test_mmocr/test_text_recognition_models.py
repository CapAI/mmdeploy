...
class TestEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        deploy_cfg = mmengine.Config({"onnx_config": {"output_names": ["output"]}})
        ...
        cls.end2end_model = End2EndModel(
            Backend.ONNXRUNTIME, [""], device="cpu", deploy_cfg=deploy_cfg, model_cfg=model_cfg
        )
    ...
    @pytest.mark.parametrize("ori_shape", [[IMAGE_SIZE, IMAGE_SIZE, 3], [2 * IMAGE_SIZE, 2 * IMAGE_SIZE, 3]])
    def test_forward(self, ori_shape):
        imgs = [torch.rand(1, 3, IMAGE_SIZE, IMAGE_SIZE)]
        img_meta = {"ori_shape": ori_shape, "img_shape": [IMAGE_SIZE, IMAGE_SIZE, 3], "scale_factor": [1.0, 1.0]}
...
def test_build_text_recognition_model():
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        from mmdeploy.codebase.mmocr.deploy.text_recognition_model import End2EndModel, build_text_recognition_model

        segmentor = build_text_recognition_model([""], model_cfg, deploy_cfg, "cpu")
