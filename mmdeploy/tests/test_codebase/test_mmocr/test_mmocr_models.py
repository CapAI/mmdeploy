...
class FPNCNeckModel(FPNC):
    ...
    def forward(self, inputs):
        neck_inputs = [inputs.repeat([1, channel, 1, 1]) for channel in self.in_channels]
...
def get_base_recognizer_model():
    ...
    cfg = dict(
        ...
        data_preprocessor=dict(type="mmocr.TextRecogDataPreprocessor", mean=[127], std=[127]),
    )
    model = CRNN(
        backbone=cfg["backbone"], encoder=None, decoder=cfg["decoder"], data_preprocessor=cfg["data_preprocessor"]
    )
    model.requires_grad_(False)
    return model
...
def test_bidirectionallstm(backend: Backend):
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg, run_with_backend=True
    )
...
def test_nrtr_decoder__get_source_mask(backend: Backend):
    ...
    backend_outputs = get_backend_outputs(ir_file_path, batch_model_inputs, deploy_cfg)[0].numpy()
    num_elements = np.prod(backend_outputs.shape[1:])
    # batch results should be same
    assert (
        np.sum(backend_outputs[0] == backend_outputs[1]) == num_elements
...
def test_crnndecoder(backend: Backend, rnn_flag: bool):
    ...
    # to get outputs of pytorch model
    model_inputs = {"feat": input, "out_enc": out_enc, "data_samples": data_samples}
    model_outputs = get_model_outputs(crnn_decoder, "forward_train", model_inputs)

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(crnn_decoder, "forward_train", out_enc=out_enc, data_samples=data_samples)
    ...
    if is_backend_output:
        for model_output, rewrite_output in zip(model_outputs, rewrite_outputs):
            ...
            assert np.allclose(model_output, rewrite_output, rtol=1e-03, atol=1e-04)
    else:
        assert rewrite_outputs is not None


@pytest.mark.parametrize("backend", [Backend.ONNXRUNTIME])
@pytest.mark.parametrize("data_samples", [[[{}]], [[{"resize_shape": [32, 32], "valid_ratio": 1.0}]]])
@pytest.mark.parametrize("is_dynamic", [True, False])
def test_forward_of_encoder_decoder_recognizer(data_samples, is_dynamic, backend):
    ...
    wrapped_model = WrapModel(base_recognizer, "forward", data_samples=data_samples[0])
    rewrite_inputs = {
        "batch_inputs": input,
    }
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
...
def test_forward_of_fpnc(backend: Backend):
        ...
        dict(
            backend_config=dict(
                ...
                model_inputs=[
                    dict(
                        input_shapes=dict(
                            inputs=dict(min_shape=[1, 1, 64, 64], opt_shape=[1, 1, 64, 64], max_shape=[1, 1, 64, 64])
                        )
                    )
                ],
            ),
            onnx_config=dict(input_shape=None, input_names=["inputs"], output_names=["output"]),
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, run_with_backend=False, deploy_cfg=deploy_cfg
    )
...
def get_sar_model_cfg(decoder_type: str):
    model = dict(
        type="SARNet",
        data_preprocessor=dict(type="mmocr.TextRecogDataPreprocessor", mean=[127, 127, 127], std=[127, 127, 127]),
        backbone=dict(type="ResNet31OCR"),
        encoder=dict(type="mmocr.SAREncoder", enc_bi_rnn=False, enc_do_rnn=0.1, enc_gru=False),
        decoder=dict(
...
@pytest.mark.parametrize("backend", [Backend.ONNXRUNTIME])
@pytest.mark.parametrize("decoder_type", ["SequentialSARDecoder", "ParallelSARDecoder"])
def test_sar_model(backend: Backend, decoder_type):
    ...
    patched_model = patch_model(pytorch_model, cfg=deploy_cfg, backend=backend.value)
    ...
    with RewriterContext(cfg=deploy_cfg, backend=backend.value), torch.no_grad():
        torch.onnx.export(
            patched_model,
...
def test_mmdet_wrapper__forward(backend):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            backend_config=dict(type=backend.value, common_config=dict(max_workspace_size=1 << 20)),
            onnx_config=dict(input_shape=None, input_names=["inputs"], output_names=["output"]),
            codebase_config=dict(
                type="mmocr",
                task="TextDetection",
    ...
    input = torch.rand(1, 3, 64, 64)
    img_meta = {"ori_shape": [64, 64], "img_shape": [64, 64], "scale_factor": [1.0, 1.0], "img_path": ""}
