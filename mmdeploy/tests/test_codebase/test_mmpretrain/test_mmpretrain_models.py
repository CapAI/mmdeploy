...
def test_baseclassifier_forward():
    ...
    backbone_cfg = dict(type="ResNet", depth=18, num_stages=4, out_indices=(3,), style="pytorch")
...
@pytest.mark.parametrize("backend_type", [Backend.ONNXRUNTIME, Backend.TENSORRT, Backend.NCNN, Backend.OPENVINO])
                ...
                backend_config=dict(
                    ...
                    model_inputs=[
                        dict(
                            input_shapes=dict(
                                input=dict(
                                    min_shape=[1, 16, 28, 28], opt_shape=[1, 16, 28, 28], max_shape=[1, 16, 28, 28]
                                )
                            )
                        )
                    ],
                ),
                onnx_config=dict(input_shape=[28, 28], output_names=["output"]),
...
@pytest.mark.parametrize("backend_type", [Backend.ONNXRUNTIME, Backend.TENSORRT, Backend.NCNN, Backend.OPENVINO])
@pytest.mark.parametrize("inputs", [torch.rand(1, 3, 5, 5), (torch.rand(1, 3, 7, 7))])
            ...
            dict(
                backend_config=dict(
                    ...
                    model_inputs=[
                        dict(
                            input_shapes=dict(
                                input=dict(min_shape=input_shape, opt_shape=input_shape, max_shape=input_shape)
                            )
                        )
                    ],
                ),
