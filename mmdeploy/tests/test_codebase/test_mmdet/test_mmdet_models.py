...
@pytest.mark.parametrize("clip_border,max_shape", [(False, None), (True, torch.tensor([100, 200]))])
def test_delta2bbox(backend_type: Backend, add_ctr_clamp: bool, clip_border: bool, max_shape: tuple):
    ...
    def delta2bbox(*args, **kwargs):
        ...
        return mmdet.models.task_modules.coders.delta_xywh_bbox_coder.delta2bbox(*args, **kwargs)
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_func, model_inputs={"rois": rois.unsqueeze(0), "deltas": deltas.unsqueeze(0)}, deploy_cfg=deploy_cfg
    )
...
def test_tblr2bbox(backend_type: Backend):
    ...
    def tblr2bboxes(*args, **kwargs):
        ...
        return mmdet.models.task_modules.coders.tblr_bbox_coder.tblr2bboxes(*args, **kwargs)
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_func, model_inputs={"priors": priors, "tblr": tblr}, deploy_cfg=deploy_cfg
    )
...
def test__distancepointbboxcoder__decode(backend_type: Backend):
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs={"points": points, "pred_bboxes": pred_bboxes}, deploy_cfg=deploy_cfg
    )
...
def test_multiclass_nms_with_keep_top_k(pre_top_k):
        ...
        dict(
            onnx_config=dict(
                ...
                dynamic_axes=dict(
                    ...
                    scores={0: "batch_size", 1: "num_boxes", 2: "num_classes"},
                ),
            ),
    ...
    wrapped_func = WrapFunction(
        multiclass_nms, nms_type="nms", max_output_boxes_per_class=max_output_boxes_per_class, keep_top_k=keep_top_k
    )

    onnx_model_path = get_onnx_model(wrapped_func, model_inputs=model_inputs, deploy_cfg=deploy_cfg)
...
def test__anchorgenerator__single_level_grid_priors():
    ...
    generator = AnchorGenerator(scales=[8], ratios=[0.5, 1.0, 2.0], strides=[4])

    def single_level_grid_priors(input):
        return generator.single_level_grid_priors(input.shape[2:], 0, input.dtype, input.device)
    ...
    export(
        ...
        input_names=["input"],
        output_names=["output"],
        dynamic_axes=dict(input={2: "h", 3: "w"}),
    )
...
def convert_to_list(rewrite_output: Dict, output_names: List[str]) -> List:
    ...
    'output_names'.
    """
    outputs = [value for name, value in rewrite_output.items() if name in output_names]
    return outputs
...
def get_anchor_head_model():
    ...
    test_cfg = Config(
        dict(
            deploy_nms_pre=0, min_bbox_size=0, score_thr=0.05, nms=dict(type="nms", iou_threshold=0.5), max_per_img=100
        )
    )
...
def get_ssd_head_model():
    """SSDHead Config."""
    test_cfg = Config(
        dict(nms_pre=1000, nms=dict(type="nms", iou_threshold=0.45), min_bbox_size=0, score_thr=0.02, max_per_img=200)
    )
...
def get_rpn_head_model():
    """RPN Head Config."""
    test_cfg = Config(
        dict(deploy_nms_pre=0, nms_pre=0, max_per_img=100, nms=dict(type="nms", iou_threshold=0.7), min_bbox_size=0)
    )
...
def get_gfl_head_model():
    test_cfg = Config(
        dict(nms_pre=1000, min_bbox_size=0, score_thr=0.05, nms=dict(type="nms", iou_threshold=0.6), max_per_img=100)
    )
    anchor_generator = dict(
        type="AnchorGenerator", scales_per_octave=1, octave_base_scale=8, ratios=[1.0], strides=[8, 16, 32, 64, 128]
    )
    from mmdet.models.dense_heads import GFLHead

    model = GFLHead(num_classes=3, in_channels=256, reg_max=3, test_cfg=test_cfg, anchor_generator=anchor_generator)
    model.requires_grad_(False)
    return model
...
def test_focus_forward(backend_type):
    ...
    deploy_cfg = Config(dict(backend_config=dict(type=backend_type.value), onnx_config=dict(input_shape=None)))
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
    ...
    for model_output, rewrite_output in zip(model_outputs[0], rewrite_outputs):
        ...
        torch_assert_close(model_output, rewrite_output, rtol=1e-03, atol=1e-05)
...
def test_l2norm_forward(backend_type):
    ...
    if is_backend_output:
        for model_output, rewrite_output in zip(model_outputs[0], rewrite_outputs[0]):
        ...
        for model_output, rewrite_output in zip(model_outputs[0], rewrite_outputs[0]):
            model_output = model_output.squeeze().cpu().numpy()
            rewrite_output = rewrite_output.squeeze()
            assert np.allclose(model_output[0], rewrite_output, rtol=1e-03, atol=1e-05)
...
def test_predict_by_feat_of_rpn_head(backend_type: Backend):
    ...
    head.cpu().eval()
    s = 4
    batch_img_metas = [{"scale_factor": np.ones(4), "pad_shape": (s, s, 3), "img_shape": (s, s, 3)}]
    ...
    # (1, 36, 8, 8), (1, 36, 4, 4), (1, 36, 2, 2)
    seed_everything(1234)
    cls_score = [torch.rand(1, 9, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    seed_everything(5678)
    bboxes = [torch.rand(1, 36, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]

    # to get outputs of onnx model after rewrite
    batch_img_metas[0]["img_shape"] = torch.Tensor([s, s])
    wrapped_model = WrapModel(head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
    rewrite_inputs = {
...
def test_predict_by_feat_of_gfl_head(backend_type):
    ...
    seed_everything(1234)
    cls_score = [torch.rand(1, 3, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    seed_everything(5678)
...
@pytest.mark.parametrize("model_cfg_path", ["tests/test_codebase/test_mmdet/data/detr_model.json"])
...
@pytest.mark.parametrize("backend_type", [Backend.ONNXRUNTIME, Backend.OPENVINO])
def test_single_roi_extractor(backend_type: Backend):
    ...
    model_outputs = get_model_outputs(single_roi_extractor, "forward", model_inputs)

    backend_outputs, _ = get_rewrite_outputs(
        wrapped_model=single_roi_extractor, model_inputs=model_inputs, deploy_cfg=deploy_cfg
    )
...
def test_single_roi_extractor__ascend():
    ...
    deploy_cfg = mmengine.Config(
        dict(
            ...
            onnx_config=dict(input_names=input_names, output_names=output_names, input_shape=None),
            codebase_config=dict(
                type="mmdet",
                task="ObjectDetection",
            ),
    ...
    # get torch output
    model_outputs = get_model_outputs(single_roi_extractor_wrapper, "forward", model_inputs)

    # get backend output
    backend_outputs, _ = get_rewrite_outputs(
        wrapped_model=single_roi_extractor_wrapper, model_inputs=model_inputs, deploy_cfg=deploy_cfg
    )
    if isinstance(backend_outputs, dict):
        backend_outputs = backend_outputs.values()
    for model_output, backend_output in zip(model_outputs[0], backend_outputs):
...
def get_cascade_roi_head(is_instance_seg=False):
    ...
    all_target_stds = [[0.1, 0.1, 0.2, 0.2], [0.05, 0.05, 0.1, 0.1], [0.033, 0.033, 0.067, 0.067]]
    bbox_head = [
        {
    ...
    mask_roi_extractor = {
        "type": "SingleRoIExtractor",
        "roi_layer": {"type": "RoIAlign", "output_size": 1, "sampling_ratio": 0},
        "out_channels": 4,
        "featmap_strides": [4, 8, 16, 32],
    }
    mask_head = {
    ...
    test_cfg = Config(
        dict(score_thr=0.05, nms=Config(dict(type="nms", iou_threshold=0.5)), max_per_img=100, mask_thr_binary=0.5)
    )
...
def test_cascade_roi_head(backend_type: Backend):
    ...
    rcnn_test_cfg = ConfigDict(dict(score_thr=0.05, nms=dict(type="nms", iou_threshold=0.5), max_per_img=100))
    ...
    backend_outputs, _ = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=model_inputs, deploy_cfg=deploy_cfg
    )
...
def test_predict_by_feat_of_fovea_head(backend_type: Backend):
    ...
    # (1, 36, 8, 8), (1, 36, 4, 4), (1, 36, 2, 2)
    seed_everything(1234)
    cls_score = [torch.rand(1, fovea_head.num_classes, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    seed_everything(5678)
    bboxes = [torch.rand(1, 4, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]

    model_inputs = {"cls_scores": cls_score, "bbox_preds": bboxes, "batch_img_metas": batch_img_metas}
    model_outputs = get_model_outputs(fovea_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    batch_img_metas[0]["img_shape"] = torch.Tensor([s, s])
    wrapped_model = WrapModel(fovea_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "cls_scores": cls_score,
        "bbox_preds": bboxes,
    }
    ...
    if is_backend_output:
        for i in range(len(model_outputs)):
            assert np.allclose(model_outputs[i].bboxes, rewrite_outputs[0][i, :, :4], rtol=1e-03, atol=1e-05)
            assert np.allclose(model_outputs[i].scores, rewrite_outputs[0][i, :, 4], rtol=1e-03, atol=1e-05)
            assert np.allclose(model_outputs[i].labels, rewrite_outputs[1][i], rtol=1e-03, atol=1e-05)
    else:
        assert rewrite_outputs is not None
...
def test_cascade_roi_head_with_mask(backend_type: Backend):
    ...
    model_inputs = {"x": x}
    wrapped_model = WrapModel(
        cascade_roi_head, "predict_mask", batch_img_metas=[batch_img_metas], results_list=proposals
    )
    backend_outputs, _ = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=model_inputs, deploy_cfg=deploy_cfg
    )
...
def get_yolov3_head_model():
    ...
    model = YOLOV3Head(num_classes=4, in_channels=[16, 8, 4], out_channels=[32, 16, 8], test_cfg=test_cfg)
...
def test_yolov3_head_predict_by_feat(backend_type):
    ...
    seed_everything(1234)
    pred_maps = [torch.rand(1, 27, 5, 5), torch.rand(1, 27, 10, 10), torch.rand(1, 27, 20, 20)]
    # to get outputs of pytorch model
    model_inputs = {"pred_maps": pred_maps, "batch_img_metas": batch_img_metas}
    model_outputs = get_model_outputs(yolov3_head, "predict_by_feat", model_inputs)
    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(yolov3_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "pred_maps": pred_maps,
    }
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
...
def test_yolov3_head_predict_by_feat_ncnn():
    ...
    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(yolov3_head, "predict_by_feat", batch_img_metas=batch_img_metas[0], with_nms=True)
    rewrite_inputs = {
        "pred_maps": pred_maps,
    }
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
...
def test_centernet_head_predict_by_feat(backend_type: Backend):
    ...
    batch_img_metas = [
        {
            "border": np.array([11.0, 99.0, 11.0, 99.0], dtype=np.float32),
            "img_shape": (s, s),
    ...
    seed_everything(1234)
    center_heatmap_preds = [torch.rand(1, centernet_head.num_classes, s // 4, s // 4)]
    seed_everything(5678)
    ...
    model_outputs = get_model_outputs(centernet_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(centernet_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "center_heatmap_preds": center_heatmap_preds,
        "wh_preds": wh_preds,
        "offset_preds": offset_preds,
    ...
    if is_backend_output:
        ...
        min_shape = min(model_outputs[0].bboxes.shape[0], rewrite_outputs[0].shape[1], 5)
        for i in range(len(model_outputs)):
            ...
            assert np.allclose(
                model_outputs[i].bboxes[:min_shape], rewrite_outputs[0][i, :min_shape, :4], rtol=1e-03, atol=1e-05
            )
            assert np.allclose(
                model_outputs[i].scores[:min_shape], rewrite_outputs[0][i, :min_shape, 4], rtol=1e-03, atol=1e-05
            )
            assert np.allclose(
                model_outputs[i].labels[:min_shape], rewrite_outputs[1][i, :min_shape], rtol=1e-03, atol=1e-05
            )
    else:
        assert rewrite_outputs is not None
...
def test_yolox_head_predict_by_feat(backend_type: Backend):
    ...
    seed_everything(1234)
    cls_scores = [torch.rand(1, yolox_head.num_classes, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(5678)
    bbox_preds = [torch.rand(1, 4, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(9101)
    objectnesses = [torch.rand(1, 1, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)]
    ...
    model_outputs = get_model_outputs(yolox_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(yolox_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
    rewrite_inputs = {
        "cls_scores": cls_scores,
        "bbox_preds": bbox_preds,
        "objectnesses": objectnesses,
...
def test_yolox_head_predict_by_feat_ncnn():
    ...
    seed_everything(1234)
    cls_scores = [torch.rand(1, yolox_head.num_classes, pow(2, i), pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(5678)
    bbox_preds = [torch.rand(1, 4, pow(2, i), pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(9101)
    objectnesses = [torch.rand(1, 1, pow(2, i), pow(2, i)) for i in range(3, 0, -1)]

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(yolox_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "cls_scores": cls_scores,
        "bbox_preds": bbox_preds,
        "objectnesses": objectnesses,
...
def get_deploy_cfg(backend_type: Backend, ir_type: str):
    return Config(
        dict(
            backend_config=dict(type=backend_type.value),
            onnx_config=dict(type=ir_type, output_names=["dets", "labels"], input_shape=None),
            codebase_config=dict(
                type="mmdet",
                task="ObjectDetection",
                post_processing=dict(
...
@pytest.mark.parametrize(
    "backend_type, ir_type",
    [(Backend.ONNXRUNTIME, "onnx"), (Backend.OPENVINO, "onnx"), (Backend.TORCHSCRIPT, "torchscript")],
)
def test_base_dense_head_predict_by_feat(backend_type: Backend, ir_type: str):
    ...
    # (1, 36, 8, 8), (1, 36, 4, 4), (1, 36, 2, 2)
    seed_everything(1234)
    cls_score = [torch.rand(1, 36, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    seed_everything(5678)
    bboxes = [torch.rand(1, 36, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]

    # to get outputs of pytorch model
    model_inputs = {"cls_scores": cls_score, "bbox_preds": bboxes, "batch_img_metas": batch_img_metas}
    model_outputs = get_model_outputs(anchor_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    batch_img_metas[0]["img_shape"] = torch.Tensor([s, s])
    wrapped_model = WrapModel(anchor_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "cls_scores": cls_score,
        "bbox_preds": bboxes,
    }
...
def test_base_dense_head_predict_by_feat__ncnn():
    ...
    # to get outputs of onnx model after rewrite
    batch_img_metas[0]["img_shape"] = torch.Tensor([s, s])
    wrapped_model = WrapModel(anchor_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
    rewrite_inputs = {
        "cls_scores": cls_score,
        "bbox_preds": bboxes,
    }
...
def test_base_dense_head_get_bboxes__rknn():
    ...
    ssd_head.cpu().eval()
    s = 128
    img_metas = [{"scale_factor": np.ones(4), "pad_shape": (s, s, 3), "img_shape": (s, s, 3)}]
    output_names = ["output"]
    input_names = []
    for i in range(6):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            ...
            onnx_config=dict(
                input_names=input_names, output_names=output_names, input_shape=None, dynamic_axes=dynamic_axes
            ),
            codebase_config=dict(
                type="mmdet",
                task="ObjectDetection",
    ...
    num_prior = 6
    seed_everything(1234)
    cls_score = [torch.rand(1, 30, feat_shape[i], feat_shape[i]) for i in range(num_prior)]
    seed_everything(5678)
    bboxes = [torch.rand(1, 24, feat_shape[i], feat_shape[i]) for i in range(num_prior)]

    # to get outputs of onnx model after rewrite
    img_metas[0]["img_shape"] = [s, s]
    wrapped_model = WrapModel(ssd_head, "get_bboxes", img_metas=img_metas, with_nms=True)
    rewrite_inputs = {
        "cls_scores": cls_score,
        "bbox_preds": bboxes,
    }
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg, run_with_backend=False
    )
...
def test_reppoints_head_predict_by_feat(backend_type: Backend, ir_type: str):
    ...
    # (1, 4, 8, 8), (1, 4, 4, 4), (1, 4, 2, 2)
    seed_everything(1234)
    cls_score = [torch.rand(1, 4, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    seed_everything(5678)
    bboxes = [torch.rand(1, 4, pow(2, i), pow(2, i)) for i in range(5, 0, -1)]
    ...
    # to get outputs of onnx model after rewrite
    batch_img_metas[0]["img_shape"] = torch.Tensor([s, s])
    wrapped_model = WrapModel(dense_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
    rewrite_inputs = {
        "cls_scores": cls_score,
        "bbox_preds": bboxes,
    }
...
def test_reppoints_head_points2bbox(backend_type: Backend, ir_type: str):
    ...
    deploy_cfg = Config(
        dict(
            backend_config=dict(type=backend_type.value),
            onnx_config=dict(input_shape=None, input_names=["pts"], output_names=output_names),
        )
    )
    ...
    _ = get_rewrite_outputs(wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg)
        ...
        dict(
            backend_config=dict(
                ...
                model_inputs=[
                    dict(
                        input_shapes=dict(
                            x=dict(min_shape=[12, 49, 96], opt_shape=[12, 49, 96], max_shape=[12, 49, 96]),
                            mask=dict(min_shape=[12, 49, 49], opt_shape=[12, 49, 49], max_shape=[12, 49, 49]),
                        )
                    )
                ],
            ),
            onnx_config=dict(input_shape=None, input_names=["x", "mask"], output_names=output_names),
        )
    )
        ...
        dict(
            backend_config=dict(
                ...
                model_inputs=[
                    dict(
                        input_shapes=dict(
                            query=dict(min_shape=[1, 60800, 96], opt_shape=[1, 60800, 96], max_shape=[1, 60800, 96])
                        )
                    )
                ],
            ),
            onnx_config=dict(input_shape=None, input_names=["query"], output_names=output_names),
        )
    )
...
@pytest.mark.skipif(reason="Only support GPU test", condition=not torch.cuda.is_available())
@pytest.mark.parametrize("backend_type", [(Backend.TENSORRT)])
def test_mlvl_point_generator__single_level_grid_priors__tensorrt(backend_type: Backend):
    check_backend(backend_type)
    ...
    deploy_cfg = Config(
        dict(
            backend_config=dict(type=backend_type.value),
            onnx_config=dict(input_shape=None, input_names=["featmap_size", "level_idx"], output_names=output_names),
        )
    )
    ...
    wrapped_model = WrapModel(model, "single_level_grid_priors", with_stride=with_stride)
    rewrite_inputs = {"featmap_size": featmap_size, "level_idx": torch.tensor(0)}
    _ = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg, run_with_backend=False
    )


@pytest.mark.parametrize("backend_type, ir_type", [(Backend.ONNXRUNTIME, "onnx")])
def test_detrhead__predict_by_feat(backend_type: Backend, ir_type: str):
    ...
    wrapped_model = WrapModel(dense_head, "predict_by_feat", batch_img_metas=img_metas)
    rewrite_inputs = {
...
def test_solo_head_predict_by_feat(backend_type: Backend):
    ...
    mask_preds = [torch.rand(1, num_grid**2, s // 4, s // 4) for num_grid in num_grids]
    seed_everything(5678)
    cls_scores = [torch.rand(1, solo_head.num_classes, num_grid, num_grid) for num_grid in num_grids]
    ...
    model_outputs = get_model_outputs(solo_head, "predict_by_feat", model_inputs)

    wrapped_model = WrapModel(solo_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "mlvl_mask_preds": mask_preds,
        "mlvl_cls_scores": cls_scores,
    }
...
def get_rtmdet_head_model():
    ...
    test_cfg = Config(
        dict(
            deploy_nms_pre=0, min_bbox_size=0, score_thr=0.05, nms=dict(type="nms", iou_threshold=0.6), max_per_img=100
        )
    )
...
def test_rtmdet_head_predict_by_feat_ncnn():
    ...
    seed_everything(1234)
    cls_scores = [torch.rand(1, 1, 40, 40), torch.rand(1, 1, 20, 20), torch.rand(1, 1, 10, 10)]

    bbox_preds = [torch.rand(1, 4, 40, 40), torch.rand(1, 4, 20, 20), torch.rand(1, 4, 10, 10)]

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(rtmdet_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
...
def get_solov2_head_model():
    ...
    model = SOLOV2Head(
        num_classes=4, in_channels=32, feat_channels=64, mask_feature_head=mask_feature_head_cfg, test_cfg=test_cfg
    )
...
def test_solov2_head_predict_by_feat(backend_type):
    ...
    num_grids = [24, 20, 16, 12, 8]
    kernel_preds = [
        torch.rand(1, solov2_head.mask_feature_head.out_channels, num_grid, num_grid) for num_grid in num_grids
    ]
    cls_scores = [torch.rand(1, solov2_head.num_classes, num_grid, num_grid) for num_grid in num_grids]
    mask_feats = torch.rand(1, solov2_head.mask_feature_head.out_channels, s // 4, s // 4)
    # to get outputs of pytorch model
    model_inputs = {
    ...
    model_outputs = get_model_outputs(solov2_head, "predict_by_feat", model_inputs)
    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(solov2_head, "predict_by_feat", batch_img_metas=batch_img_metas)
    rewrite_inputs = {
        "mlvl_kernel_preds": kernel_preds,
        "mlvl_cls_scores": cls_scores,
        "mask_feats": mask_feats,
