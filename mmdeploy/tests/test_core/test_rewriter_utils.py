...
from mmdeploy.core.rewriters.rewriter_utils import BackendChecker, RewriterRegistry, collect_env
...
class TestChecker:
    ...
    def test_lib_version_checker(self):
        true_checker = rewriter_utils.LibVersionChecker("mmdeploy", mmdeploy.__version__, mmdeploy.__version__)
        assert true_checker.check(self.env) is True

        false_checker = rewriter_utils.LibVersionChecker("mmdeploy", max_version="0.0.0")
        assert false_checker.check(self.env) is False
...
def test_register_object():
    ...
    @registry.register_object("add", backend=Backend.DEFAULT.value, ir=IR.DEFAULT, extra_checkers=checker)
    def add(a, b):
        return a + b
...
def test_get_records():
    ...
    @registry.register_object("get_num", backend=Backend.ONNXRUNTIME.value, ir=IR.ONNX)
    def get_num_1():
        return 1

    @registry.register_object("get_num", backend=Backend.ONNXRUNTIME.value, ir=IR.TORCHSCRIPT)
    def get_num_2():
        return 2

    @registry.register_object("get_num", backend=Backend.TENSORRT.value, ir=IR.ONNX)
    def get_num_3():
        return 3

    @registry.register_object("get_num", backend=Backend.TENSORRT.value, ir=IR.TORCHSCRIPT)
    def get_num_4():
        return 4

    @registry.register_object("get_num", backend=Backend.DEFAULT.value, ir=IR.DEFAULT)
    def get_num_5():
        return 5

    records = dict(registry.get_records(collect_env(Backend.ONNXRUNTIME, IR.ONNX)))
    assert records["get_num"]["_object"]() == 1

    records = dict(registry.get_records(collect_env(Backend.ONNXRUNTIME, IR.TORCHSCRIPT)))
    assert records["get_num"]["_object"]() == 2

    records = dict(registry.get_records(collect_env(Backend.TENSORRT, IR.ONNX)))
    assert records["get_num"]["_object"]() == 3

    records = dict(registry.get_records(collect_env(Backend.TENSORRT, IR.TORCHSCRIPT)))
