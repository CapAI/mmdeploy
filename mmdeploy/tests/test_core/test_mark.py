...
def test_mark():
    ...
    assert attribute_to_dict(nodes[0].attribute) == dict(
        dtype=1, func="add", func_id=0, id=0, type="input", name="a", shape=[2, 3, 4]
    )

    assert nodes[1].op_type == "Mark"
    assert nodes[1].domain == "mmdeploy"
    assert attribute_to_dict(nodes[1].attribute) == dict(
        dtype=1, func="add", func_id=0, id=1, type="input", name="b", shape=[2, 3, 4]
    )
    ...
    assert attribute_to_dict(nodes[3].attribute) == dict(
        dtype=1, func="add", func_id=0, id=0, type="output", name="c", shape=[2, 3, 4]
    )

    with RewriterContext(
        cfg=None, backend=Backend.TORCHSCRIPT.value, ir=IR.TORCHSCRIPT
    ), torch.no_grad(), torch.jit.optimized_execution(True):
