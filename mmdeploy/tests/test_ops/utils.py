...
class TestOnnxRTExporter:
    ...
    def run_and_validate(
        ...
        with torch.no_grad():
            onnx_outputs = onnx_model.forward(dict(zip(input_names, input_list)))
...
class TestTensorRTExporter:
    ...
    def run_and_validate(
                        ...
                        dict(
                            input_shapes=dict(
                                zip(
                                    input_names,
                                    [
                                        dict(min_shape=data.shape, opt_shape=data.shape, max_shape=data.shape)
        ...
        trt_apis.onnx2tensorrt(work_dir, filename, 0, deploy_cfg=deploy_cfg, onnx_model=onnx_file_path)
        if expected_result is None and not isinstance(model, onnx.onnx_ml_pb2.ModelProto):
            with torch.no_grad():
                model_outputs = model(*input_list)
        else:
            model_outputs = expected_result
...
class TestNCNNExporter:
    ...
    def run_and_validate(
        ...
        subprocess.call([onnx2ncnn_path, onnx_file_path, ncnn_param_path, ncnn_bin_path])
        ...
        if isinstance(model_outputs, torch.Tensor):
            ...
            model_outputs = list(model_outputs)
        model_outputs = [model_output.float() for model_output in model_outputs]
        ...
        if model_name.startswith("topk_no_sorted"):
            ...
            model_outputs = torch.stack(model_outputs, dim=-1).sort(dim=dim).values
            ncnn_outputs = torch.stack(ncnn_outputs, dim=-1).sort(dim=dim).values
    ...
    def onnx2ncnn(self, model, model_name, output_names, save_dir=None):
        ...
        if save_dir is None:
            with tempfile.TemporaryDirectory() as save_dir:
                return _from_onnx(self, model, model_name, output_names, save_dir=save_dir)
        else:
            return _from_onnx(self, model, model_name, output_names, save_dir=save_dir)
