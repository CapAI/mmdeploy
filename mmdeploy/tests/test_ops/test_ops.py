...
@pytest.mark.parametrize("pool_h,pool_w,spatial_scale,sampling_ratio", [(2, 2, 1.0, 2), (4, 4, 2.0, 4)])
def test_roi_align(backend, pool_h, pool_w, spatial_scale, sampling_ratio, input_list=None, save_dir=None):
    backend.check_env()
    ...
    def wrapped_function(torch_input, torch_rois):
        return roi_align(torch_input, torch_rois, (pool_w, pool_h), spatial_scale, sampling_ratio, "avg", True)

    wrapped_model = WrapFunction(wrapped_function).eval()

    with RewriterContext(
        Config({"backend_config": {"type": backend.backend_name}}), backend=backend.backend_name, opset=11
    ):
...
def test_grid_sample(backend, mode, padding_mode, align_corners, input_list=None, save_dir=None):
    backend.check_env()
    ...
    grid = nn.functional.affine_grid(grid, (1, 1, input.shape[2] * 2, input.shape[3] * 2)).type_as(input)

    def wrapped_function(inputs, grid):
        return nn.functional.grid_sample(
            inputs, grid, mode=mode, padding_mode=padding_mode, align_corners=align_corners
        )
...
def test_bicubic_interpolate(
    backend, dynamic_export, mode, align_corners, output_size, scale_factor, n, c, h, w, input_list=None, save_dir=None
):
    backend.check_env()
    ...
    if output_size is None:
        resize = nn.Upsample(scale_factor=scale_factor, mode=mode, align_corners=align_corners)
    else:
        resize = nn.Upsample(size=output_size, mode=mode, align_corners=align_corners)
...
@pytest.mark.parametrize(
    "in_channels,out_channels,stride,padding," "dilation,groups,deform_groups,kernel_size",
    [(3, 64, 1, 0, 1, 1, 1, 3), (1, 32, 3, 2, 1, 1, 1, 3)],
)
...
def test_modulated_deform_conv(
    ...
    if input_list is None:
        input = torch.rand(1, in_channels, 28, 28, requires_grad=False)  # (n, c, h, w)
    else:
        input = torch.tensor(input_list[0])
    ...
    model = ModulatedDeformConv2d(
        in_channels, out_channels, kernel_size, stride, padding, dilation, groups, deform_groups, bias
    ).eval()
...
@pytest.mark.parametrize(
    "in_channels,out_channels,stride,padding," "dilation,groups,deform_groups,kernel_size", [(1, 32, 3, 2, 1, 1, 1, 3)]
)
def test_deform_conv(in_channels, out_channels, stride, padding, dilation, groups, deform_groups, kernel_size):
    inputs = torch.rand(1, in_channels, 28, 28, requires_grad=False)  # (n, c, h, w)
    ...
    model = DeformConv2d(
        in_channels, out_channels, kernel_size, stride, padding, dilation, groups, deform_groups
    ).eval()
...
def test_instance_norm(backend, dynamic_export, fp16_mode, n, c, h, w, input_list=None, save_dir=None):
    backend.check_env()
...
@pytest.mark.parametrize("backend", [TEST_TENSORRT])
@pytest.mark.parametrize(
    "num_classes,pre_topk,after_topk,iou_threshold," "score_threshold,background_label_id", [(5, 6, 3, 0.7, 0.1, -1)]
)
def test_batched_nms(
    ...
    expected_result = (expected_result[0][:, 0:-1, :], expected_result[1][:, 0:-1])
    ...
    def wrapped_function(boxes, scores):
        return batched_nms(
            boxes, scores, num_classes, pre_topk, after_topk, iou_threshold, score_threshold, background_label_id
        )
...
def test_batched_rotated_nms(
    ...
    if input_list is None:
        nms_boxes = torch.tensor(
            [
                [
                    [291.1746, 316.2263, 343.5029, 347.7312, 1.0],
                    [288.4846, 315.0447, 343.7267, 346.5630, 2.0],
                    [288.5307, 318.1989, 341.6425, 349.7222, 3.0],
                    [918.9102, 83.7463, 933.3920, 164.9041, 4.0],
                    [895.5786, 78.2361, 907.8049, 172.0883, 5.0],
                    [292.5816, 316.5563, 340.3462, 352.9989, 6.0],
                    [609.4592, 83.5447, 631.2532, 144.0749, 7.0],
                    [917.7308, 85.5870, 933.2839, 168.4530, 8.0],
                    [895.5138, 79.3596, 908.2865, 171.0418, 9.0],
                    [291.4747, 318.6987, 347.1208, 349.5754, 10.0],
                ]
            ]
        )
        scores = torch.tensor(
...
@pytest.mark.parametrize("backend", [TEST_TENSORRT])
@pytest.mark.parametrize(
    "out_size, pool_mode, sampling_ratio,roi_scale_factor," " finest_scale,featmap_strides, aligned",
    [(tuple([2, 2]), 0, 2, 1.0, 2, list([2.0, 4.0]), 1), (tuple([2, 2]), 1, 2, 1.0, 2, list([2.0, 4.0]), 1)],
)
def test_multi_level_roi_align(
    backend,
    out_size,
    pool_mode,
    sampling_ratio,
    ...
    if input_list is None:
        ...
        rois = torch.tensor([[0.0, 0.0, 0.0, 4.0, 4.0]])
        if pool_mode == 1:
            expected_result = torch.tensor(
                [
                    [
                        [[0.1939, 0.3950], [0.3437, 0.4543]],
                        [[0.0778, 0.1641], [0.1305, 0.2301]],
                        [[0.1542, 0.2413], [0.2094, 0.2688]],
                    ]
                ]
            )
        else:
            expected_result = torch.tensor(
                [
                    [
                        [[0.1939, 0.4956], [0.4185, 0.5167]],
                        [[0.0778, 0.2073], [0.1569, 0.3162]],
                        [[0.1542, 0.2849], [0.2370, 0.3053]],
                    ]
                ]
            )
    ...
    inputs = [
        onnx.helper.make_tensor_value_info(input_name[i + 1], onnx.TensorProto.FLOAT, shape=input[i].shape)
        for i in range(len(input_name) - 1)
    ]
    inputs.append(onnx.helper.make_tensor_value_info("rois", onnx.TensorProto.FLOAT, shape=rois.shape))
    outputs = [onnx.helper.make_tensor_value_info("bbox_feats", onnx.TensorProto.FLOAT, shape=expected_result.shape)]
    ...
    onnx_model = onnx.helper.make_model(graph, producer_name="pytorch", producer_version="1.8")
    onnx_model.opset_import[0].version = 11
    onnx_model.opset_import.append(onnx.onnx_ml_pb2.OperatorSetIdProto(domain="mmdeploy", version=1))
...
def test_topk(backend, k, dim, largest, sorted, input_list=None, save_dir=None):
    backend.check_env()
...
@pytest.mark.parametrize("backend", [TEST_NCNN])
@pytest.mark.parametrize("dim, n, c, h, w", [(1, 1, 1, 1, 8), (2, 1, 1, 5, 7), (3, 1, 3, 10, 15)])
def test_shape(
    backend,
    dim,
    n,
    ...
    if input_list is None:
        ...
        assert (
            tuple(input.shape) == orig_shape
        ), "input.shape must the \
            same as orig_shape"
    ...
    shape_graph = make_graph(
        [shape_node],
        "shape_graph",
        [make_tensor_value_info(input_names[0], onnx.TensorProto.FLOAT, orig_shape)],
        [make_tensor_value_info(output_names[0], onnx.TensorProto.FLOAT, (dim + 1,))],
    )
    shape_model = make_model(shape_graph)

    with RewriterContext(cfg={}, backend=backend.backend_name, opset=11):
        ncnn_model = backend.onnx2ncnn(shape_model, "shape", output_names, save_dir)
...
@pytest.mark.parametrize("backend", [TEST_NCNN])
@pytest.mark.parametrize("dim, n, c, h, w", [(1, 1, 1, 1, 8), (2, 1, 1, 5, 7), (3, 1, 3, 10, 15)])
@pytest.mark.parametrize("val", [0.0, 1.0, -3, 4.25])
def test_constantofshape(
    ...
    if input_list is None:
        ...
        assert input.dim() == dim + 1, "input.dim() must equal to dim + 1"
        assert (
            tuple(input.shape) == (n, c, h, w)[-dim - 1 :]
        ), "input.shape must the \
            same as orig_shape"

    assert (
        input.shape[0] == 1
    ), f"ncnn input batch must be 1, \
        got {input.shape[0]}"
    assert (
        input[0][0] == 1
    ), f"ncnn output mat batch must be 1, \
        got {input[0][0]}"

    constantofshape_node = make_node("ConstantOfShape", input_names, output_names, value=float(val))
    assert len(input_names) == 1, "length of input_names must be 1"
    assert len(output_names) == 1, "length of output_names must be 1"
    constantofshape_graph = make_graph(
        [constantofshape_node],
        "constantofshape_graph",
        [make_tensor_value_info(input_names[0], onnx.TensorProto.FLOAT, input.shape)],
        [make_tensor_value_info(output_names[0], onnx.TensorProto.FLOAT, torch.Size(input[0]))],
    )
    constantofshape_model = make_model(constantofshape_graph)
    with RewriterContext(cfg={}, backend=backend.backend_name, opset=11):
        ncnn_model = backend.onnx2ncnn(constantofshape_model, "constantofshape", output_names, save_dir)
...
@pytest.mark.parametrize("backend", [TEST_NCNN])
@pytest.mark.parametrize(
    "axis, data_dims, indice_dims", [(0, 1, 1), (0, 2, 1), (1, 2, 1), (0, 3, 1), (1, 3, 1), (2, 3, 1)]
)
def test_gather(
    backend,
    axis,
    ...
    if input_list is None:
        ...
        data = input_list[0]
        indice = input_list[1]
    assert data.shape[0] == 1, "ncnn batch must be 1," f"but got {data.shape[0]}"
    assert indice.shape[0] == 1, "ncnn batch must be 1," f"but got {indice.shape[0]}"

    gather_node = make_node("Gather", input_names, output_names, axis=axis + 1)
    gather_graph = make_graph(
        [gather_node],
        "gather_graph",
        [
    ...
    with RewriterContext(cfg={}, backend=backend.backend_name, opset=11):
        ncnn_model = backend.onnx2ncnn(gather_model, "gather", output_names, save_dir)
    ...
    ort_model = ORTWrapper(gather_model.SerializeToString(), device="cpu", output_names=output_names)
    model_outputs = ort_model(dict(zip(input_names, [data, indice[0]])))
    model_outputs = ort_model.output_to_list(model_outputs)

    ncnn_outputs = ncnn_model(dict(zip(input_names, [data.float(), indice.float()])))
    ncnn_outputs = [ncnn_outputs[name] for name in output_names]
    assert_allclose(model_outputs, ncnn_outputs, tolerate_small_mismatch)
...
@pytest.mark.parametrize("backend", [TEST_NCNN])
@pytest.mark.parametrize("input_dim, output_dim", [(1, 1), (1, 2), (1, 3), (2, 2), (2, 3), (3, 3)])
def test_expand(backend, input_dim, output_dim, input_list=None, save_dir=None):
    backend.check_env()
    if input_list is None:
...
@pytest.mark.parametrize("score_threshold", [0.0, 0.1])
def test_nms_rotated(backend, iou_threshold, score_threshold, save_dir=None):
    ...
    def wrapped_function(torch_boxes, torch_scores):
        return ONNXNMSRotatedOp.apply(torch_boxes, torch_scores, iou_threshold, score_threshold)
...
@pytest.mark.parametrize("backend", [TEST_ONNXRT])
@pytest.mark.parametrize("pool_h,pool_w,spatial_scale,sampling_ratio", [(2, 2, 1.0, 2), (4, 4, 2.0, 4)])
def test_roi_align_rotated(backend, pool_h, pool_w, spatial_scale, sampling_ratio, input_list=None, save_dir=None):
    backend.check_env()

    if input_list is None:
        # input = torch.rand(1, 1, 16, 16, dtype=torch.float32)
        input = torch.tensor([[[[1.0, 2.0], [3.0, 4.0]]]], dtype=torch.float32)
        single_roi = torch.tensor([[0.0, 0.5, 0.5, 1.0, 1.0, 0]], dtype=torch.float32)
    else:
        input = torch.tensor(input_list[0], dtype=torch.float32)
        single_roi = torch.tensor(input_list[1], dtype=torch.float32)
    ...
    def wrapped_function(torch_input, torch_rois):
        return roi_align_rotated(torch_input, torch_rois, (pool_w, pool_h), spatial_scale, sampling_ratio, True, False)
...
@pytest.mark.parametrize("backend", [TEST_TENSORRT])
@pytest.mark.parametrize(
    "out_size, clockwise, sampling_ratio, roi_scale_factor," " finest_scale, featmap_strides, aligned",
    [(tuple([2, 2]), False, 2, 1.0, 2, list([1.0]), 1)],
)
def test_multi_level_rotated_roi_align(
    backend,
    out_size,
    clockwise,
    sampling_ratio,
    ...
    if input_list is None:
        ...
        rois = torch.tensor([[0.0, 1.5, 1.5, 3.0, 3.0, np.pi / 2]])
...
def test_trt_grid_priors(backend, strides, input_list=None, save_dir=None):
    ...
    class GridPriorsTestOps(torch.autograd.Function):
        @staticmethod
        def forward(ctx, base_anchor, feat_h, feat_w, stride_h: int, stride_w: int):
            a = base_anchor.shape[0]
            return base_anchor.new_empty(feat_h * feat_w * a, 4)

        @staticmethod
        def symbolic(g, base_anchor, feat_h, feat_w, stride_h: int, stride_w: int):
            ...
            return g.op(
                "mmdeploy::GridPriorsTRT", base_anchor, zero_h, zero_w, stride_h_i=stride_h, stride_w_i=stride_w
            )
    ...
    class GridPriorsTestModel(torch.nn.Module):
        ...
        def forward(self, x):
            ...
            return GridPriorsTestOps.apply(base_anchors, h, w, strides[0], strides[1])
...
def test_gather_topk(backend, save_dir=None):
    ...
    with RewriterContext(
        ...
        backend.run_and_validate(model, [x], "gather_topk", input_names=["x"], output_names=["out"], save_dir=save_dir)
...
def test_multiclass_nms_rotated_with_keep_top_k(backend, pre_top_k):
    ...
    wrapped_func = WrapFunction(multiclass_nms, nms_type="nms_rotated", keep_top_k=keep_top_k)
...
def test_multi_scale_deformable_attn(backend, save_dir=None):
    ...
    class TestModel(torch.nn.Module):
        ...
        def forward(self, value, value_spatial_shapes, level_start_index, sampling_locations, attention_weights):
    ...
    with RewriterContext(
        ...
        backend.run_and_validate(
            model,
            [value, value_spatial_shapes, level_start_index, sampling_locations, attention_weights],
