...
def generate_torchscript_file():
    ...
    trace(model, example_inputs, output_path_prefix=output_prefix, backend=backend, context_info=context_info)
...
def ir2backend(backend, onnx_file, ts_file):
    if backend == Backend.TENSORRT:
        ...
        from_onnx(
            ...
            {"input": {"min_shape": [1, 3, 8, 8], "opt_shape": [1, 3, 8, 8], "max_shape": [1, 3, 8, 8]}},
        )
        return backend_file
        ...
        model_inputs = mmengine.Config(dict(input_shapes=dict(input=test_img.shape)))
        ...
        from_onnx(onnx_file, lib_file, shape=shape, dtype=dtype, tuner=tuner_dict)
        ...
        output_names = ["output"]
        from mmdeploy.backend.coreml.torchscript2coreml import from_torchscript, get_model_suffix
        ...
        from_torchscript(
            ...
            input_names=input_names,
            output_names=output_names,
            input_shapes=dict(input=dict(min_shape=[1, 3, 8, 8], default_shape=[1, 3, 8, 8], max_shape=[1, 3, 8, 8])),
            convert_to=convert_to,
        )
        suffix = get_model_suffix(convert_to)
        return output_file_prefix + suffix
    else:
        raise NotImplementedError(f"Convert for {backend.value} has not been implemented.")
...
def create_wrapper(backend, model_files):
    ...
    if isinstance(model_files, str):
        ...
        deploy_cfg = dict(backend_config=dict(common_config=dict(target_platform=target_platform)))
    return backend_mgr.build_wrapper(
        model_files, input_names=input_names, output_names=output_names, deploy_cfg=deploy_cfg
    )
