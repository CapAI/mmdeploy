...
from mmdeploy.utils import IR, Backend, get_backend, get_dynamic_axes, get_ir_config, get_onnx_config
...
def backend_checker(backend: Backend, require_plugin: bool = False):
    ...
    checker = pytest.mark.skipif(not result, reason=f"{backend.value} package is not available")
...
class SwitchBackendWrapper:
    ...
    def recover(self):
        ...
        assert self.init is not None and self.forward is not None, "recover method must be called after exchange"
        obj = self._recover_class
...
def get_flatten_inputs(model_inputs: Dict[str, Union[Tuple, List, torch.Tensor]]) -> Dict[str, torch.Tensor]:
...
def get_onnx_model(
    wrapped_model: nn.Module, model_inputs: Dict[str, Union[Tuple, List, torch.Tensor]], deploy_cfg: Config
) -> str:
    ...
    with RewriterContext(cfg=deploy_cfg, backend=backend.value, opset=11), torch.no_grad():
        torch.onnx.export(
            model,
            (model_inputs, {}),
            onnx_file_path,
...
def get_ts_model(
    ...
    trace(wrapped_model, example_inputs, output_path_prefix=output_prefix, backend=backend, context_info=context_info)
    return ir_file_path


def get_backend_outputs(
    ir_file_path: str, model_inputs: Dict[str, Union[Tuple, List, torch.Tensor]], deploy_cfg: Config
) -> Union[Any, None]:
    ...
    backend_files = to_backend(backend.value, [ir_file_path], work_dir=work_dir, deploy_cfg=deploy_cfg, device=device)
    ...
    backend_model = BaseBackendModel._build_wrapper(
        backend, backend_files, device, input_names=input_names, output_names=output_names
    )
    with torch.no_grad():
...
def get_random_name(length: int = 10, seed: Optional[Union[int, float, str, bytes, bytearray]] = None) -> str:
