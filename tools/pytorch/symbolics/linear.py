...
def linear_normal(g, input, weight, bias):
    ...
    PyTorch `nn.Linear` will be exported as ONNX node 'Gemm'.
    """
    return g.op("Gemm", input, weight, bias, alpha_f=1.0, beta_f=1.0, transA_i=0, transB_i=1)


@SYMBOLIC_REWRITER.register_symbolic("linear", is_pytorch=True, backend=Backend.NCNN.value)
