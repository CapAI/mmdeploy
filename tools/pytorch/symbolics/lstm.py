...
@FUNCTION_REWRITER.register_rewriter(func_name="torch.onnx.symbolic_opset9._generic_rnn", backend="ncnn")
    ...
    for i in range(num_layers):
        ...
        if variant == "RNN":
            ...
            prev_output, h_out = g.op(
                "RNN", *inputs, outputs=2, hidden_size_i=hidden_size, activations_s=activation, **extra_kwargs
            )
        elif variant == "GRU":
            prev_output, h_out = g.op(
                "GRU", *inputs, outputs=2, hidden_size_i=hidden_size, linear_before_reset_i=1, **extra_kwargs
            )
        elif variant == "LSTM":
            # g.op will add some node to h0 and c0,
            # which is not necessary for us
            prev_output, h_out, c_out = g.op(
                "ncnn::LSTM", *inputs, outputs=3, hidden_size_i=hidden_size, **extra_kwargs
            )
        if bidirectional:
