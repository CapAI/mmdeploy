...
def generate_onnx_file(model, export_img, onnx_file, input_name, output_name):
    with torch.no_grad():
        dynamic_axes = {input_name: {0: "batch", 2: "width", 3: "height"}, output_name: {0: "batch"}}
        torch.onnx.export(
            model,
            export_img,
            onnx_file,
...
def get_outputs(pytorch_model, openvino_model_path, input, input_name, output_name):
...
@pytest.mark.parametrize("get_deploy_cfg", [get_base_deploy_cfg, get_deploy_cfg_with_mo_args])
@backend_checker(Backend.OPENVINO)
def test_onnx2openvino(get_deploy_cfg):
    from mmdeploy.apis.openvino import from_onnx, get_mo_options_from_cfg, get_output_model_file
    ...
    generate_onnx_file(pytorch_model, export_img, onnx_file, input_name, output_name)
    ...
    assert osp.exists(openvino_model_path), "The file (.xml) for OpenVINO IR has not been created."

    test_img = torch.rand([1, 3, 16, 16])
    output_pytorch, openvino_output = get_outputs(pytorch_model, openvino_model_path, test_img, input_name, output_name)
    assert np.allclose(output_pytorch, openvino_output), "OpenVINO and PyTorch outputs are not the same."
...
def test_get_input_info_from_cfg():
    ...
    deploy_cfg = Config({"backend_config": {"model_inputs": [{"opt_shapes": expected_input_info}]}})
    input_info = get_input_info_from_cfg(deploy_cfg)
    assert input_info == expected_input_info, (
        "Test 2: " "The expected value of 'input_info' does not match the received one."
    )
    ...
    deploy_cfg.merge_from_dict({"onnx_config": {"input_names": [onnx_config_input_name]}})
    expected_input_info = {onnx_config_input_name: shape}
    input_info = get_input_info_from_cfg(deploy_cfg)
    assert input_info == expected_input_info, (
        "Test 3: " "The expected value of 'input_info' does not match the received one."
    )

    # Test 4
    # The case where 'backend_config.model_inputs.opt_shapes'
    # is given by a list, not a dictionary.
    deploy_cfg.merge_from_dict({"backend_config": {"model_inputs": [{"opt_shapes": [shape]}]}})
    input_info = get_input_info_from_cfg(deploy_cfg)
    assert input_info == expected_input_info, (
        "Test 4: " "The expected value of 'input_info' does not match the received one."
    )
