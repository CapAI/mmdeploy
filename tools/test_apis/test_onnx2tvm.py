...
def test_onnx2tvm():
    ...
    tuner_dict = dict(type="AutoTVMTuner", target=target, log_file=log_file, n_trial=1, tuner=dict(type="XGBTuner"))
    from_onnx(onnx_file, lib_path, use_vm=True, bytecode_file=bytecode_path, shape=shape, dtype=dtype, tuner=tuner_dict)
    assert osp.exists(lib_path)
    ...
    tuner_dict = dict(type="AutoScheduleTuner", target=target, log_file=log_file, num_measure_trials=2)
    from_onnx(onnx_file, lib_path, use_vm=True, bytecode_file=bytecode_path, shape=shape, dtype=dtype, tuner=tuner_dict)
    assert osp.exists(lib_path)
    assert osp.exists(bytecode_path)
