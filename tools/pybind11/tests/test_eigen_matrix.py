...
def test_mutator_descriptors():
    ...
    assert "(arg0: numpy.ndarray[numpy.float32[5, 6]," " flags.writeable, flags.c_contiguous]) -> None" in str(
        excinfo.value
    )
    with pytest.raises(TypeError) as excinfo:
        m.fixed_mutator_c(zr)
    assert "(arg0: numpy.ndarray[numpy.float32[5, 6]," " flags.writeable, flags.f_contiguous]) -> None" in str(
        excinfo.value
    )
    with pytest.raises(TypeError) as excinfo:
