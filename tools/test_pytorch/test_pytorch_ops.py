...
def prepare_symbolics_ncnn():
    context = RewriterContext(Config({"backend_config": {"type": "ncnn"}}), "ncnn", opset=11)
    context.enter()
...
def test_adaptive_pool_2d_ncnn():
    x = torch.rand(2, 2, 2)
    model = OpModel(torch.nn.functional.adaptive_avg_pool2d, torch.tensor([2, 2], dtype=torch.int64)).eval()
    nodes = get_model_onnx_nodes(model, x)
...
def test_instance_norm():
    ...
    model = OpModel(torch.group_norm, 1, torch.rand([2]), torch.rand([2]), 1e-05).eval()
    nodes = get_model_onnx_nodes(model, x)
