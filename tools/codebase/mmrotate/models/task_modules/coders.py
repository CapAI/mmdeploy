...
@FUNCTION_REWRITER.register_rewriter("mmrotate.models.task_modules.coders.gliding_vertex_coder" ".GVFixCoder.decode")
def gvfixcoder__decode(self, hboxes, fix_deltas):
    """Rewriter for GVFixCoder decode, support more dimension input."""

    assert hboxes.size(-1) == 4, f"expect hboxes.size(-1)==4 get {hboxes.size(-1)}."
    ...
    polys = torch.stack([pred_t_x, y1, x2, pred_r_y, pred_d_x, y2, x1, pred_l_y], dim=-1)
