...
@FUNCTION_REWRITER.register_rewriter("mmrotate.models.roi_heads.gv_ratio_roi_head" ".GVRatioRoIHead.predict_bbox")
def gv_ratio_roi_head__predict_bbox(
    self,
    x: Tuple[Tensor],
    ...
    fix_preds = fix_preds.reshape(batch_size, num_proposals_per_img, fix_preds.size(-1))
    ratio_preds = ratio_preds.reshape(batch_size, num_proposals_per_img, ratio_preds.size(-1))
