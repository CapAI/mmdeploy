...
def _get_dataset_metainfo(model_cfg: mmengine.Config):
    ...
    for dataloader_name in ["test_dataloader", "val_dataloader", "train_dataloader"]:
        ...
        if dataset_mmpose is None:
            continue
        if hasattr(dataset_mmpose, "_load_metainfo") and isinstance(dataset_mmpose._load_metainfo, Callable):
...
class PoseDetection(BaseTask):
    ...
    def create_input(
        ...
        **kwargs,
        ...
        test_pipeline = [TRANSFORMS.build(c) for c in cfg.test_dataloader.dataset.pipeline]
        ...
        for i in range(len(imgs)):
            for bbox in bboxes[i]:
                # prepare data
                bbox_score = np.array([bbox[4] if len(bbox) == 5 else 1])  # shape (1,)
                data = {
                    "img": imgs[i],
                    "bbox_score": bbox_score,
                    "bbox": []
                    if hasattr(cfg.model, "bbox_head") and cfg.model.bbox_head.type == "YOLOXPoseHead"
                    else bbox[None],
                    "img_path": img_path[i],
                }
    ...
    def visualize(
        ...
        **kwargs,
        ...
        visualizer.add_datasample(
            name, image, data_sample=result, draw_gt=False, draw_bbox=draw_bbox, show=show_result, out_file=output_file
        )
