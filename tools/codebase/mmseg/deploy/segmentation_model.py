...
from mmdeploy.utils import Backend, get_backend, get_codebase_config, get_root_logger, load_config
...
class End2EndModel(BaseBackendModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(End2EndModel, self).__init__(deploy_cfg=deploy_cfg, data_preprocessor=data_preprocessor)
        self.deploy_cfg = deploy_cfg
        self.device = device
        self._init_wrapper(backend=backend, backend_files=backend_files, device=device, **kwargs)
    ...
    def forward(self, inputs: torch.Tensor, data_samples: List[BaseDataElement], mode: str = "predict", **kwargs):
        ...
        """
        assert mode == "predict", "Backend model only support mode==predict," f" but get {mode}"
        if inputs.device != torch.device(self.device):
            get_root_logger().warning(f"expect input device {self.device}" f" but get {inputs.device}.")
        inputs = inputs.to(self.device)
        batch_outputs = self.wrapper({self.input_name: inputs})[self.output_names[0]]
        return self.pack_result(batch_outputs, data_samples)

    def pack_result(self, batch_outputs: torch.Tensor, data_samples: List[BaseDataElement]):
        ...
        for seg_pred, data_sample in zip(batch_outputs, data_samples):
            ...
            if get_codebase_config(self.deploy_cfg).get("with_argmax", True) is False:
            ...
            if metainfo["ori_shape"] != metainfo["img_shape"]:
                ...
                seg_pred = (
                    resize(seg_pred.unsqueeze(0).to(torch.float32), size=metainfo["ori_shape"], mode="nearest")
                    .squeeze(0)
                    .to(ori_type)
                )
...
class VACCModel(End2EndModel):
    ...
    def forward(
        ...
        batch_outputs = [output.argmax(dim=1, keepdim=True) for output in batch_outputs.values()]
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(
        ...
        outputs = self.wrapper.invoke(inputs.contiguous().detach().cpu().numpy())
        batch_outputs = torch.from_numpy(outputs).to(torch.int64).to(self.device)
