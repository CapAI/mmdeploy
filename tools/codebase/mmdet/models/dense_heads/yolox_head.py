...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.dense_heads.yolox_head." "YOLOXHead.predict_by_feat")
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.yolox_head." "YOLOXHead.predict_by_feat", backend=Backend.NCNN.value
)
def yolox_head__predict_by_feat__ncnn(
    ...
    flatten_bbox_preds = [bbox_pred.permute(0, 2, 3, 1).reshape(batch_size, -1, 4) for bbox_pred in bbox_preds]
    flatten_objectness = [objectness.permute(0, 2, 3, 1).reshape(batch_size, -1, 1) for objectness in objectnesses]

    cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid()
    dummy_cls_scores = torch.zeros(batch_size, cls_scores.shape[-2], 1, device=cls_scores.device)
    ...
    prior_box_x1 = (flatten_priors[:, :, 0:1] - flatten_priors[:, :, 2:3] / 2) / img_width
    prior_box_y1 = (flatten_priors[:, :, 1:2] - flatten_priors[:, :, 3:4] / 2) / img_height
    prior_box_x2 = (flatten_priors[:, :, 0:1] + flatten_priors[:, :, 2:3] / 2) / img_width
    prior_box_y2 = (flatten_priors[:, :, 1:2] + flatten_priors[:, :, 3:4] / 2) / img_height
    prior_box_ncnn = torch.cat([prior_box_x1, prior_box_y1, prior_box_x2, prior_box_y2], dim=2)

    scores = batch_mlvl_scores.permute(0, 2, 1).unsqueeze(3) * score_factor.permute(0, 2, 1).unsqueeze(3)
