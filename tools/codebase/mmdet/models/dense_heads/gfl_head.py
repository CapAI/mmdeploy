...
from mmdeploy.codebase.mmdet.deploy import gather_topk, get_post_processing_params, pad_with_value
from mmdeploy.core import FUNCTION_REWRITER
from mmdeploy.mmcv.ops import multiclass_nms
from mmdeploy.utils import Backend, get_backend, is_dynamic_shape


@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.dense_heads.gfl_head." "GFLHead.predict_by_feat")
def gfl_head__predict_by_feat(
    ...
    for cls_score, bbox_pred, score_factors, priors, stride in zip(
        ...
        def _batched_integral(intergral, x):
            ...
            x = F.linear(x, intergral.project.type_as(x).unsqueeze(0)).reshape(batch_size, -1, 4)
            return x

        bbox_pred = _batched_integral(self.integral, bbox_pred.permute(0, 2, 3, 1)) * stride[0]
