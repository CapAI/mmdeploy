...
from mmdet.models.task_modules.coders import DeltaXYWHBBoxCoder, DistancePointBBoxCoder, TBLRBBoxCoder
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.base_dense_head." "BaseDenseHead.predict_by_feat"
)
    ...
    **kwargs,
    ...
    for cls_score, bbox_pred, score_factors, priors in zip(
        ...
        if pre_topk > 0:
            ...
            if with_score_factors:
                score_factors = pad_with_value_if_necessary(score_factors, 1, pre_topk, 0.0)
            ...
            bbox_pred, scores, score_factors = gather_topk(
                bbox_pred, scores, score_factors, inds=topk_inds, batch_size=batch_size, is_batched=True
            )
            priors = gather_topk(priors, inds=topk_inds, batch_size=batch_size, is_batched=False)
    ...
    batch_bboxes = self.bbox_coder.decode(batch_priors, batch_mlvl_bboxes_pred, max_shape=img_shape)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.base_dense_head." "BaseDenseHead.predict_by_feat", backend=Backend.RKNN.value
)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.base_dense_head." "BaseDenseHead.predict_by_feat", backend=Backend.NCNN.value
)
    ...
    **kwargs,
    ...
    if isinstance(self.bbox_coder, DeltaXYWHBBoxCoder):
        ...
        if isinstance(normalizer, float):
            ...
            vars = torch.tensor([normalizer[0], normalizer[2], 1, 1], dtype=torch.float32)
    ...
    mlvl_priors = self.prior_generator.grid_priors(featmap_sizes, device=cls_scores[0].device)
    batch_mlvl_priors = []
    ...
    for level_idx, (cls_score, bbox_pred, score_factor, priors) in enumerate(
        ...
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(batch_size, -1, 4)
        if with_score_factors:
            score_factor = score_factor.permute(0, 2, 3, 1).reshape(batch_size, -1, 1).sigmoid()
        ...
        if self.use_sigmoid_cls:
            ...
            dummy_background_score = torch.zeros(batch_size, cls_score.shape[1], 1, device=cls_score.device)
    ...
    batch_mlvl_scores = torch.cat(
        [batch_mlvl_scores[:, :, self.num_classes :], batch_mlvl_scores[:, :, 0 : self.num_classes]], dim=2
    )
    if isinstance(self.bbox_coder, TBLRBBoxCoder):
        ...
        batch_mlvl_bboxes = torch.cat([bboxes_x0, bboxes_y0, bboxes_x1, bboxes_y1], dim=2)
    ...
    if with_score_factors:
        ...
        batch_mlvl_scores = batch_mlvl_scores.permute(0, 2, 1).unsqueeze(3) * batch_mlvl_score_factors.permute(
            0, 2, 1
        ).unsqueeze(3)
...
def _tblr_pred_to_delta_xywh_pred(bbox_pred: torch.Tensor, normalizer: torch.Tensor) -> torch.Tensor:
