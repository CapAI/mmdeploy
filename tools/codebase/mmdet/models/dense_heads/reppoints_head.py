...
def _bbox_pre_decode(points: torch.Tensor, bbox_pred: torch.Tensor, stride: torch.Tensor):
    """compute real bboxes."""
    points = points[..., :2]
...
@FUNCTION_REWRITER.register_rewriter("mmdet.models.dense_heads.reppoints_head.RepPointsHead.points2bbox")
...
@FUNCTION_REWRITER.register_rewriter("mmdet.models.dense_heads.reppoints_head.RepPointsHead.predict_by_feat")
def reppoints_head__predict_by_feat(
    ...
    for level_idx, (cls_score, bbox_pred, priors) in enumerate(zip(mlvl_cls_scores, mlvl_bbox_preds, mlvl_priors)):
        ...
        if pre_topk > 0:
            ...
            bbox_pred, scores = gather_topk(bbox_pred, scores, inds=topk_inds, batch_size=batch_size, is_batched=True)
