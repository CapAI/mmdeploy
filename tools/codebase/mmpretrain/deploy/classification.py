...
def process_model_config(model_cfg: Config, imgs: Union[str, np.ndarray], input_shape: Optional[Sequence[int]] = None):
...
class Classification(BaseTask):
    ...
    def build_backend_model(
        self, model_files: Sequence[str] = None, data_preprocessor_updater: Optional[Callable] = None, **kwargs
    ) -> torch.nn.Module:
    ...
    def create_input(
        ...
        if not isinstance(imgs, (list, tuple)):
            imgs = [imgs]
        assert "test_pipeline" in self.model_cfg, f"test_pipeline not found in {self.model_cfg}."
    ...
    def visualize(
        ...
        **kwargs,
        ...
        visualizer.visualize_cls(image, result, out_file=output_file, show=show_result, name=name)
    ...
    def get_preprocess(self, *args, **kwargs) -> Dict:
        ...
        transforms = [item for item in pipeline if "Random" not in item["type"]]
        move_pipeline = []
        import re
        ...
        for i, transform in enumerate(transforms):
            ...
            if transform["type"] == "ResizeEdge":
                transforms[i] = dict(type="Resize", keep_ratio=True, size=(transform["scale"], -1))
    ...
    def get_postprocess(self, *args, **kwargs) -> Dict:
        ...
        if "topk" not in postprocess:
            topk = (1,)
            logger = get_root_logger()
            logger.warning("no topk in postprocess config, using default " "topk value.")
        else:
