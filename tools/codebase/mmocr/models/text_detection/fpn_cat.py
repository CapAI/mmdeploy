...
@FUNCTION_REWRITER.register_rewriter(func_name="mmocr.models.textdet.FPNC.forward", backend="tensorrt")
def fpnc__forward__tensorrt(self, inputs, **kwargs):
    ...
    for i in range(used_backbone_levels - 1, 0, -1):
        ...
        laterals[i - 1] += F.interpolate(laterals[i], size=prev_shape, mode=mode)
    # build outputs
