...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmocr.models.textrecog.decoders.ABILanguageDecoder._get_length", IR=IR.ONNX
)
def abi_language_decoder___get_length__default(self, logit: torch.Tensor, dim: int = -1, **kwargs) -> torch.Tensor:
    ...
    out = out + 1
    out = torch.where(abn, out, out.new_tensor(logit.shape[1]).to(out.device)).float()
    return out
