...
class TestEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        from mmdeploy.codebase.mmpretrain.deploy.classification_model import End2EndModel

        cls.end2end_model = End2EndModel(Backend.ONNXRUNTIME, [""], device="cpu", deploy_cfg=deploy_cfg)
    ...
    def test_forward(self):
        ...
        data_sample = DataSample(
            metainfo=dict(scale_factor=(1, 1), ori_shape=(IMAGE_SIZE, IMAGE_SIZE), img_shape=(IMAGE_SIZE, IMAGE_SIZE))
        )
        results = self.end2end_model.forward(imgs, [data_sample], mode="predict")
...
class TestRKNNEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        cls.wrapper.set(outputs=cls.outputs)
        deploy_cfg = Config({"onnx_config": {"output_names": ["outputs"]}, "backend_config": {"common_config": {}}})

        from mmdeploy.codebase.mmpretrain.deploy.classification_model import RKNNEnd2EndModel

        class_names = ["" for i in range(NUM_CLASS)]
        cls.end2end_model = RKNNEnd2EndModel(
            Backend.RKNN, [""], device="cpu", class_names=class_names, deploy_cfg=deploy_cfg
        )
...
def test_build_classification_model():
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        from mmdeploy.codebase.mmpretrain.deploy.classification_model import End2EndModel, build_classification_model

        classifier = build_classification_model([""], model_cfg, deploy_cfg, "cpu")
