...
def test_encoderdecoder_predict(backend):
    ...
    rewrite_outputs = segmentor.postprocess_result(rewrite_outputs[0], data_samples)
    rewrite_outputs = rewrite_outputs[0].pred_sem_seg.data
    assert torch.allclose(model_outputs, rewrite_outputs)


@pytest.mark.parametrize("backend", [Backend.ONNXRUNTIME])
@pytest.mark.parametrize("with_argmax,use_sigmoid", [(True, False), (False, True)])
def test_basesegmentor_forward(backend: Backend, with_argmax: bool, use_sigmoid: bool):
    check_backend(backend)
    ...
    task_processor = generate_mmseg_task_processor(deploy_cfg=deploy_cfg, model_cfg=model_cfg)
    segmentor = task_processor.build_pytorch_model()
    size = 256
    inputs = torch.randn(1, 3, size, size)
    data_samples = [generate_datasample(size, size)]
    wrapped_model = WrapModel(segmentor, "forward", data_samples=data_samples, mode="predict")
    model_outputs = wrapped_model(inputs)[0].pred_sem_seg.data
    rewrite_inputs = {
        "inputs": inputs,
    }
...
def test_emamodule_forward(backend):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            ...
            onnx_config=dict(output_names=["result"], input_shape=(1, 8, 16, 16)),
    ...
    if is_backend_output:
        rewrite_outputs = rewrite_outputs[0]
    rewrite_outputs = rewrite_outputs.to(model_outputs).reshape(model_outputs.shape)
    assert torch.allclose(rewrite_outputs, model_outputs, rtol=1e-03, atol=1e-05)
...
def test_upconvblock_forward(backend, is_dynamic_shape):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            backend_config=dict(type=backend.value),
            onnx_config=dict(input_names=["x", "skip"], output_names=["output"], dynamic_axes=dynamic_axes),
            codebase_config=dict(type=Codebase.MMSEG.value, task=Task.SEGMENTATION.value),
        )
    )
