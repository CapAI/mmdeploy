...
from mmdeploy.utils.test import WrapModel, check_backend, get_model_outputs, get_rewrite_outputs

try:
    import_codebase(Codebase.MMROTATE)
except ImportError:
    pytest.skip(f"{Codebase.MMROTATE} is not installed.", allow_module_level=True)
...
def get_anchor_head_model():
    """AnchorHead Config."""
    test_cfg = Config(dict(nms_pre=2000, min_bbox_size=0, score_thr=0.05, nms=dict(iou_thr=0.1), max_per_img=2000))
...
def get_single_roi_extractor():
    ...
    model = RotatedSingleRoIExtractor(roi_layer, out_channels, featmap_strides).eval()
...
def test_rotated_single_roi_extractor(backend_type: Backend):
    ...
    rois = torch.tensor([[0.0000, 587.8285, 52.1405, 886.2484, 341.5644, 0.0000]])
...
def get_oriented_rpn_head_model():
    ...
    test_cfg = Config(
        dict(nms_pre=2000, min_bbox_size=0, score_thr=0.05, nms=dict(type="nms", iou_thr=0.1), max_per_img=2000)
    )
...
def test_oriented_rpn_head__predict_by_feat(backend_type: Backend):
    ...
    deploy_cfg = Config(
        dict(
            ...
            codebase_config=dict(
                type="mmrotate",
                task="RotatedDetection",
                post_processing=dict(score_threshold=0.05, iou_threshold=0.1, pre_top_k=2000, keep_top_k=2000),
            ),
        )
    )
    ...
    wrapped_model = WrapModel(head, "predict_by_feat", batch_img_metas=img_metas, with_nms=True)
    rewrite_inputs = {
...
def test_gv_ratio_roi_head__predict_bbox(backend_type: Backend):
    ...
    head = GVRatioRoIHead(
        ...
        bbox_head=dict(
            ...
            ratio_thr=0.8,
            bbox_coder=dict(
                type="DeltaXYWHQBBoxCoder", target_means=(0.0, 0.0, 0.0, 0.0), target_stds=(0.1, 0.1, 0.2, 0.2)
            ),
            ...
            loss_fix=dict(type="mmdet.SmoothL1Loss", beta=1.0 / 3.0, loss_weight=1.0),
            loss_ratio=dict(type="mmdet.SmoothL1Loss", beta=1.0 / 3.0, loss_weight=16.0),
        ),
    )
    head.cpu().eval()
    ...
    bboxes = torch.cat([bboxes, bboxes + torch.rand(1, 100, 2) + torch.rand(1, 100, 1)], dim=-1)
    proposals = [bboxes, torch.randint(0, 10, (1, 100))]
    img_metas = [{"img_shape": torch.tensor([224, 224])}]

    wrapped_model = WrapModel(
        head, "predict_bbox", rpn_results_list=proposals, batch_img_metas=img_metas, rcnn_test_cfg=test_cfg["rcnn"]
    )
    rewrite_inputs = {"x": x}
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg
...
def test_gvfixcoder__decode(backend_type: Backend):
    ...
    rewrite_outputs, _ = get_rewrite_outputs(
        wrapped_model,
        model_inputs={"hboxes": hboxes, "fix_deltas": fix_deltas},
...
def test_rotated_rtmdet_head_predict_by_feat(backend_type: Backend):
    ...
    seed_everything(1234)
    cls_scores = [torch.rand(1, rtm_r_head.num_classes, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(5678)
    bbox_preds = [torch.rand(1, 4, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)]
    seed_everything(9101)
    angle_preds = [
        torch.rand(1, rtm_r_head.angle_coder.encode_size, 2 * pow(2, i), 2 * pow(2, i)) for i in range(3, 0, -1)
    ]
    ...
    model_outputs = get_model_outputs(rtm_r_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(rtm_r_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True)
    rewrite_inputs = {
        "cls_scores": cls_scores,
        "bbox_preds": bbox_preds,
        "angle_preds": angle_preds,
    ...
    if is_backend_output:
        ...
        for i in range(len(model_outputs)):
            ...
            assert np.allclose(
                model_outputs[i].scores[:min_shape], rewrite_outputs[0][i, :min_shape, 5], rtol=1e-03, atol=1e-05
            )
            assert np.allclose(
