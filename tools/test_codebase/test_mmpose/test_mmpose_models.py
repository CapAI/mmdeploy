...
try:
    ...
    pytest.skip(f"{Codebase.MMPOSE} is not installed.", allow_module_level=True)
...
def get_heatmap_head():
    ...
    model = HeatmapHead(
        2, 4, deconv_out_channels=(16, 16, 16), loss=dict(type="KeypointMSELoss", use_target_weight=False)
    )
...
def get_msmu_head():
    ...
    model = MSPNHead(num_stages=1, num_units=1, out_shape=(32, 48), unit_channels=16, level_indices=[1])
    model.requires_grad_(False)
    return model
...
def test_yolox_pose_head(backend_type: Backend):
    ...
    deploy_cfg = mmengine.Config.fromfile("configs/mmpose/pose-detection_yolox-pose_onnxruntime_dynamic.py")
    check_backend(backend_type, True)
    ...
    head = YOLOXPoseHead(
        ...
        loss_bbox=dict(type="mmdet.IoULoss", mode="square", eps=1e-16, reduction="sum", loss_weight=5.0),
        loss_obj=dict(type="mmdet.CrossEntropyLoss", use_sigmoid=True, reduction="sum", loss_weight=1.0),
        loss_pose=dict(type="OksLoss", metainfo="configs/_base_/datasets/coco.py", loss_weight=30.0),
        ...
        train_cfg=ConfigDict(
            assigner=dict(
                ...
                oks_calculator=dict(type="OksLoss", metainfo="configs/_base_/datasets/coco.py"),
            )
        ),
        test_cfg=ConfigDict(
            yolox_style=True,
    ...
    class TestYOLOXPoseHeadModel(torch.nn.Module):
        ...
        def forward(self, x1, x2, x3):
            ...
            data_sample.set_metainfo(dict(ori_shape=(640, 640), scale_factor=(1.0, 1.0)))
            return self.yolox_pose_head.predict(inputs, batch_data_samples=[data_sample])

    model = TestYOLOXPoseHeadModel(head)
    model.cpu().eval()

    model_inputs = [torch.randn(1, 128, 8, 8), torch.randn(1, 128, 4, 4), torch.randn(1, 128, 2, 2)]
    ...
    wrapped_model = WrapModel(model, "forward")
    rewrite_inputs = {"x1": model_inputs[0], "x2": model_inputs[1], "x3": model_inputs[2]}
    deploy_cfg.onnx_config.input_names = ["x1", "x2", "x3"]

    rewrite_outputs, _ = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, run_with_backend=True, deploy_cfg=deploy_cfg
    )
