...
model_cfg.test_dataloader.dataset.data_root = "tests/test_codebase/test_mmdet/data"
model_cfg.test_dataloader.dataset.ann_file = "coco_sample.json"
model_cfg.test_evaluator.ann_file = "tests/test_codebase/test_mmdet/data/coco_sample.json"
deploy_cfg = Config(
    dict(
...
def test_build_pytorch_model(from_mmrazor: Any):
    ...
    if from_mmrazor is False:
        ...
        _model_cfg_path = "tests/test_codebase/test_mmdet/data/" "mmrazor_model.py"
        _model_cfg = load_config(_model_cfg_path)[0]
        _model_cfg.algorithm.architecture.model.type = "mmdet.YOLOV3"
        _model_cfg.algorithm.architecture.model.backbone.type = "mmpretrain.SearchableShuffleNetV2"
    ...
    if not isinstance(from_mmrazor, bool):
        with pytest.raises(
            TypeError, match="`from_mmrazor` attribute must be " "boolean type! " f"but got: {from_mmrazor}"
        ):
...
def backend_model():
    ...
    wrapper.set(outputs={"dets": torch.rand(1, 10, 5).sort(2).values, "labels": torch.randint(0, 10, (1, 10))})
...
def test_can_postprocess_masks():
    ...
    for num_det in num_dets:
        ...
        img_w, img_h = (30, 40)
        masks = End2EndModel.postprocessing_masks(det_bboxes, det_masks, img_w, img_h)
        expected_shape = (num_det, img_h, img_w)
        actual_shape = masks.shape
        assert actual_shape == expected_shape, (
            f"The expected shape of masks {expected_shape} " f"did not match actual shape {actual_shape}."
        )
...
def test_get_partition_cfg(partition_type):
    ...
    partition_cfg = task_processor.get_partition_cfg(partition_type=partition_type)
...
def test_build_dataset_and_dataloader():
    dataset = task_processor.build_dataset(dataset_cfg=model_cfg.test_dataloader.dataset)
