...
class TRTWrapper(BaseWrapper):
    ...
    def __init__(
        self, engine: Union[str, trt.ICudaEngine], output_names: Optional[Sequence[str]] = None, device_id: int = 0
    ):
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        profile_id = 0
        inputs = dict(
            (name, data.contiguous().int() if data.dtype == torch.long else data.contiguous())
