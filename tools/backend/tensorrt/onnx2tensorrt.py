...
from mmdeploy.utils import get_calib_filename, get_common_config, get_model_inputs, load_config, parse_device_id
...
def onnx2tensorrt(
    ...
    **kwargs,
    ...
    assert device.startswith(
        "cuda"
    ), f"TensorRT requires cuda device, \
        but given: {device}"

    device_id = parse_device_id(device)
    assert save_file.endswith(".engine"), "Expect save file ends with `.engine`." f" but get {save_file}"
