...
class ModelDesc:
    ...
    def __init__(self, model_id):
        ...
        for index in range(num_inputs):
            ...
            binding = Binding(index, dims["name"], dims["dims"], data_type, size)
        ...
        for index in range(num_outputs):
            ...
            self.outputs.append(Binding(index, dims["name"], dims["dims"], data_type, size))
...
class AscendWrapper(BaseWrapper):
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        with self._device():
            ...
            for binding in self._model_desc.inputs:
                tensor = inputs[binding.name].to(torch_device, dtype=binding.data_type).contiguous()
                self._copy_tensor_to_buffer(tensor, self._input.buffers[binding.index])

            outputs = {}
            for binding in self._model_desc.outputs:
                shape = output_shapes[binding.index]
                tensor = torch.empty(shape, dtype=binding.data_type, device=torch_device)
            ...
            for binding in self._model_desc.outputs:
                self._copy_buffer_to_tensor(self._output.buffers[binding.index], outputs[binding.name])
    ...
    def _copy_tensor_to_buffer(self, tensor: torch.Tensor, buffer: DataBuffer):
        if tensor.device.type == "cpu":
            ...
            ret = acl.rt.memcpy(
                buffer.data, buffer.size, tensor.data_ptr(), tensor.element_size() * tensor.numel(), kind
            )
            _check(ret, "acl.rt.memcpy")
        else:
            ret = acl.update_data_buffer(buffer.handle, tensor.data_ptr(), tensor.element_size() * tensor.numel())
            _check(ret, "acl.update_data_buffer")
    ...
    def _copy_buffer_to_tensor(self, buffer: DataBuffer, tensor: torch.Tensor):
        if tensor.device.type == "cpu":
            ...
            ret = acl.rt.memcpy(tensor.data_ptr(), size, buffer.data, size, kind)
            _check(ret, "acl.rt.memcpy")
    ...
    def _reshape_dynamic_batch_size(self, input_shapes: Sequence[Sequence[int]]):
        ...
        for src, ref in zip(input_shapes, self._model_desc.inputs):
            if ref.dims[0] == -1:
                if batch_size is None:
                    ...
                    raise RuntimeError(f"Inconsistent batch size {batch_size} vs {src[0]}")

        if batch_size is None:
            raise RuntimeError("Can't determine batch size")

        candidates = list(filter(lambda x: x >= batch_size, self._dynamic_batch_size))
        if not candidates:
            raise RuntimeError(f"Batch size {batch_size} is not supported." f" ({self._dynamic_batch_size})")

        ret = acl.mdl.set_dynamic_batch_size(
            self._model_id, self._input.handle, self._model_desc.dynamic_tensor.index, candidates[0]
        )
        _check(ret, "acl.mdl.set_dynamic_batch_size")

    def _reshape_dynamic_image_size(self, input_shapes: Sequence[Sequence[int]]):
        ...
        for src, ref in zip(input_shapes, self._model_desc.inputs):
            if -1 in ref.dims:
                ...
                if size is None:
                    ...
                    raise RuntimeError(f"Inconsistent image size {size} vs {tmp_size}")

        if size is None:
            raise RuntimeError("Can't determine dynamic HW")
        if not list(size) in self._dynamic_hw:
            raise RuntimeError(f"size {size} is not supported. ({self._dynamic_hw})")
        height, width = size
        ret = acl.mdl.set_dynamic_hw_size(
            self._model_id, self._input.handle, self._model_desc.dynamic_tensor.index, height, width
        )
    ...
    def _reshape_dynamic_dims(self, input_shapes: Sequence[Sequence[int]]):
        ...
        ret = acl.mdl.set_input_dynamic_dims(
            self._model_id, self._input.handle, self._model_desc.dynamic_tensor.index, self._dynamic_dims[index]
        )
    ...
    def _create_input_buffers(self):
        ...
        if self._model_desc.dynamic_tensor:
            self._input.add_buffer(DataBuffer(self._model_desc.dynamic_tensor.size))
    ...
    def __ascend_execute(self):
        ...
        ret = acl.mdl.execute(self._model_id, self._input.handle, self._output.handle)
