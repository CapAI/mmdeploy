...
class PPLNNWrapper(BaseWrapper):
    ...
    def __init__(
        self, onnx_file: str, algo_file: str, device: str, output_names: Optional[Sequence[str]] = None, **kwargs
    ):
        ...
        engines = register_engines(
            device_id, disable_avx512=False, quick_select=algo_file is None, import_algo_file=algo_file
        )
        ...
        self.inputs = {
            runtime.GetInputTensor(i).GetName(): runtime.GetInputTensor(i) for i in range(runtime.GetInputCount())
        }
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.runtime.GetOutputCount()):
            ...
            if out_tensor:
                ...
                out_shape = self.runtime.GetOutputTensor(i).GetShape().GetDims()
    ...
    def __pplnn_execute(self):
        ...
        assert status == pplcommon.RC_SUCCESS, "Run() failed: " + pplcommon.GetRetCodeStr(status)
