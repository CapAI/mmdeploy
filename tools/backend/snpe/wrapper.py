...
class SNPEWrapper(BaseWrapper):
    ...
    def __init__(self, dlc_file: str, uri: str, output_names: Optional[Sequence[str]] = None, **kwargs):
        ...
        if uri is None and get_env_key() in os.environ:
            logger.warn("snpe remote service URI not set, search from environment")
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for name, input_tensor in inputs.items():
            ...
            tensor = inference_pb2.Tensor(data=data.tobytes(), name=name, dtype="float32", shape=list(data.shape))

            snpe_inputs.append(tensor)

        return self.__snpe_execute(tensorList=inference_pb2.TensorList(data=snpe_inputs), device=device_type)

    @TimeCounter.count_time(Backend.SNPE.value)
    def __snpe_execute(self, tensorList: inference_pb2.TensorList, device: str) -> Dict[str, torch.tensor]:
