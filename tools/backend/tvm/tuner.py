...
class TVMTunerBase:
    ...
    def __init__(self, target: Union[str, Target], opt_level: int = 3, use_vm: bool = False) -> None:
    ...
    def build(self, mod: IRModule, params: Dict):
        ...
        with tvm.transform.PassContext(opt_level=self._opt_level):
            if self._use_vm:
                ...
                ret = relay.build_module.build(mod, target=self._target, params=params)
...
class AutoTVMTuner(TVMTunerBase):
    def __init__(
        ...
        builder: Union[Dict, Any] = dict(type="LocalBuilder", timeout=10),
        runner: Union[Dict, Any] = dict(type="LocalRunner", number=20, repeat=3, timeout=4, min_repeat_ms=150),
        ...
        self._measure_option = autotvm.measure_option(builder=builder, runner=runner)
    ...
    def tune(self, mod: IRModule, params: Dict):
        ...
        for i, task in enumerate(reversed(tasks)):
            ...
            if self._use_transfer_learning:
                if os.path.isfile(tmp_log_file) and os.path.exists(tmp_log_file):
...
class AutoScheduleTuner(TVMTunerBase):
    def __init__(
        ...
        runner: Union[Dict, Any] = dict(type="LocalRunner", repeat=10, enable_cpu_cache_flush=True),
    ) -> None:
        """The Ansor tuner.
        ...
        if isinstance(runner, Dict):
            ...
            if runner["type"] == "LocalRunner":
                ...
                self._measure_ctx = auto_scheduler.LocalRPCMeasureContext(**runner)
    ...
    def build(self, mod: IRModule, params: Dict):
        ...
        with auto_scheduler.ApplyHistoryBest(self._log_file):
            with tvm.transform.PassContext(
                opt_level=self._opt_level, config={"relay.backend.use_auto_scheduler": True}
            ):
                if self._use_vm:
                    ret = relay.vm.compile(mod, target=self._target, params=params)
                else:
