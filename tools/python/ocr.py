...
def parse_args():
    ...
    parser.add_argument(
        "--textdet",
        default="",
        help="path of mmdeploy text-detector SDK model dumped by" "model converter",
    )
    parser.add_argument(
        "--textrecog",
        default="",
        help="path of mmdeploy text-recognizer SDK model dumped by" "model converter",
    )
...
def main():
    ...
    if args.textdet:
        detector = TextDetector(model_path=args.textdet, device_name=args.device_name, device_id=0)
        bboxes = detector(img)
        ...
        print("do nothing since neither text detection sdk model or " "text recognition sdk model in input")
