...
@FUNCTION_REWRITER.register_rewriter("mmdeploy.core.optimizers.function_marker.Mark.symbolic")
...
@FUNCTION_REWRITER.register_rewriter("mmdeploy.core.optimizers.function_marker.Mark.forward")
def forward_of_mark(ctx, x, dtype, shape, func, func_id, type, name, id, attrs) -> torch.Tensor:
    ...
    if apply_marks and create_calib:
        ...
        for partition_id, partition_cfg in enumerate(partition_cfgs):
            ...
            if (f"{func}:{type}" not in start) and (f"{func}[{func_id}]:{type}" not in start):
                continue
            ...
            input_data_group.create_dataset(
                str(data_id), shape=x_np.shape, compression="gzip", compression_opts=4, data=x_np
            )

    return rewriter.origin_func(ctx, x, dtype, shape, func, func_id, type, name, id, attrs)


def mark_tensors(
    xs: Any, func: str, func_id: int, io_type: str, ctx: Any, attrs: Dict, is_inspecting: bool, level: int
) -> tuple:
...
@FUNCTION_REWRITER.register_rewriter("mmdeploy.core.optimizers.function_marker.mark_tensors", ir=IR.TORCHSCRIPT)
