...
def install_pplcv(dep_dir, build_cuda):
    ...
    if not os.path.exists(pplcv_dir):
        os.system("git clone --depth 1 --branch v0.7.0 https://github.com/openppl-public/ppl.cv/")  # noqa: E501

    # build
    os.chdir(pplcv_dir)
    if build_cuda is True:
        os.system("./build.sh cuda")
        pplcv_cmake_dir = os.path.join(pplcv_dir, "cuda-build/install/lib/cmake/ppl")
    else:
        os.system("./build.sh x86_64")
        pplcv_cmake_dir = os.path.join(pplcv_dir, "x86-64-build/install/lib/cmake/ppl")
...
def install_pplnn(dep_dir, build_cuda):
    ...
    if not os.path.exists(pplnn_dir):
        os.system("git clone --depth 1 --branch v0.8.2 https://github.com/openppl-public/ppl.nn/")  # noqa: E501

    # build
    os.chdir(pplnn_dir)
    if build_cuda is True:
        os.system("./build.sh -DPPLNN_USE_CUDA=ON -DPPLNN_USE_X86_64=ON  -DPPLNN_ENABLE_PYTHON_API=ON")  # noqa: E501
    else:
        os.system("./build.sh -DPPLNN_USE_X86_64=ON  -DPPLNN_ENABLE_PYTHON_API=ON")  # noqa: E501

    os.system("cd python/package && ./build.sh")
    os.system("cd /tmp/pyppl-package/dist && python3 -m pip install pyppl*.whl --force-reinstall --user")  # noqa: E501

    pplnn_cmake_dir = os.path.join(pplnn_dir, "pplnn-build/install/lib/cmake/ppl")
    print("\n")
...
def main():
    ...
    if install_mmdeploy(work_dir, pplnn_cmake_dir, pplcv_cmake_dir, build_cuda) != 0:
        return -1
