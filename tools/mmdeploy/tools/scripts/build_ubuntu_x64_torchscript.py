...
from ubuntu_utils import cmd_result, cu_version_name, ensure_base_env, get_job, pytorch_version
...
def install_libtorch(dep_dir):
    ...
    cuda = cmd_result(" nvcc --version | grep  release | awk '{print $5}' | awk -F , '{print $1}' ")  # noqa: E501
    ...
    filename = "libtorch-shared-with-deps-{}%2B{}.zip".format(torch_version, version_name)
    url = "https://download.pytorch.org/libtorch/{}/{}".format(version_name, filename)
    os.system("wget -q --show-progress {} -O libtorch.zip".format(url))
    os.system("unzip libtorch.zip")
    if not os.path.exists(unzipped_name):
        print(
            "download or unzip libtorch from {} failed, please check https://pytorch.org/get-started/locally/".format(  # noqa: E501
                url
            )
        )
        return None
