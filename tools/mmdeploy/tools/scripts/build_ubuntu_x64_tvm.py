...
def install_llvm(dep_dir):
    ...
    nickname_dict = {"18.04": "bionic", "20.04": "focal", "22.04": "jammy", "22.10": "kinetic"}
    ...
    os.system("sudo apt-get install llvm-10 lldb-10 llvm-10-dev libllvm10 llvm-10-runtime")  # noqa: E501
...
def install_tvm(dep_dir):
    ...
    if not osp.exists("tvm"):
        os.system("git clone --branch v0.10.0 --depth 1 --recursive https://github.com/apache/tvm tvm")  # noqa: E501
    ...
    os.system(""" sed -i "s@set(USE_LLVM OFF)@set(USE_LLVM /usr/bin/llvm-config-10)@g" config.cmake """)  # noqa: E501

    os.system("cmake .. && make -j {} && make runtime".format(g_jobs))

    # set env
    os.system(
        """ echo 'export LD_LIBRARY_PATH={}:$LD_LIBRARY_PATH' >> ~/mmdeploy.env """.format(  # noqa: E501
            os.path.join(tvm_dir, "build")
        )
    )
    ...
    # install dependency
    os.system(""" python3 -m pip install xgboost decorator psutil scipy attrs tornado """)  # noqa: E501
...
def install_mmdeploy(work_dir, tvm_dir):
    ...
    os.system(
        """ echo 'export PATH={}:$PATH' >> ~/mmdeploy.env """.format(
