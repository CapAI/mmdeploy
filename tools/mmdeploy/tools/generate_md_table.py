...
def parse_args():
    parser = argparse.ArgumentParser(description="from yaml export markdown table")
...
def main():
    ...
    with open(args.yml_file, "r") as reader, open(args.output, "w") as writer:
        ...
        for i in range(len(config.models)):
            ...
            deploy_config = [pipelines[i].deploy_config for i in range(len(pipelines))]
            cfg = [load_config(deploy_config[i]) for i in range(len(deploy_config))]
            task = [get_task_type(cfg[i][0]).value for i in range(len(deploy_config))]
            backend_type = [get_backend(cfg[i][0]).value for i in range(len(deploy_config))]
