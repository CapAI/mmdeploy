...
def parse_args():
    ...
    parser.add_argument("output_prefix", help="output PPLNN algorithm prefix in json format")
    parser.add_argument("--device", help="`the device of model during conversion", default="cuda:0")
    parser.add_argument(
        "--opt-shapes",
...
def main():
    ...
    assert isinstance(input_shapes, collections.Sequence), "The opt-shape must be a sequence."
    assert isinstance(input_shapes[0], int) or (
        isinstance(input_shapes[0], collections.Sequence)
