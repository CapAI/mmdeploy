...
def parse_args():
    ...
    parser.add_argument("img", nargs="+", help="image used to convert model model")
    parser.add_argument("--test-img", default=None, type=str, nargs="+", help="image used to test model")
    ...
    parser.add_argument("--dump-info", action="store_true", help="Output information for SDK")
    parser.add_argument("--quant-image-dir", default=None, help="Image directory for quantize model.")
    parser.add_argument("--quant", action="store_true", help="Quantize model to low bit.")
    parser.add_argument(
        "--uri", default="192.168.1.1:60000", help="Remote ipv4:port or ipv6:port for inference on edge device."
    )
...
def main():
    ...
    pipeline_funcs = [torch2onnx, torch2torchscript, extract_model, create_calib_input_data]
    ...
    if args.dump_info:
        export2SDK(deploy_cfg, model_cfg, args.work_dir, pth=checkpoint_path, device=args.device)
    ...
    torch2ir(ir_type)(
        args.img, args.work_dir, ir_save_file, deploy_cfg_path, model_cfg_path, checkpoint_path, device=args.device
    )
    ...
    if backend == Backend.RKNN:
        ...
        from mmdeploy.utils import get_common_config, get_normalization, get_quantization_config, get_rknn_quantization
        ...
        if get_rknn_quantization(deploy_cfg) is True:
            ...
            common_params.update(dict(mean_values=[transform["mean"]], std_values=[transform["std"]]))
        ...
        for onnx_path, model_input in zip(ir_files, model_inputs):
            ...
            if quant_mode == "int8":
                create_process(
                    "vacc quant dataset",
                    target=get_quant,
                    args=(deploy_cfg, model_cfg, shape_dict, checkpoint_path, args.work_dir, args.device),
                    kwargs=dict(),
                    ret_value=ret_value,
                )
    ...
    if backend == Backend.NCNN and quant:
        ...
        backend_files = []
        for onnx_path, model_param_path, model_bin_path in zip(ir_files, model_param_paths, model_bin_paths):
            deploy_cfg, model_cfg = load_config(deploy_cfg_path, model_cfg_path)
            ...
            create_process(
                "ncnn quant table",
                target=get_table,
                args=(onnx_path, deploy_cfg, model_cfg, quant_onnx, quant_table, quant_image_dir, args.device),
                kwargs=dict(),
                ret_value=ret_value,
            )
            ...
            create_process(
                "ncnn_int8",
                target=ncnn2int8,
                args=(model_param_path, model_bin_path, quant_table, quant_param, quant_bin),
                kwargs=dict(),
                ret_value=ret_value,
            )
    ...
    extra = dict(
        backend=backend, output_file=osp.join(args.work_dir, f"output_{backend.value}.jpg"), show_result=args.show
    )
    ...
    create_process(
        ...
        kwargs=dict(
            backend=Backend.PYTORCH, output_file=osp.join(args.work_dir, "output_pytorch.jpg"), show_result=args.show
        ),
        ret_value=ret_value,
    )
