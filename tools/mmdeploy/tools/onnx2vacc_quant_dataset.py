...
def process_model_config(model_cfg: Config, input_shape: Optional[Sequence[int]] = None):
...
def get_quant(
    ...
    model_cfg = process_model_config(model_cfg, (model_shape[3], model_shape[2]))
    ...
    with h5py.File(output_quant_dataset_path, mode="w") as file:
        ...
        for data_id, input_data in enumerate(tqdm.tqdm(dataloader)):
            ...
            assert (
                model_shape[2] >= calib_data_shape[2] and model_shape[3] >= calib_data_shape[3]
            ), f"vacc backend model shape is {tuple(model_shape[2:])}, \
                        the calib_data shape {calib_data_shape[2:]} is bigger"

            input_data_group.create_dataset(
                str(data_id), shape=input_data.shape, compression="gzip", compression_opts=4, data=input_data
            )


def parse_args():
    parser = argparse.ArgumentParser(description="Generate vacc quant dataset from ONNX.")
