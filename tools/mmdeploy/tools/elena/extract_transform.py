...
from mmdeploy.backend.sdk.export_info import get_preprocess, get_transform_static
...
CODEBASE = ["mmpretrain", "mmdetection", "mmpose", "mmrotate", "mmocr", "mmsegmentation", "mmediting"]
...
def parse_args():
    ...
    parser.add_argument("root_path", help="parent path to codebase(mmdetection for example)")
...
def generate_source_code(preprocess, transform_static, tag, args):
    kernel_base_dir = osp.join(MMDEPLOY_PATH, "csrc", "mmdeploy", "preprocess", "elena")
    ...
    mmengine.dump(transform_static, static_json_path, sort_keys=False, indent=4)
...
def main():
    ...
    elena_path = osp.abspath(
        os.path.join(MMDEPLOY_PATH, "third_party", "CVFusion", "build", "examples", "MMDeploy", "OpFuse")
    )
