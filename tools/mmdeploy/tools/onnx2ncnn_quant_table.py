...
def get_table(
    ...
    if image_dir is not None:
        ...
        dataset = QuantizationImageDataset(path=image_dir, deploy_cfg=deploy_cfg, model_cfg=model_cfg)
    ...
    for _, input_data in enumerate(dataloader):
        ...
        collate_fn = lambda x: data_preprocessor(x)["inputs"].to(device)  # noqa: E731
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate ncnn quant table from ONNX.")
    ...
    parser.add_argument("--image-dir", type=str, default=None, help="Calibration Image Directory.")
...
def main():
    ...
    get_table(onnx_path, deploy_cfg, model_cfg, quant_onnx_path, quant_table_path, image_dir)
