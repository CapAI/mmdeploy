...
class TestEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        deploy_cfg = mmengine.Config({"onnx_config": {"output_names": ["outputs"]}})
    ...
    @pytest.mark.parametrize("ori_shape", [[IMAGE_SIZE, IMAGE_SIZE], [2 * IMAGE_SIZE, 2 * IMAGE_SIZE]])
...
def test_build_text_detection_model():
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        from mmdeploy.codebase.mmocr.deploy.text_detection_model import End2EndModel, build_text_detection_model

        segmentor = build_text_detection_model([""], model_cfg, deploy_cfg, "cpu")
