...
class TestRKNNModel:
    ...
    def setup_class(cls):
        ...
        deploy_cfg = mmengine.Config(
            {"onnx_config": {"output_names": ["outputs"]}, "backend_config": {"common_config": {}}}
        )
        ...
        cls.rknn_model = RKNNModel(
            Backend.RKNN, [""], device="cpu", class_names=class_names, palette=palette, deploy_cfg=deploy_cfg
        )
...
def test_build_segmentation_model():
    model_cfg = mmengine.Config(dict(data=dict(test={"type": "CityscapesDataset"})))
    deploy_cfg = generate_mmseg_deploy_config()
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        from mmdeploy.codebase.mmseg.deploy.segmentation_model import End2EndModel, build_segmentation_model

        segmentor = build_segmentation_model([""], model_cfg, deploy_cfg, "cpu")
