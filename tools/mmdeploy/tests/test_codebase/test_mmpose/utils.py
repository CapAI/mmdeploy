...
def generate_datasample(img_size, heatmap_size=(64, 48)):
    ...
    h, w = img_size[:2]
    metainfo = dict(img_shape=(h, w, 3), crop_size=(h, w), input_size=(h, w), heatmap_size=heatmap_size)
...
def generate_mmpose_deploy_config(backend=Backend.ONNXRUNTIME.value, cfg_options=None):
    deploy_cfg = mmengine.Config(
        dict(
            backend_config=dict(type=backend),
            codebase_config=dict(type=Codebase.MMPOSE.value, task=Task.POSE_DETECTION.value),
            onnx_config=dict(
                type=IR.ONNX.value,
                export_params=True,
