...
def assert_forward_results(results, module_name: str = "model"):
    ...
    assert (
        results[0].pred_instances.scores.shape[0]
...
class TestEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        cls.outputs = {"dets": torch.rand(1, 10, 5), "labels": torch.rand(1, 10)}
        cls.wrapper.set(outputs=cls.outputs)
        deploy_cfg = Config({"onnx_config": {"output_names": ["dets", "labels"]}})

        from mmdeploy.codebase.mmdet.deploy.object_detection_model import End2EndModel

        cls.end2end_model = End2EndModel(Backend.ONNXRUNTIME, [""], "cpu", deploy_cfg)
    ...
    def test_forward(self):
        ...
        img_metas = [BaseDataElement(metainfo={"img_shape": [64, 64], "scale_factor": [1, 1]})]
...
class TestMaskEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        deploy_cfg = Config(
            {
                "onnx_config": {"output_names": ["dets", "labels", "masks"]},
                "codebase_config": {"post_processing": {"export_postprocess_mask": False}},
            }
        )
    ...
    def test_forward(self):
        imgs = torch.rand(1, 3, 64, 64)
        img_metas = [BaseDataElement(metainfo={"img_shape": [64, 64], "ori_shape": [32, 32], "scale_factor": [1, 1]})]
        results = self.end2end_model.forward(imgs, img_metas)
...
def prepare_model_deploy_cfgs():
    ...
    model_cfg = Config(
        dict(model=dict(neck=dict(num_outs=0), test_cfg=dict(rpn=test_cfg, rcnn=test_cfg), roi_head=roi_head))
    )
    deploy_cfg = Config(dict(codebase_config=dict(post_processing=post_processing)))
...
def test_build_object_detection_model(partition_type):
    ...
    deploy_cfg = Config(
        dict(
            ...
            codebase_config=dict(type="mmdet", post_processing=post_processing),
        )
    )
    if partition_type:
        deploy_cfg.partition_config = dict(apply_marks=True, type=partition_type, partition_cfg=[dict(output_names=[])])
    ...
    with SwitchBackendWrapper(ORTWrapper) as wrapper:
        ...
        from mmdeploy.codebase.mmdet.deploy.object_detection_model import build_object_detection_model

        detector = build_object_detection_model([""], model_cfg, deploy_cfg, "cpu")
...
class TestNCNNEnd2EndModel:
    ...
    def setup_class(cls):
        ...
        from mmdeploy.codebase.mmdet.deploy.object_detection_model import NCNNEnd2EndModel

        cls.ncnn_end2end_model = NCNNEnd2EndModel(Backend.NCNN, ["", ""], "cpu", model_cfg, deploy_cfg)
...
class TestRKNNModel:
    ...
    def setup_class(cls):
        ...
        cls.outputs = [torch.rand(1, 255, 5, 5), torch.rand(1, 255, 10, 10), torch.rand(1, 255, 20, 20)]
        cls.wrapper.set(outputs=cls.outputs)
        deploy_cfg = Config({"onnx_config": {"output_names": ["output"]}, "backend_config": {"common_config": {}}})
        model_cfg = Config(
            dict(
                model=dict(
                    bbox_head=dict(
        ...
        cls.rknn_model = RKNNModel(Backend.RKNN, ["", ""], "cpu", ["" for i in range(80)], model_cfg, deploy_cfg)
