...
def test_pad_with_value():
    ...
    assert np.allclose(padded_x.shape, torch.Size([3, 6]), rtol=1e-03, atol=1e-05)
    assert np.allclose(padded_x.sum(), x.sum(), rtol=1e-03, atol=1e-05)


def test_pad_with_value_if_necessary():
    x = torch.rand(3, 2)
    padded_x = pad_with_value_if_necessary(x, pad_dim=1, pad_size=4, pad_value=0)
    assert np.allclose(padded_x.shape, torch.Size([3, 2]), rtol=1e-03, atol=1e-05)
    assert np.allclose(padded_x.sum(), x.sum(), rtol=1e-03, atol=1e-05)
...
def test_get_topk_from_heatmap():
    ...
    from mmdeploy.codebase.mmdet.models.utils.gaussian_target import get_topk_from_heatmap__default
