...
try:
    ...
    pytest.skip(f"{Codebase.MMDET3D} is not installed.", allow_module_level=True)
model_cfg = load_config("tests/test_codebase/test_mmdet3d/data/model_cfg.py")[0]
...
def test_pillar_encoder(backend_type: Backend):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            ...
            onnx_config=dict(
                input_shape=None, input_names=["features", "num_points", "coors"], output_names=["outputs"]
            ),
            codebase_config=dict(type=Codebase.MMDET3D.value, task=Task.VOXEL_DETECTION.value),
        )
    )
    ...
    rewrite_inputs = {"features": features, "num_points": num_points, "coors": coors}
    ...
    assert np.allclose(model_outputs.shape, rewrite_outputs.shape, rtol=1e-03, atol=1e-03)
...
def test_pointpillars_scatter(backend_type: Backend):
    ...
    deploy_cfg = mmengine.Config(
        dict(
            backend_config=dict(type=backend_type.value),
            onnx_config=dict(input_shape=None, input_names=["voxel_features", "coors"], output_names=["outputs"]),
            codebase_config=dict(type=Codebase.MMDET3D.value, task=Task.VOXEL_DETECTION.value),
        )
    )
...
def get_centerpoint_head():
    ...
    model_cfg.centerpoint_model.pts_bbox_head.test_cfg = model_cfg.centerpoint_model.test_cfg
...
def test_pointpillars(backend_type: Backend):
    ...
    _, data = task_processor.create_input(
        pcd="tests/test_codebase/test_mmdet3d/data/kitti/kitti_000008.bin", data_preprocessor=preproc
    )

    with RewriterContext(
        cfg=deploy_cfg, backend=deploy_cfg.backend_config.type, opset=deploy_cfg.onnx_config.opset_version
    ):
...
def test_centerpoint(backend_type: Backend):
    ...
    task_processor = build_task_processor(centerpoint_model_cfg, deploy_cfg, "cpu")
    model = task_processor.build_pytorch_model(None)
    model.eval()

    preproc = task_processor.build_data_preprocessor()
    _, data = task_processor.create_input(
        pcd="tests/test_codebase/test_mmdet3d/data/nuscenes/n008-2018-09-18-12-07-26-0400__LIDAR_TOP__1537287083900561.pcd.bin",  # noqa: E251  # noqa: E501
