...
def test_build_pytorch_model(from_mmrazor: Any):
    ...
    if from_mmrazor is False:
        ...
        _model_cfg_path = "tests/test_codebase/test_mmpretrain/data/" "mmrazor_model.py"
        _model_cfg = load_config(_model_cfg_path)[0]
        _model_cfg.algorithm.architecture.model.type = "mmpretrain." "ImageClassifier"
        _model_cfg.algorithm.architecture.model.backbone = dict(type="SearchableShuffleNetV2", widen_factor=1.0)
...
def test_build_test_runner():
    ...
    label = LabelData(label=torch.tensor([0]), score=torch.rand(10), metainfo=dict(num_classes=10))
    outputs = [
        DataSample(
            ...
            _gt_label=label,
            metainfo=dict(
                img_shape=(224, 224), img_path="", ori_shape=(300, 400), scale_factor=(0.8525, 0.8533333333333334)
            ),
        )
    ]
