...
try:
    ...
    pytest.skip(f"{Codebase.MMAGIC} is not installed.", allow_module_level=True)
    ...
    dict(
        ...
        backend_config=dict(
            ...
            model_inputs=[
                dict(
                    input_shapes=dict(
                        input=dict(min_shape=[1, 3, 4, 4], opt_shape=[1, 3, 4, 4], max_shape=[1, 3, 4, 4])
                    )
                )
            ],
        ),
...
def test_base_edit_model_forward():
    ...
    class DummyBaseEditModel(BaseEditModel):
        ...
        def forward(
            self, inputs: torch.Tensor, data_samples: Optional[List[DataSample]] = None, mode: str = "tensor", **kwargs
        ):
            return inputs

    generator = dict(type="SRCNNNet", channels=(3, 64, 32, 3), kernel_sizes=(9, 1, 5), upscale_factor=4)
...
def test_srcnn():
    ...
    with RewriterContext(cfg=deploy_cfg, backend=Backend.TENSORRT.value), torch.no_grad():
        torch.onnx.export(
            pytorch_model,
