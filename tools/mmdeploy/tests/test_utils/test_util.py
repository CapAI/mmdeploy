...
class TestGetCodebaseExternalModule:
    ...
    def test_get_codebase_external_module(self):
        ...
        assert isinstance(custom_module_list, list) and len(custom_module_list) == 1
...
class TestIsDynamic:
    config_with_onnx_config = Config(dict(onnx_config=dict(), backend_config=dict(type="default")))

    config_with_dynamic_axes = Config(
        dict(
            onnx_config=dict(type="onnx", dynamic_axes={"input": {0: "batch", 2: "height", 3: "width"}}),
    ...
    config_with_dynamic_axes_and_input_names = Config(
        dict(
            onnx_config=dict(
                type="onnx", input_names=["image"], dynamic_axes={"image": {0: "batch", 2: "height", 3: "width"}}
            ),
            backend_config=dict(type="default"),
        )
    )
    ...
    config_with_dynamic_axes_list = Config(
        dict(
            onnx_config=dict(type="onnx", input_names=["image"], dynamic_axes=[[0, 2, 3]]),
            backend_config=dict(type="default"),
        )
    )

    def test_is_dynamic_batch_none(self):
        assert util.is_dynamic_batch(TestIsDynamic.config_with_onnx_config) is False

    def test_is_dynamic_batch_error_name(self):
        assert util.is_dynamic_batch(TestIsDynamic.config_with_dynamic_axes, "output") is False

    def test_is_dynamic_batch(self):
        assert util.is_dynamic_batch(TestIsDynamic.config_with_dynamic_axes) is True

    def test_is_dynamic_batch_axes_list(self):
        assert util.is_dynamic_batch(TestIsDynamic.config_with_dynamic_axes_list) is True

    def test_is_dynamic_shape_none(self):
        assert util.is_dynamic_shape(TestIsDynamic.config_with_onnx_config) is False

    def test_is_dynamic_shape_error_name(self):
        assert util.is_dynamic_shape(TestIsDynamic.config_with_dynamic_axes, "output") is False

    def test_is_dynamic_shape(self):
        assert util.is_dynamic_shape(TestIsDynamic.config_with_dynamic_axes) is True

    def test_is_dynamic_shape_input_names(self):
        assert util.is_dynamic_shape(TestIsDynamic.config_with_dynamic_axes_and_input_names) is True

    def test_is_dynamic_shape_different_names(self):
        config_with_different_names = TestIsDynamic.config_with_dynamic_axes_and_input_names
        util.get_ir_config(config_with_different_names).input_names = "another_name"
        assert util.is_dynamic_shape(config_with_different_names) is False

    def test_is_dynamic_shape_axes_list(self):
        assert util.is_dynamic_shape(TestIsDynamic.config_with_dynamic_axes_list) is True


class TestGetInputShape:
    config_without_input_shape = Config(dict(onnx_config=dict(input_shape=None)))
    config_with_input_shape = Config(dict(onnx_config=dict(input_shape=[1, 1])))
    config_with_error_shape = Config(dict(onnx_config=dict(input_shape=[1, 1, 1])))

    def test_get_input_shape_none(self):
        assert util.get_input_shape(TestGetInputShape.config_without_input_shape) is None
    ...
    def test_get_input_shape(self):
        assert util.get_input_shape(TestGetInputShape.config_with_input_shape) == [1, 1]
...
class TestGetPartitionConfig:
    config_with_mask = Config(dict(partition_config=dict(apply_marks=True)))
    config_without_mask = Config(dict(partition_config=dict(apply_marks=False)))
    ...
    def test_get_partition_config_without_mask(self):
        assert util.get_partition_config(TestGetPartitionConfig.config_without_mask) is None

    def test_get_partition_config(self):
        assert util.get_partition_config(TestGetPartitionConfig.config_with_mask) == dict(apply_marks=True)


class TestGetCalib:
    config_with_calib = Config(dict(calib_config=dict(create_calib=True, calib_file="calib_data.h5")))

    config_without_calib = Config(dict(calib_config=dict(create_calib=False, calib_file="calib_data.h5")))
    ...
    def test_get_calib_filename_false(self):
        assert util.get_calib_filename(TestGetCalib.config_without_calib) is None

    def test_get_calib_filename(self):
        assert util.get_calib_filename(TestGetCalib.config_with_calib) == "calib_data.h5"


class TestGetCommonConfig:
    config_with_common_config = Config(dict(backend_config=dict(type="tensorrt", common_config=dict(fp16_mode=False))))

    def test_get_common_config(self):
        assert util.get_common_config(TestGetCommonConfig.config_with_common_config) == dict(fp16_mode=False)


class TestGetModelInputs:
    config_with_model_inputs = Config(dict(backend_config=dict(model_inputs=[dict(input_shapes=None)])))

    def test_model_inputs(self):
        assert util.get_model_inputs(TestGetModelInputs.config_with_model_inputs) == [dict(input_shapes=None)]
...
class TestGetDynamicAxes:
    ...
    def test_can_get_axes_from_dict(self):
        expected_dynamic_axes = {self.input_name: {0: "batch", 2: "height", 3: "width"}}
        deploy_cfg = Config(dict(onnx_config=dict(dynamic_axes=expected_dynamic_axes)))
    ...
    def test_can_get_axes_from_list_with_cfg(self):
        ...
        axes = [[0, 2, 3], [0]]
        expected_dynamic_axes = {self.input_name: axes[0], output_name: axes[1]}
        deploy_cfg = Config(
            dict(onnx_config=dict(input_names=[self.input_name], output_names=[output_name], dynamic_axes=axes))
        )
        dynamic_axes = util.get_dynamic_axes(deploy_cfg)
        assert expected_dynamic_axes == dynamic_axes
...
def test_AdvancedEnum():
    ...
    vals = ["TextDetection", "TextRecognition", "Segmentation", "SuperResolution", "Classification", "ObjectDetection"]
    for k, v in zip(keys, vals):
        assert Task.get(v) == k
        assert k.value == v


@pytest.mark.skipif(not importlib.util.find_spec("mmagic"), reason="requires mmagic")
