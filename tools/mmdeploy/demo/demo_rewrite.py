...
@FUNCTION_REWRITER.register_rewriter(func_name="torchvision.models.ResNet._forward_impl")
...
async def visualize(original_path: str, rewritten_path: str):
    ...
    await page1.screenshot(
        {"path": original_path.replace(".onnx", ".png")},
        clip={"x": width / 4, "y": 0, "width": width / 2, "height": height},
    )
    await page2.screenshot(
        {"path": rewritten_path.replace(".onnx", ".png")},
        clip={"x": width / 4, "y": 0, "width": width / 2, "height": height},
    )
    await browser.close()
...
if __name__ == "__main__":
    ...
    asyncio.get_event_loop().run_until_complete(visualize(original_file_path, rewritten_file_path))
    import mmcv
