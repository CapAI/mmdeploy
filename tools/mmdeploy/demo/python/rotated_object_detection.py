...
def parse_args():
    ...
    parser.add_argument("model_path", help="path of SDK model dumped by model converter")
...
def main():
    ...
    for index, rbbox, label_id in zip(indices, rbboxes, labels):
        ...
        if score < 0.1:
            continue
        [wx, wy, hx, hy] = 0.5 * np.array([w, w, -h, h]) * np.array([cos(angle), sin(angle), sin(angle), cos(angle)])
        points = np.array(
            [
                [
                    [int(cx - wx - hx), int(cy - wy - hy)],
                    [int(cx + wx - hx), int(cy + wy - hy)],
                    [int(cx + wx + hx), int(cy + wy + hy)],
                    [int(cx - wx + hx), int(cy - wy + hy)],
                ]
