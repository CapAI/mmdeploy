...
def parse_args():
    parser = argparse.ArgumentParser(description="Demo of MMDeploy SDK pipeline API")
...
def main():
    ...
    config = dict(
        ...
        input="img",
        tasks=[
            dict(type="Inference", input="img", output="dets", params=dict(model=det_model)),
            dict(
                type="Pipeline",
                # flatten dets ([[a]] -> [a]) and broadcast img
                input=["boxes=*dets", "imgs=+img"],
                tasks=[
                    dict(type="Task", module="CropBox", input=["imgs", "boxes"], output="patches"),
                    dict(type="Inference", input="patches", output="labels", params=dict(model=reg_model)),
                ],
