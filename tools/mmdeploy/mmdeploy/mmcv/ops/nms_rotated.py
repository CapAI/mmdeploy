...
def select_rnms_index(
    ...
    topk_batch_inds = torch.arange(batch_size, dtype=topk_inds.dtype, device=topk_inds.device).unsqueeze(1)
...
def _multiclass_nms_rotated(
    ...
    selected_indices = ONNXNMSRotatedOp.apply(boxes, scores, iou_threshold, score_threshold)

    dets, labels = select_rnms_index(scores, boxes, selected_indices, batch_size, keep_top_k=keep_top_k)

    return dets, labels


@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdeploy.mmcv.ops.nms_rotated._multiclass_nms_rotated", backend="tensorrt"
)
def multiclass_nms_rotated__tensorrt(
    ...
    dets, labels = TRTBatchedRotatedNMSop.apply(
        boxes, scores, int(scores.shape[-1]), pre_top_k, keep_top_k, iou_threshold, score_threshold, -1
    )
