...
def extract_partition(
    ...
    for s in start_marker:
        ...
        for node in model.graph.node:
            if node.op_type == "Mark":
                ...
                if attr["func"] == start_name and attr["type"] == start_type and attr["func_id"] == func_id:
                    name = node.input[0]
                    if name not in inputs:
                        new_name = get_new_name(attr, mark_name=s, name_map=start_name_map)
                        rename_value(model, name, new_name)
                        if not any([v_info.name == new_name for v_info in model.graph.value_info]):
                            new_val_info = onnx.helper.make_tensor_value_info(new_name, attr["dtype"], attr["shape"])
    ...
    for e in end_marker:
        ...
        for node in model.graph.node:
            if node.op_type == "Mark":
                attr = attribute_to_dict(node.attribute)
                if attr["func"] == end_name and attr["type"] == end_type and attr["func_id"] == func_id:
                    name = node.output[0]
                    if name not in outputs:
                        new_name = get_new_name(attr, mark_name=e, name_map=end_name_map)
                        rename_value(model, name, new_name)
                        if not any([v_info.name == new_name for v_info in model.graph.value_info]):
                            new_val_info = onnx.helper.make_tensor_value_info(new_name, attr["dtype"], attr["shape"])
                            model.graph.value_info.append(new_val_info)
    ...
    for xs in [extracted_model.graph.output]:
        for x in xs:
            if not x.type.tensor_type.shape.dim:
                ...
                x.CopyFrom(onnx.helper.make_tensor_value_info(x.name, x.type.tensor_type.elem_type, []))
