...
def update_squeeze_unsqueeze_opset13_pass(graph, params_dict, torch_out):
    ...
    for node in graph.nodes():
        if node.kind() in ["onnx::Squeeze", "onnx::Unsqueeze"] and node.hasAttribute("axes"):
            axes = node["axes"]
...
def model_to_graph__custom_optimizer(*args, **kwargs):
    ...
    if opset_version >= 13:
        graph, params_dict, torch_out = update_squeeze_unsqueeze_opset13_pass(graph, params_dict, torch_out)
    custom_passes = getattr(ctx, "onnx_custom_passes", None)

    if custom_passes is not None:
        assert isinstance(custom_passes, Callable), f"Expect a callable onnx_custom_passes, get {type(custom_passes)}."
        graph, params_dict, torch_out = custom_passes(ctx, graph, params_dict, torch_out)

    return graph, params_dict, torch_out


@FUNCTION_REWRITER.register_rewriter("torch._C._jit_pass_onnx_deduplicate_initializers", backend="tensorrt")
...
@FUNCTION_REWRITER.register_rewriter("torch._C._jit_pass_onnx_autograd_function_process")
