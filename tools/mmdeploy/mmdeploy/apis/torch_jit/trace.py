...
def trace(
    ...
    if "backend" not in context_info:
        ...
        logger.warning(f'Find backend {context_info["backend"]} in context_info.' f" Expect {backend}.")
    if "ir" not in context_info:
        context_info["ir"] = IR.TORCHSCRIPT
    elif context_info["ir"] != backend:
        logger.warning(f'Find ir {context_info["ir"]} in context_info.' f" Expect {IR.TORCHSCRIPT}.")
    ...
    with RewriterContext(**context_info), torch.no_grad():
        ...
        ts_model = torch.jit.trace(func, inputs, check_trace=check_trace, check_tolerance=check_tolerance)
