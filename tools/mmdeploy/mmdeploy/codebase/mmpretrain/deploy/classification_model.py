...
class End2EndModel(BaseBackendModel):
    ...
    def _init_wrapper(self, backend: Backend, backend_files: Sequence[str], device: str, **kwargs):
    ...
    def forward(
        ...
        cls_score = self.wrapper({self.input_name: inputs})[self.output_names[0]]
        ...
        if self.head is None or not isinstance(self.head, MultiLabelClsHead):
            ...
            if data_samples is not None:
                ...
                for score, label in zip(pred_scores, pred_labels):
                    data_samples.append(DataSample().set_pred_score(score).set_pred_label(label))
        else:
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        cls_score = []
        for input in inputs:
            pred = self.wrapper.invoke(input.permute(1, 2, 0).contiguous().detach().cpu().numpy())
        ...
        predict = ClsHead._get_predictions(None, cls_score, data_samples=data_samples)
        return predict
