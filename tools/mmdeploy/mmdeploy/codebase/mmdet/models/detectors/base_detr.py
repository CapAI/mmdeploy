...
def __predict_impl(self, batch_inputs, data_samples, rescale):
    ...
    results_list = self.bbox_head.predict(**head_inputs_dict, rescale=rescale, batch_data_samples=data_samples)
    return results_list
...
@FUNCTION_REWRITER.register_rewriter("mmdet.models.detectors.base_detr.DetectionTransformer.predict")
def detection_transformer__predict(
    self, batch_inputs: torch.Tensor, data_samples: OptSampleList = None, rescale: bool = True, **kwargs
) -> ForwardResults:
