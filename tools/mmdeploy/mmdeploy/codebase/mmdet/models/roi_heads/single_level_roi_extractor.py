...
@FUNCTION_REWRITER.register_rewriter(
    "mmdet.models.roi_heads.roi_extractors." "single_level_roi_extractor.SingleRoIExtractor.forward", backend="tensorrt"
)
def single_roi_extractor__forward__tensorrt(self, feats, rois, roi_scale_factor=None):
    ...
    for roi_layer in self.roi_layers:
        assert isinstance(roi_layer, RoIAlign), f"{type(roi_layer)} is not supported in TensorRT."
    ...
    return MultiLevelRoiAlign.apply(
        *feats, rois, out_size, pool_mode, sampling_ratio, roi_scale_factor, finest_scale, featmap_strides, aligned
    )
...
@FUNCTION_REWRITER.register_rewriter(
    "mmdet.models.roi_heads.roi_extractors." "single_level_roi_extractor.SingleRoIExtractor.forward", backend="ascend"
)
def single_roi_extractor__forward__ascend(self, feats, rois, roi_scale_factor=None):
    ...
    for roi_layer in self.roi_layers:
        assert isinstance(roi_layer, RoIAlign), f"{type(roi_layer)} is not supported in Ascend."
...
@mark("roi_extractor", inputs=["feats", "rois"], outputs=["bbox_feats"])
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.roi_heads.SingleRoIExtractor.forward")
def single_roi_extractor__forward(self, feats, rois, roi_scale_factor=None):
    ...
    """
    ctx = FUNCTION_REWRITER.get_context("mmdet.models.roi_heads.SingleRoIExtractor.forward")
    ...
    roi_feats = torch.cat((roi_feats.new_zeros(num_levels * 2, *roi_feats.shape[-3:]), roi_feats))
    rois = torch.cat((rois.new_zeros(num_levels * 2, 5), rois))
    _tmp = torch.linspace(0, num_levels - 1, num_levels, dtype=target_lvls.dtype, device=target_lvls.device)
...
@FUNCTION_REWRITER.register_rewriter(
    "mmdet.models.roi_heads.roi_extractors." "single_level_roi_extractor.SingleRoIExtractor.forward", backend="openvino"
)
...
@mark("roi_extractor", inputs=["feats", "rois"], outputs=["bbox_feats"])
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.roi_heads.SingleRoIExtractor.forward", backend=Backend.TVM.value
)
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.roi_heads.SingleRoIExtractor.forward", backend=Backend.COREML.value
)
