...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.backbones.csp_darknet.Focus.forward")
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.backbones.csp_darknet.Focus.forward", backend="ncnn")
def focus__forward__ncnn(self, x):
    ...
    x = x.reshape(_b, c * 4, torch.div(h, 2, rounding_mode="floor"), torch.div(w, 2, rounding_mode="floor"))

    return self.conv(x)


@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.backbones.swin.WindowMSA.forward", backend="tensorrt")
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.backbones.swin.ShiftWindowMSA.window_reverse", backend="tensorrt"
)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.backbones.swin.ShiftWindowMSA.window_partition", backend="tensorrt"
)
...
@FUNCTION_REWRITER.register_rewriter(func_name="mmdet.models.backbones.swin.ShiftWindowMSA.forward")
def shift_window_msa__forward__default(self, query, hw_shape):
    ...
    query = torch.cat([query, query.new_zeros(B, C, self.window_size, query.shape[-1])], dim=-2)
    slice_h = torch.div((H + self.window_size - 1), self.window_size, rounding_mode="floor") * self.window_size
    slice_w = torch.div((W + self.window_size - 1), self.window_size, rounding_mode="floor") * self.window_size
