...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdet.models.dense_heads.rtmdet_ins_head." "RTMDetInsHead.predict_by_feat"
)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmdeploy.codebase.mmdet.models." "dense_heads.rtmdet_ins_head._nms_with_mask_static", backend="tensorrt"
)
def _nms_with_mask_static__tensorrt(
    ...
    dets, labels, inds = TRTBatchedNMSop.apply(
        boxes, scores, int(scores.shape[-1]), pre_top_k, keep_top_k, iou_threshold, score_threshold, -1, True
    )
...
def _mask_predict_by_feat_single(self, mask_feat, kernels, priors):
    ...
    coord = self.prior_generator.single_level_grid_priors((h, w), level_idx=0).reshape(1, -1, 2).to(mask_feat.device)
    num_inst = priors.shape[0]
