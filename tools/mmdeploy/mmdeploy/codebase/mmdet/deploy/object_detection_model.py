...
class End2EndModel(BaseBackendModel):
    ...
    @staticmethod
    def __clear_outputs(test_outputs: List[Union[Tensor, np.ndarray]]) -> List[Union[List[Tensor], List[np.ndarray]]]:
    ...
    def postprocessing_masks(
        ...
        for bbox, mask in zip(bboxes, masks):
            ...
            img_masks = F.grid_sample(
                mask.to(dtype=torch.float32)[None, None, :, :], grid[None, :, :, :], align_corners=False
            )
        ...
        for i in range(batch_size):
            ...
            if batch_masks is not None:
                ...
                if self.deploy_cfg is not None:
                    ...
                    export_postprocess_mask = mmdet_deploy_cfg.get("export_postprocess_mask", False)
                if not export_postprocess_mask:
                    masks = End2EndModel.postprocessing_masks(dets[:, :4], masks, ori_w, ori_h, self.device)
                else:
                ...
                if export_postprocess_mask and rescale and masks.shape[0] != 0:
                    masks = torch.nn.functional.interpolate(
                        masks.unsqueeze(0),
                        size=[
                            math.ceil(masks.shape[-2] / img_metas[i]["scale_factor"][0]),
                            math.ceil(masks.shape[-1] / img_metas[i]["scale_factor"][1]),
                        ],
                    )[..., :ori_h, :ori_w]
...
class PartitionSingleStageModel(End2EndModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(backend, backend_files, device, deploy_cfg, data_preprocessor, **kwargs)
...
class PartitionTwoStageModel(End2EndModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bbox_roi_extractor = build_roi_extractor(model_cfg.model.roi_head.bbox_roi_extractor)
    ...
    def _init_wrapper(self, backend, backend_files, device):
        ...
        partition0_output_names = ["feat/{}".format(i) for i in range(num_feat)] + ["scores", "boxes"]

        self.first_wrapper = BaseBackendModel._build_wrapper(
            backend, backend_files[0:n], device, output_names=partition0_output_names, deploy_cfg=self.deploy_cfg
        )
    ...
    def partition0_postprocess(self, x: Sequence[Tensor], scores: Tensor, bboxes: Tensor):
        """Perform post-processing for partition 0.
    ...
    def partition1_postprocess(self, rois: Tensor, cls_score: Tensor, bbox_pred: Tensor, img_metas: Sequence[dict]):
...
class NCNNEnd2EndModel(End2EndModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(NCNNEnd2EndModel, self).__init__(backend, backend_files, device, deploy_cfg, **kwargs)
        # load cfg if necessary
        model_cfg = load_config(model_cfg)[0]
        self.model_cfg = model_cfg
...
class VACCDetModel(End2EndModel):
    def __init__(
        ...
        **kwargs,
        ...
        assert backend == Backend.VACC, "only supported vacc, but give " f"{backend.value}"

        super(VACCDetModel, self).__init__(backend, backend_files, device, deploy_cfg, **kwargs)
        # load cfg if necessary
        model_cfg = load_config(model_cfg)[0]
        self.model_cfg = model_cfg
    ...
    def _get_bboxes(self, outputs, img_metas):
        ...
        if head_cfg.type == "YOLOV3Head":
            ret = head.predict_by_feat(outputs, [dict(scale_factor=None)], cfg=self.model_cfg._cfg_dict.model.test_cfg)
        else:
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(
        self, inputs: torch.Tensor, data_samples: Optional[List[BaseDataElement]] = None, mode: str = "predict"
    ):
        ...
        dets, labels, masks = self.wrapper.invoke(inputs.contiguous().detach().cpu().numpy())
        result = InstanceData()
        if self.has_mask:
            segm_results = []
...
class RKNNModel(End2EndModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(RKNNModel, self).__init__(backend, backend_files, device, deploy_cfg, **kwargs)
        # load cfg if necessary
        model_cfg = load_config(model_cfg)[0]
        self.model_cfg = model_cfg
    ...
    def _init_wrapper(self, backend, backend_files, device):
        ...
        if self.deploy_cfg is not None:
            ...
            if get_partition_config(self.deploy_cfg) is not None:
                output_names = get_partition_config(self.deploy_cfg)["partition_cfg"][0]["output_names"]

        self.wrapper = BaseBackendModel._build_wrapper(
            backend, backend_files, device, output_names=output_names, deploy_cfg=self.deploy_cfg
        )
    ...
    def _get_bboxes(self, outputs: List[Tensor], metainfos: Any):
        ...
        if head_cfg.type == "YOLOXHead":
            ...
            ret = head.predict_by_feat(outputs, metainfos, cfg=self.model_cfg._cfg_dict.model.test_cfg, rescale=True)
        elif head_cfg.type == "YOLOv5Head":
            from mmdet.models.utils import multi_apply

            outs = multi_apply(self.yolov5_head_forward_single, outputs, [head.num_base_priors] * len(outputs))
            ret = head.predict_by_feat(
                *outs, batch_img_metas=metainfos, cfg=self.model_cfg._cfg_dict.model.test_cfg, rescale=True
            )
            ...
            if partition_cfgs is None:  # bbox decoding done in rknn model
                ...
                for i, instance_data in enumerate(ret):
                    ...
                    instance_data.bboxes = scale_boxes(instance_data.bboxes, scale_factor)
                return ret
            divisor = round(len(outputs) / 2)
            ret = head.predict_by_feat(
                outputs[:divisor],
    ...
    def yolov5_head_forward_single(self, pred_map: Tensor, num_base_priors: int) -> Tuple[Tensor, Tensor, Tensor]:
