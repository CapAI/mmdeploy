...
class MMRazor(MMCodebase):
    ...
    def build_task_processor(cls, model_cfg: Config, deploy_cfg: Config, device: str):
        ...
        """
        return ModelCompress(model_cfg=model_cfg, deploy_cfg=deploy_cfg, device=device)
...
class ModelCompress(BaseTask):
    ...
    def __init__(self, model_cfg: Config, deploy_cfg: Config, device: str, experiment_name: str = "BaseTask"):
        ...
        self.base_task = build_task_processor(self.origin_model_cfg, deploy_cfg, device)
    ...
    def revert_model_cfg(self, model_cfg: Config):
        ...
        if "data_preprocessor" in origin_model:
            origin_model_cfg["data_preprocessor"] = origin_model["data_preprocessor"]
        return origin_model_cfg

    # abstract method

    def build_backend_model(self, model_files=None, data_preprocessor_updater=None, **kwargs) -> torch.nn.Module:
        """Build backend model for using base task."""
        return self.base_task.build_backend_model(model_files, data_preprocessor_updater, **kwargs)
    ...
    def create_input(
        ...
        **kwargs
        ...
        return self.base_task.create_input(imgs, input_shape, data_preprocessor, **kwargs)
    ...
    def build_pytorch_model(
        ...
        mmdeploy."""
        model = super().build_pytorch_model(model_checkpoint, cfg_options, **kwargs)
