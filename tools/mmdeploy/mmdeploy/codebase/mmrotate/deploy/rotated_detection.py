...
class RotatedDetection(BaseTask):
    ...
    def __init__(self, model_cfg: Config, deploy_cfg: Config, device: str, experiment_name: str = "RotatedDetection"):
    ...
    def build_backend_model(self, model_files: Optional[str] = None, **kwargs) -> torch.nn.Module:
        ...
        data_preprocessor = copy.deepcopy(self.model_cfg.model.get("data_preprocessor", {}))
        data_preprocessor.setdefault("type", "mmdet.DetDataPreprocessor")

        model = build_rotated_detection_model(
            model_files, self.deploy_cfg, device=self.device, data_preprocessor=data_preprocessor
        )
    ...
    def create_input(
        ...
        if not dynamic_flag:
            for i, trans in enumerate(pipeline):
                if trans["type"] == "Pad" and "pad_to_square" in trans and trans["pad_to_square"]:
                    trans.pop(i)
    ...
    def visualize(self, image: Union[str, np.ndarray], result: list, output_file: str, *args, **kwargs):
        ...
        return super().visualize(image, result.cpu(), output_file, *args, **kwargs)
