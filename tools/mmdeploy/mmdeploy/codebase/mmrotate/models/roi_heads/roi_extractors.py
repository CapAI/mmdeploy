...
@FUNCTION_REWRITER.register_rewriter(
    "mmrotate.models.roi_heads.roi_extractors." "rotate_single_level_roi_extractor.RotatedSingleRoIExtractor.forward",
    backend="tensorrt",
)
@mark("rotated_roi_extractor", inputs=["feats", "rois"], outputs=["bbox_feats"])
def rotated_single_roi_extractor__forward__tensorrt(self, feats, rois, roi_scale_factor=None):
    ...
    for roi_layer in self.roi_layers:
        assert isinstance(roi_layer, RoIAlignRotated), f"{type(roi_layer)} is not supported in TensorRT."
    ...
    return MultiLevelRotatedRoiAlign.apply(
        *feats, rois, out_size, clockwise, sampling_ratio, roi_scale_factor, finest_scale, featmap_strides, aligned
    )
