...
from mmdeploy.utils import Codebase, Task, get_codebase_config, get_input_shape, get_root_logger
...
def process_model_config(
    ...
    if input_shape is not None:
        ...
        if not found_resize:
            ...
            logger.warning(f"Not found Resize in test_pipeline: {cfg.test_pipeline}")
...
def _get_dataset_metainfo(model_cfg: Config):
    ...
    for dataloader_name in ["test_dataloader", "val_dataloader", "train_dataloader"]:
        ...
        if dataset_mmseg is None:
            continue
        if hasattr(dataset_mmseg, "_load_metainfo") and isinstance(dataset_mmseg._load_metainfo, Callable):
            meta = dataset_mmseg._load_metainfo(dataset_cfg.get("metainfo", None))
            if meta is not None:
                return meta
...
class Segmentation(BaseTask):
    ...
    def visualize(
        ...
        **kwargs,
        ...
        visualizer.add_datasample(name, image, data_sample=result.cpu(), show=show_result)
    ...
    def get_preprocess(self, *args, **kwargs) -> Dict:
        ...
        preprocess.append(dict(type="Normalize", mean=dp.mean, std=dp.std, to_rgb=dp.bgr_to_rgb))
        preprocess.append(dict(type="ImageToTensor", keys=["img"]))
        preprocess.append(
            dict(
                type="Collect",
                keys=["img"],
                meta_keys=["img_shape", "pad_shape", "ori_shape", "img_norm_cfg", "scale_factor"],
            )
        )
        return preprocess
    ...
    def get_postprocess(self, *args, **kwargs) -> Dict:
        ...
        if isinstance(params, list):
            params = params[-1]
        with_argmax = get_codebase_config(self.deploy_cfg).get("with_argmax", True)
