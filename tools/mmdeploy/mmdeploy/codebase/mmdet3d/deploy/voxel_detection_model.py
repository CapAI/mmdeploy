...
class VoxelDetectionModel(BaseBackendModel):
    ...
    def forward(self, inputs: dict, data_samples: Optional[List[BaseDataElement]] = None, **kwargs) -> Any:
        ...
        new_outputs = dict(
            ...
            dir_cls_pred=[outputs[f"dir_cls_pred{i}"] for i in range(num_level)],
        )
        outputs = new_outputs
        ...
        prediction = VoxelDetectionModel.postprocess(
            model_cfg=self.model_cfg, deploy_cfg=self.deploy_cfg, outs=outputs, metas=data_samples
        )

        return prediction

    def show_result(self, data: Dict, result: List, out_dir: str, file_name: str, show=False, snapshot=False, **kwargs):
        ...
        show_result(
            points, None, pred_bboxes, out_dir, file_name, show=show, snapshot=snapshot, pred_labels=pred_labels
        )
    ...
    @staticmethod
    def postprocess(model_cfg: Union[str, Config], deploy_cfg: Union[str, Config], outs: Dict, metas: Dict):
        ...
        if not hasattr(head, "task_heads"):
            ...
            for task_id in range(len(head.num_classes)):
                ...
                batch_heatmap = cls_score[:, scores_range[task_id] : scores_range[task_id + 1], ...].sigmoid()

                batch_reg = bbox_pred[:, bbox_range[task_id] : bbox_range[task_id] + 2, ...]
                batch_hei = bbox_pred[:, bbox_range[task_id] + 2 : bbox_range[task_id] + 3, ...]

                if head.norm_bbox:
                    batch_dim = torch.exp(bbox_pred[:, bbox_range[task_id] + 3 : bbox_range[task_id] + 6, ...])
                else:
                    batch_dim = bbox_pred[:, bbox_range[task_id] + 3 : bbox_range[task_id] + 6, ...]

                batch_vel = bbox_pred[:, bbox_range[task_id] + 6 : bbox_range[task_id + 1], ...]

                batch_rots = dir_cls_pred[:, dir_range[task_id] : dir_range[task_id + 1], ...][:, 0].unsqueeze(1)
                batch_rotc = dir_cls_pred[:, dir_range[task_id] : dir_range[task_id + 1], ...][:, 1].unsqueeze(1)
                ...
                if pts["nms_type"] == "circle":
                    ...
                    keep = torch.tensor(
                        circle_nms(
                            boxes.detach().cpu().numpy(), pts["min_radius"][task_id], post_max_size=pts["post_max_size"]
                        ),
                        dtype=torch.long,
                        device=boxes.device,
                    )
            ...
            for i in range(num_samples):
                ...
                for k in rets[0][i].keys():
                    if k == "bboxes":
                        ...
                        bboxes = batch_input_metas[i]["box_type_3d"](bboxes, head.bbox_coder.code_size)
                    elif k == "scores":
