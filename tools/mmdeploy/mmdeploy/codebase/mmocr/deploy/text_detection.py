...
def process_model_config(
    ...
    for i, transform in enumerate(pipeline):
        if transform.type == "PackTextDetInputs":
            pipeline[i].meta_keys = tuple(j for j in pipeline[i].meta_keys if j != "instances")
        # for static exporting
        if input_shape is not None:
            if transform.type in ("Resize", "ShortScaleAspectJitter"):
                pipeline[i] = mmengine.ConfigDict(dict(type="Resize", scale=input_shape, keep_ratio=False))

    pipeline = [transform for transform in pipeline if transform.type != "LoadOCRAnnotations"]
...
class TextDetection(BaseTask):
    ...
    def build_backend_model(self, model_files: Optional[str] = None, **kwargs) -> torch.nn.Module:
        ...
        model = build_text_detection_model(model_files, self.model_cfg, self.deploy_cfg, device=self.device, **kwargs)
        return model.eval()
