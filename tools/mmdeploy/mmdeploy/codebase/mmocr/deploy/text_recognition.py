...
def process_model_config(
    ...
    for i, transform in enumerate(test_pipeline):
        if transform.type == "PackTextRecogInputs":
            test_pipeline[i].meta_keys = tuple(j for j in test_pipeline[i].meta_keys if j != "instances")

        # for static exporting
        if input_shape is not None and transform.type == "RescaleToHeight":
            resize = {"height": input_shape[1], "min_width": input_shape[0], "max_width": input_shape[0]}
            test_pipeline[i].update(resize)

    test_pipeline = [transform for transform in test_pipeline if transform.type != "LoadOCRAnnotations"]
...
def _get_dataset_metainfo(model_cfg: Config):
    ...
    for dataloader_name in ["test_dataloader", "val_dataloader", "train_dataloader"]:
        ...
        if dataset_cls is None:
            continue
        if hasattr(dataset_cls, "_load_metainfo") and isinstance(dataset_cls._load_metainfo, Callable):
...
class TextRecognition(BaseTask):
    ...
    def build_backend_model(self, model_files: Optional[str] = None, **kwargs) -> torch.nn.Module:
        ...
        model = build_text_recognition_model(model_files, self.model_cfg, self.deploy_cfg, device=self.device)
        return model.eval()
    ...
    def create_input(
        ...
        self.model_cfg = process_model_config(self.model_cfg, imgs, input_shape)
        ...
        for img in imgs:
            ...
            if isinstance(img, np.ndarray):
                # TODO: remove img_id.
                data_ = dict(img=img, img_id=0, ori_shape=input_shape, instances=None)
            else:
                # TODO: remove img_id.
        ...
        data["inputs"] = cast_data_device(data["inputs"], torch.device(self.device))
    ...
    def get_preprocess(self, *args, **kwargs) -> Dict:
        ...
        transforms = [item for item in pipeline if "Random" not in item["type"] and "Annotation" not in item["type"]]
        ...
        transforms.insert(-2, dict(type="Pad", size_divisor=data_preprocessor.get("pad_size_divisor", 1)))
        transforms.insert(
            -3,
            dict(
                type="Normalize",
    ...
    def get_postprocess(self, work_dir: Optional[str] = None, **kwargs) -> Dict:
        ...
        shutil.copy(self.model_cfg.dictionary.dict_file, f"{work_dir}/dict_file.txt")
        params = deepcopy(self.model_cfg.model.decoder.dictionary)
        params.update(dict(dict_file="dict_file.txt"))
        params.update(dict(ignore_chars=postprocess.get("ignore_chars", ["padding"])))
