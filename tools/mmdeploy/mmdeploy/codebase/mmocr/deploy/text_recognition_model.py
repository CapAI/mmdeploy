...
class End2EndModel(BaseBackendModel):
    ...
    def __init__(
        ...
        super(End2EndModel, self).__init__(deploy_cfg=deploy_cfg, data_preprocessor=model_cfg.model.data_preprocessor)
    ...
    def forward(self, inputs: torch.Tensor, data_samples: RecSampleList, *args, **kwargs) -> RecSampleList:
    ...
    def extract_feat(self, imgs: torch.Tensor, *args, **kwargs) -> torch.Tensor:
...
class SDKEnd2EndModel(End2EndModel):
    ...
    def forward(self, inputs: Sequence[torch.Tensor], data_samples: RecSampleList, *args, **kwargs):
        ...
        Returns:
            list[str]: Text label result of each image.
        """
        text, score = self.wrapper.invoke(inputs[0].permute([1, 2, 0]).contiguous().detach().cpu().numpy())
        pred_text = LabelData()
