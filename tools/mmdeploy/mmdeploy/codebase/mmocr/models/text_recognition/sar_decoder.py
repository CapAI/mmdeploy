...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmocr.models.textrecog.decoders.ParallelSARDecoder" "._2d_attention", backend="default"
)
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmocr.models.textrecog.decoders.SequentialSARDecoder" "._2d_attention", backend="default"
)
def sequential_sar_decoder__2d_attention(self, y_prev, feat, holistic_feat, hx1, cx1, hx2, cx2, valid_ratios=None):
    ...
    if self.dec_gru:
        ...
        _, (hx1, cx1) = self.rnn_decoder_layer1(y_prev.unsqueeze(0), (hx1, cx1))
...
@FUNCTION_REWRITER.register_rewriter(
    func_name="mmocr.models.textrecog.decoders.SequentialSARDecoder" ".forward_test", backend="default"
)
def sequential_sar_decoder__forward_test(
    self, feat: torch.Tensor, out_enc: torch.Tensor, data_samples: Optional[Sequence[TextRecogDataSample]] = None
):
    ...
    if data_samples is not None:
        valid_ratios = [data_sample.get("valid_ratio", 1.0) for data_sample in data_samples] if self.mask else None
...
@MODULE_REWRITER.register_rewrite_module("mmocr.models.textrecog.decoders.SequentialSARDecoder", backend="default")
