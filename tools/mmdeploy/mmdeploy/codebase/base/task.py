...
from mmdeploy.utils import get_backend_config, get_codebase, get_codebase_config, get_root_logger
from mmdeploy.utils.config_utils import get_codebase_external_module, get_rknn_quantization
...
class BaseTask(metaclass=ABCMeta):
    ...
    def __init__(self, model_cfg: Config, deploy_cfg: Config, device: str, experiment_name: str = "BaseTask"):
        ...
        if not DefaultScope.check_instance_created(self.experiment_name):
            self.scope = DefaultScope.get_instance(self.experiment_name, scope_name=self.model_cfg.get("default_scope"))
        else:
    ...
    def build_dataset(self, dataset_cfg: Union[str, Config], is_sort_dataset: bool = False, **kwargs) -> Dataset:
        ...
        if is_sort_dataset:
            if is_can_sort_dataset(dataset):
                ...
                logger.info("Sorting the dataset by 'height' and 'width' " "is not possible.")
        return dataset

    @staticmethod
    def build_dataloader(dataloader: Union[DataLoader, Dict], seed: Optional[int] = None) -> DataLoader:
    ...
    def visualize(
        ...
        **kwargs,
        ...
        visualizer.add_datasample(
            name, image, data_sample=result, draw_gt=draw_gt, show=show_result, out_file=output_file
        )
    ...
    def from_mmrazor(self) -> bool:
        ...
        if not isinstance(from_mmrazor, bool):
            raise TypeError("`from_mmrazor` attribute must be boolean type! " f"but got: {from_mmrazor}")
