...
def instance_norm(g, input, num_groups, weight, bias, eps, cudnn_enabled):
    ...
    weight_ = g.op(
        "Constant", value_t=torch.tensor([1.0] * num_groups).type("torch." + input.type().scalarType() + "Tensor")
    )
    bias_ = g.op(
        "Constant", value_t=torch.tensor([0.0] * num_groups).type("torch." + input.type().scalarType() + "Tensor")
    )

    norm_reshaped = g.op("mmdeploy::TRTInstanceNormalization", input_reshaped, weight_, bias_, epsilon_f=eps)
    norm = g.op("Reshape", norm_reshaped, g.op("Shape", input))

    if weight is None or weight.node().mustBeNone():
        weight_value = torch.tensor([1.0]).type("torch." + input.type().scalarType() + "Tensor")
        weight = g.op("Constant", value_t=weight_value)
    if bias is None or bias.node().mustBeNone():
        bias_value = torch.tensor([0.0]).type("torch." + input.type().scalarType() + "Tensor")
    ...
    return add(g, mul(g, norm, _unsqueeze_helper(g, weight, axes)), _unsqueeze_helper(g, bias, axes))
