...
@FUNCTION_REWRITER.register_rewriter(func_name="torch.onnx.symbolic_opset11._prepare_onnx_paddings", backend="tensorrt")
def _prepare_onnx_paddings__tensorrt(g, input, pad):
    ...
    paddings = g.op(
        "Concat", pad, g.op("ConstantOfShape", extension, value_t=torch.tensor([0], dtype=torch.int64)), axis_i=0
    )
    ...
    begins = sym_help._slice_helper(g, paddings, axes=[0], starts=[1], ends=[0xFFFF], steps=[2])
    ends = sym_help._slice_helper(g, paddings, axes=[0], starts=[0], ends=[0xFFFF], steps=[2])
