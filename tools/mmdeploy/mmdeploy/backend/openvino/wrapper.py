...
class OpenVINOWrapper(BaseWrapper):
    ...
    def __init__(self, ir_model_file: str, output_names: Optional[Sequence[str]] = None, **kwargs):
        ...
        for input in self.net.input_info.values():
            ...
            # need batch_size==1
            assert not dims == 4 or batch_size == 1, "Only batch 1 is supported."
        self.device = "cpu"
        self.sess = self.ie.load_network(network=self.net, device_name=self.device.upper(), num_requests=1)
    ...
    def __update_device(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        updated_inputs = {name: data.to(torch.device(self.device)).contiguous() for name, data in inputs.items()}
    ...
    def __process_outputs(self, outputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict[str, torch.Tensor]: The output name and tensor pairs
                after processing.
        """
        outputs = {name: torch.from_numpy(tensor) for name, tensor in outputs.items()}
        cleaned_outputs = {}
    ...
    def __openvino_execute(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
