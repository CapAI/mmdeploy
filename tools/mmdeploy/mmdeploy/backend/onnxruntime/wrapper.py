...
class ORTWrapper(BaseWrapper):
    ...
    def __init__(self, onnx_file: str, device: str, output_names: Optional[Sequence[str]] = None):
        ...
        if osp.exists(ort_custom_op_path):
            ...
            logger.info("Successfully loaded onnxruntime custom ops from " f"{ort_custom_op_path}")
        else:
            logger.warning("The library of onnxruntime custom ops does" f"not exist: {ort_custom_op_path}")
        device_id = parse_device_id(device)
        providers = (
            ["CPUExecutionProvider"] if device == "cpu" else [("CUDAExecutionProvider", {"device_id": device_id})]
        )
