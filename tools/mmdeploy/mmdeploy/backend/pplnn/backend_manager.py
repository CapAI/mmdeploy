...
class PPLNNManager(BaseBackendManager):
    ...
    def to_backend(
        ...
        **kwargs
        ...
        assert is_available(), "PPLNN is not available, please install PPLNN first."

        pplnn_files = []
        for onnx_path in ir_files:
            algo_file = onnx_path.replace(".onnx", ".json")
            model_inputs = get_model_inputs(deploy_cfg)
            assert "opt_shape" in model_inputs, "Expect opt_shape " "in deploy config for PPLNN"
            ...
            from_onnx(onnx_path, algo_prefix, device=device, input_shapes=input_shapes)
